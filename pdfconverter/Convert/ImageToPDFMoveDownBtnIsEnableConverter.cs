...
// Type: pdfconverter.Convert.ImageToPDFMoveDownBtnIsEnableConverter
    ...
    try
    {
      if (values[0] is ToPDFFileItem toPdfFileItem)
      {
        if (values[1] is ToPDFFileItemCollection fileItemCollection)
          return fileItemCollection.IndexOf(toPdfFileItem) == fileItemCollection.Count - 1 ? (object) false : (object) true;
      }
    }
    catch
    {
