...
// Type: pdfconverter.ViewModels.TXTToPDFUCViewModel
...
public class TXTToPDFUCViewModel : ObservableObject
{
  ...
  public TXTToPDFUCViewModel() => this.InitializeEnv();
...
label_6:
    ...
    if (!App.convertType.Equals((object) ConvToPDFType.TxtToPDF) || App.selectedFile == null || App.selectedFile.Length == 0)
      return;
  ...
  public int AddOneFileToFileList(string fileName)
  {
    try
    {
      ...
      ToPDFFileItem toPdfFileItem = new ToPDFFileItem(fileName, ConvToPDFType.TxtToPDF);
      if (toPdfFileItem != null)
      {
        if (UtilsManager.IsnotSupportFile(fileName, UtilManager.TxtExtentions))
        {
  ...
  public RelayCommand AddOneFile
  {
    get
    {
      ...
      {
        string[] strArray = ConvertManager.selectMultiFiles("Text Format", UtilManager.TxtExtention);
  ...
  {
    TXTToPDFUCViewModel toPdfucViewModel = this;
