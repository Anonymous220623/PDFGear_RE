...
// Type: pdfconverter.ViewModels.PPTToPDFUCViewModel
...
public class PPTToPDFUCViewModel : ObservableObject
{
  ...
  public PPTToPDFUCViewModel() => this.InitializeEnv();
...
label_6:
    ...
    if (!App.convertType.Equals((object) ConvToPDFType.PPTToPDF) || App.selectedFile == null || App.selectedFile.Length == 0)
      return;
  ...
  public int AddOneFileToFileList(string fileName)
  {
    try
    {
      ...
      {
        int num = (int) MessageBox.Show(Resources.WinMergeSplitAddFileCheckMsg.Replace("XXX", fileName), UtilManager.GetProductName());
        return 1;
      }
      ...
      ToPDFFileItem toPdfFileItem = new ToPDFFileItem(fileName, ConvToPDFType.PPTToPDF);
      if (toPdfFileItem != null)
      {
        string password = "";
        if (UtilsManager.IsnotSupportFile(fileName, UtilManager.PPTExtentions))
        {
        ...
        else if (ConToPDFUtils.CheckPPTPassword(fileName, out password, Application.Current.MainWindow))
        {
  ...
  public RelayCommand AddOneFile
  {
    get
    {
      ...
      {
        string[] strArray = ConvertManager.selectMultiFiles("Microsoft Office PowerPoint", UtilManager.PPTExtention);
  ...
  {
    PPTToPDFUCViewModel toPdfucViewModel = this;
    ...
    else
    {
      ...
      await Task.Run(TaskExceptionHelper.ExceptionBoundary(new Func<Task>(toPdfucViewModel.\u003CProcessingFiles\u003Eb__70_1))).ConfigureAwait(false);
