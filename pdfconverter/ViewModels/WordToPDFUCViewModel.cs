...
// Type: pdfconverter.ViewModels.WordToPDFUCViewModel
...
public class WordToPDFUCViewModel : ObservableObject
{
  ...
  public WordToPDFUCViewModel() => this.InitializeEnv();
...
label_6:
    ...
    if (!App.convertType.Equals((object) ConvToPDFType.WordToPDF) || App.selectedFile == null || App.selectedFile.Length == 0)
      return;
  ...
  public int AddOneFileToFileList(string fileName)
  {
    try
    {
      ...
      ToPDFFileItem toPdfFileItem = new ToPDFFileItem(fileName, ConvToPDFType.WordToPDF);
      if (toPdfFileItem != null)
      {
        string password = "";
        if (UtilsManager.IsnotSupportFile(fileName, UtilManager.WordExtentions))
        {
        ...
        else if (ConToPDFUtils.CheckWordPassword(fileName, out password, Application.Current.MainWindow))
        {
  ...
  public RelayCommand AddOneFile
  {
    get
    {
      ...
      {
        string[] strArray = ConvertManager.selectMultiFiles("Word document", ".docx;*.doc;");
  ...
  {
    WordToPDFUCViewModel toPdfucViewModel = this;
    ...
    else
    {
      ...
      await Task.Run(TaskExceptionHelper.ExceptionBoundary(new Func<Task>(toPdfucViewModel.\u003CProcessingFiles\u003Eb__72_1))).ConfigureAwait(false);
