...
// Type: pdfconverter.ViewModels.RTFToPDFUCViewModel
...
public class RTFToPDFUCViewModel : ObservableObject
{
  ...
  public RTFToPDFUCViewModel() => this.InitializeEnv();
...
label_6:
    ...
    if (!App.convertType.Equals((object) ConvToPDFType.RtfToPDF) || App.selectedFile == null || App.selectedFile.Length == 0)
      return;
  ...
  public int AddOneFileToFileList(string fileName)
  {
    try
    {
      ...
      ToPDFFileItem toPdfFileItem = new ToPDFFileItem(fileName, ConvToPDFType.RtfToPDF);
      if (toPdfFileItem != null)
      {
        if (UtilsManager.IsnotSupportFile(fileName, UtilManager.RtfExtentions))
        {
  ...
  public RelayCommand AddOneFile
  {
    get
    {
      ...
      {
        string[] strArray = ConvertManager.selectMultiFiles("Rich Test Format", UtilManager.RtfExtention);
  ...
  {
    RTFToPDFUCViewModel toPdfucViewModel = this;
