...
public class ZipFile : IEnumerable, IEnumerable<ZipEntry>, IDisposable
{
  ...
  private static Encoding _defaultEncoding = (Encoding) null;
  ...
  private void _SaveSfxStub(string exeToGenerate, SelfExtractorSaveOptions options)
  {
    ...
    try
    {
      ...
      {
        ZipFile.ExtractorSettings extractorSettings = ((IEnumerable<ZipFile.ExtractorSettings>) ZipFile.SettingsList).Where<ZipFile.ExtractorSettings>((Func<ZipFile.ExtractorSettings, bool>) (x => x.Flavor == options.Flavor)).FirstOrDefault<ZipFile.ExtractorSettings>();
        if ((object) extractorSettings == null)
            ...
            using (Stream stream = (Stream) zipFile[fileName].OpenReader())
            {
              ...
              using (StreamReader streamReader = new StreamReader(stream))
              {
                while (streamReader.Peek() >= 0)
                {
                  ...
                  string str5 = str4;
                  bool flag = options.RemoveUnpackedFilesAfterExecute;
                  string newValue3 = flag.ToString();
                  string str6 = str5.Replace("@@REMOVE_AFTER_EXECUTE", newValue3);
                  flag = options.Quiet;
                  string newValue4 = flag.ToString();
                  string str7 = str6.Replace("@@QUIET", newValue4);
                  if (!string.IsNullOrEmpty(options.SfxExeWindowTitle))
                    str7 = str7.Replace("@@SFX_EXE_WINDOW_TITLE", options.SfxExeWindowTitle);
                  string str8 = str7.Replace("@@EXTRACT_EXISTING_FILE", ((int) options.ExtractExistingFile).ToString());
                  if (newValue2 != null)
                    str8 = str8.Replace("@@POST_UNPACK_CMD_LINE", newValue2);
                  stringBuilder.Append(str8).Append("\n");
                }
              }
              stringBuilder.Append("\n\n");
            }
        ...
        CompilerResults compilerResults = csharpCodeProvider.CompileAssemblyFromSource(options1, str9);
        if (compilerResults == null)
          throw new SfxGenerationException("Cannot compile the extraction logic!");
        if (this.Verbose)
        {
          foreach (string str10 in compilerResults.Output)
            this.StatusMessageTextWriter.WriteLine(str10);
        }
        if (compilerResults.Errors.Count != 0)
        {
          using (TextWriter textWriter = (TextWriter) new StreamWriter(tempPathname))
          {
            textWriter.Write(str9);
            textWriter.Write("\n\n\n// ------------------------------------------------------------------\n");
  ...
  {
    ze._container = ze._container == null ? new ZipContainer((object) this) : throw new InvalidOperationException("Entry already belongs to a zip file");
  ...
  internal void InternalAddEntry(string name, ZipEntry entry)
  {
    ...
    if (!this._entriesInsensitive.ContainsKey(name))
      this._entriesInsensitive.Add(name, entry);
  ...
  {
    return this.RetrievalEntries.ContainsKey(SharedUtilities.NormalizePathForUseInZipFile(name));
  }

  public bool CaseSensitiveRetrieval
  {
    get => this._CaseSensitiveRetrieval;
    set => this._CaseSensitiveRetrieval = value;
  }

  private Dictionary<string, ZipEntry> RetrievalEntries
  {
    get => !this.CaseSensitiveRetrieval ? this._entriesInsensitive : this._entries;
  }

  public bool IgnoreDuplicateFiles
  {
    get => this._IgnoreDuplicateFiles;
    set => this._IgnoreDuplicateFiles = value;
  }
  ...
  public bool? RequiresZip64
  {
    get
    {
      ...
      foreach (ZipEntry zipEntry in this._entries.Values)
      {
        bool? requiresZip64 = zipEntry.RequiresZip64;
        if (requiresZip64.Value)
        {
          requiresZip64 = new bool?(true);
  ...
  public int MaxOutputSegmentSize
  {
    get
    {
      return this._maxOutputSegmentSize <= (long) int.MaxValue ? (int) this._maxOutputSegmentSize : throw new ZipException("MaxOutputSegmentSize is too large, use MaxOutputSegmentSize64 instead.");
    }
    set
    {
      this._maxOutputSegmentSize = value >= 65536 /*0x010000*/ || value == 0 ? (long) value : throw new ZipException("The minimum acceptable segment size is 65536.");
    }
  }
  ...
  public long MaxOutputSegmentSize64
  {
    ...
    set
    {
      this._maxOutputSegmentSize = value >= 65536L /*0x010000*/ || value == 0L ? value : throw new ZipException("The minimum acceptable segment size is 65536.");
    }
  }
  ...
  internal void Reset(bool whileSaving)
  {
    ...
    using (ZipFile zf = new ZipFile())
    {
      ...
      {
        if (this._readstream.CanSeek)
          this._readstream.Seek(0L, SeekOrigin.Begin);
        zf._readstream = this._readstream;
      }
      ...
      {
        ZipEntry zipEntry = this[source.FileName];
  ...
  {
    if (ZipFile.DefaultEncoding == null)
    {
  ...
  private void _InitInstance(string zipFileName, TextWriter statusMessageWriter)
  {
    ...
    this._entries = new Dictionary<string, ZipEntry>((IEqualityComparer<string>) StringComparer.Ordinal);
    this._entriesInsensitive = new Dictionary<string, ZipEntry>((IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase);
  ...
  public ZipEntry this[string fileName]
  {
    get
    {
      Dictionary<string, ZipEntry> retrievalEntries = this.RetrievalEntries;
      string key1 = SharedUtilities.NormalizePathForUseInZipFile(fileName);
      if (retrievalEntries.ContainsKey(key1))
        return retrievalEntries[key1];
      string key2 = key1.Replace("/", "\\");
      return retrievalEntries.ContainsKey(key2) ? retrievalEntries[key2] : (ZipEntry) null;
    }
  }
  ...
  {
    string str = entry != null ? SharedUtilities.NormalizePathForUseInZipFile(entry.FileName) : throw new ArgumentNullException(nameof (entry));
    this._entries.Remove(str);
    if (!this.AnyCaseInsensitiveMatches(str))
      this._entriesInsensitive.Remove(str);
    this._zipEntriesAsList = (List<ZipEntry>) null;
    this._contentsChanged = true;
  }
  ...
  private bool AnyCaseInsensitiveMatches(string path)
  {
    foreach (ZipEntry zipEntry in this._entries.Values)
    {
      if (string.Equals(zipEntry.FileName, path, StringComparison.OrdinalIgnoreCase))
        return true;
    }
    return false;
  }
  ...
  private Stream WriteStream
  {
    get
    {
      ...
      if (this._maxOutputSegmentSize != 0L)
      {
  ...
  {
    bool flag = false;
    Dictionary<string, object> previouslySeen = new Dictionary<string, object>((IEqualityComparer<string>) StringComparer.Ordinal);
    ZipEntry zipEntry;
  ...
  private static void ReadIntoInstance_Orig(ZipFile zf)
  {
    ...
    for (ZipContainer zc = new ZipContainer((object) zf); (zipEntry1 = ZipEntry.ReadEntry(zc, first)) != null; first = false)
    {
      ...
      if (!zf._entriesInsensitive.ContainsKey(zipEntry1.FileName))
        zf._entriesInsensitive.Add(zipEntry1.FileName, zipEntry1);
    }
    try
    {
      Dictionary<string, object> previouslySeen = new Dictionary<string, object>((IEqualityComparer<string>) StringComparer.Ordinal);
  ...
  public void Save(string fileName)
  {
    ...
    this._contentsChanged = true;
    this._fileAlreadyExists = File.Exists(this._readName);
    this.Save();
  }
    ...
    string replacement)
  {
    ...
    string str = upper2;
    return upper1.IndexOf(str) != 0 ? original : replacement + original.Substring(upper2.Length);
  }
  ...
  static ZipFile()
  {
    ...
    ZipFile._defaultEncoding = encoding;
  }

  public static Encoding DefaultEncoding
  {
    get => ZipFile._defaultEncoding;
    set
    {
      if (ZipFile._defaultEncodingInitialized)
        return;
      ZipFile._defaultEncoding = value;
      ZipFile._defaultEncodingInitialized = true;
    }
  }
