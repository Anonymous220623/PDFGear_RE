...
// Type: NLog.LayoutRenderers.NdlcLayoutRenderer
...
public class NdlcLayoutRenderer : LayoutRenderer
{
  ...
  protected override void Append(StringBuilder builder, LogEventInfo logEvent)
  {
    if (this.TopFrames == 1)
    {
      object message = NestedDiagnosticsLogicalContext.PeekObject();
      if (message == null)
        return;
      NdlcLayoutRenderer.AppendAsString(message, this.GetFormatProvider(logEvent), builder);
    }
    else
    {
      ...
      {
        builder.Append(str);
        NdlcLayoutRenderer.AppendAsString(allObjects[index], formatProvider, builder);
        str = this.Separator;
      }
    }
  }
