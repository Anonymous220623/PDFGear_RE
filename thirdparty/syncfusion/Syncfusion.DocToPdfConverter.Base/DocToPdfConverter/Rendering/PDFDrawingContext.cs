...
// Type: Syncfusion.DocToPdfConverter.Rendering.PDFDrawingContext
...
internal class PDFDrawingContext : DocumentLayouter
{
  ...
  private System.Drawing.Graphics m_graphicsBmp;
  ...
  private Dictionary<string, PdfFont> PdfFontCollection
  {
    get
    {
      if (PDFDrawingContext.pdfFontCollection == null)
        PDFDrawingContext.pdfFontCollection = new Dictionary<string, PdfFont>();
      return PDFDrawingContext.pdfFontCollection;
    }
  }
  ...
  private Dictionary<string, PdfFont> SubstitutedFonts
  {
    get
    {
      if (PDFDrawingContext.m_substitutedFonts == null)
        PDFDrawingContext.m_substitutedFonts = new Dictionary<string, PdfFont>();
      return PDFDrawingContext.m_substitutedFonts;
    }
  }
  ...
  internal static void ClearFontCache()
  {
    if (PDFDrawingContext.pdfFontCollection != null)
    {
      PDFDrawingContext.pdfFontCollection.Clear();
      PDFDrawingContext.pdfFontCollection = (Dictionary<string, PdfFont>) null;
    }
    if (PDFDrawingContext.m_substitutedFonts != null)
    {
      PDFDrawingContext.m_substitutedFonts.Clear();
      PDFDrawingContext.m_substitutedFonts = (Dictionary<string, PdfFont>) null;
    }
    if (PDFDrawingContext.m_bookmarks == null)
      return;
    for (int index = 0; index < PDFDrawingContext.m_bookmarks.Count; ++index)
    {
      if (PDFDrawingContext.m_bookmarks[index] != null)
        ;
    }
    PDFDrawingContext.m_bookmarks.Clear();
    PDFDrawingContext.m_bookmarks = (List<BookmarkPosition>) null;
  }

  internal PdfDocument PdfDocument
  {
    get => this.m_pdfdocument;
    set => this.m_pdfdocument = value;
  }
  ...
  {
    get => this.m_pdfgraphics;
    set
    {
      if (this.m_pdfgraphics == value)
        return;
      this.m_pdfgraphics = value;
    }
  }
  ...
  internal System.Drawing.Graphics GraphicsBmp
  {
    get
    {
  ...
  internal static List<BookmarkPosition> Bookmarks
  {
    ...
    {
      if (PDFDrawingContext.m_bookmarks == null)
        PDFDrawingContext.m_bookmarks = new List<BookmarkPosition>();
      return PDFDrawingContext.m_bookmarks;
    }
  }
    ...
    PdfDocument pdfDocument)
  {
    ...
    this.m_pdfgraphics = pdfgraphics;
    this.m_pdfgraphics.PageUnit = pageUnit;
  ...
  internal PdfFont CreatePdfFont(
    Font tempfont,
    bool embedFont,
    bool EmbedCompleteFonts,
    string text)
  {
    PdfFont pdfFont;
    if (this.PdfFontCollection.ContainsKey(this.GetPdfFontCollectionKey(tempfont, embedFont)))
    {
      pdfFont = this.PdfFontCollection[this.GetPdfFontCollectionKey(tempfont, embedFont)];
    }
    else
    {
      pdfFont = (PdfFont) new PdfTrueTypeFont(tempfont, embedFont, false, EmbedCompleteFonts);
      this.PdfFontCollection.Add(this.GetPdfFontCollectionKey(tempfont, embedFont), pdfFont);
    }
    return pdfFont;
  }
  ...
  internal PdfStructureElement CreateAutoTag(
    PdfTagType tagType,
    string alternateText,
    string title,
    bool isOverlapedShape)
  {
    PdfStructureElement autoTag = (PdfStructureElement) null;
    if (!isOverlapedShape)
      ++this.autoTagCount;
    switch (tagType)
    {
      case PdfTagType.Figure:
        autoTag = new PdfStructureElement(PdfTagType.Figure);
        autoTag.AlternateText = alternateText;
        autoTag.Title = title;
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.Form:
        autoTag = new PdfStructureElement(PdfTagType.Form);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel1:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel1);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel2:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel2);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel3:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel3);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel4:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel4);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel5:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel5);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.HeadingLevel6:
        autoTag = new PdfStructureElement(PdfTagType.HeadingLevel6);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.Paragraph:
        autoTag = new PdfStructureElement(PdfTagType.Paragraph);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.Table:
        autoTag = new PdfStructureElement(PdfTagType.Table);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.TableDataCell:
        autoTag = new PdfStructureElement(PdfTagType.TableDataCell);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.TableHeader:
        autoTag = new PdfStructureElement(PdfTagType.TableHeader);
        autoTag.Order = this.autoTagCount;
        break;
      case PdfTagType.TableRow:
        autoTag = new PdfStructureElement(PdfTagType.TableRow);
        autoTag.Order = this.autoTagCount;
        break;
    }
    return autoTag;
  ...
  internal PdfStructureElement CreatePdfStructureElement(
    WParagraph paragraph,
    LayoutedWidget layoutedWidget)
  {
    if (paragraph.ParaStyle is WParagraphStyle paraStyle && paraStyle.ParagraphFormat.IsBuiltInHeadingStyle(paraStyle.Name) && !paragraph.IsInCell)
    {
      int headingLevel = paragraph.GetHeadingLevel(paraStyle, paragraph);
      return headingLevel >= 0 && headingLevel < 6 ? this.CreateAutoTag(this.GetHeadingLevelPdfTagType(headingLevel), "", "", this.IsOverLappedShapeWidget(layoutedWidget)) : this.CreateAutoTag(PdfTagType.Paragraph, "", "", this.IsOverLappedShapeWidget(layoutedWidget));
    }
    if (!paragraph.IsInCell)
      return this.CreateAutoTag(PdfTagType.Paragraph, "", "", this.IsOverLappedShapeWidget(layoutedWidget));
    PdfStructureElement autoTag = this.CreateAutoTag(PdfTagType.Paragraph, "", "", true);
    autoTag.Parent = this.pdfStructureTable.Peek();
    return autoTag;
  }
  ...
  internal PdfTagType GetHeadingLevelPdfTagType(int outlineLevel)
  {
    ...
    {
      case 0:
        return PdfTagType.HeadingLevel1;
      case 1:
        return PdfTagType.HeadingLevel2;
      case 2:
        return PdfTagType.HeadingLevel3;
      case 3:
        return PdfTagType.HeadingLevel4;
      case 4:
        return PdfTagType.HeadingLevel5;
      case 5:
        return PdfTagType.HeadingLevel6;
      default:
        return PdfTagType.Heading;
    }
  }

  internal void CreatePdfStructureElement(
    LayoutedWidget ltWidget,
    Entity ownerWidget,
    WTableCell cellWidget)
  {
    if (ltWidget.Widget is WTable)
    {
      if (this.pdfStructureTable == null)
        this.pdfStructureTable = new Stack<PdfStructureElement>();
      PdfStructureElement autoTag;
      if (this.pdfStructureTable.Count > 0)
      {
        autoTag = this.CreateAutoTag(PdfTagType.Table, "", "", true);
        autoTag.Parent = this.pdfStructureTable.Peek();
      }
      else
        autoTag = this.CreateAutoTag(PdfTagType.Table, "", "", false);
      this.pdfStructureTable.Push(autoTag);
    }
    else if (ltWidget.Widget is WTableRow)
    {
      PdfStructureElement autoTag = this.CreateAutoTag(PdfTagType.TableRow, "", "", true);
      autoTag.Parent = this.pdfStructureTable.Peek();
      this.pdfStructureTable.Push(autoTag);
    }
    else
    {
      if (!(ownerWidget is WTableRow))
        return;
      PdfStructureElement structureElement = ownerWidget.Index == 0 || cellWidget.Index == 0 ? this.CreateAutoTag(PdfTagType.TableHeader, "", "", true) : this.CreateAutoTag(PdfTagType.TableDataCell, "", "", true);
      structureElement.Parent = this.pdfStructureTable.Peek();
      this.pdfStructureTable.Push(structureElement);
    }
  }
  ...
  internal void DrawParagraph(WParagraph paragraph, LayoutedWidget ltWidget)
  {
    ...
    if ((paragraphFormat.TextureStyle != TextureStyle.TextureNone || !isEmpty) && !this.IsLinesInteresectWithFloatingItems(ltWidget, true))
    {
      ...
      if (paragraphFormat.TextureStyle != TextureStyle.TextureNone)
        ...
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) paragraphFormat.BackColor), paragraphBackGroundColor);
      if (resetTransform)
        this.ResetTransform();
    }
  ...
  internal void DrawTextBox(WTextBox textBox, LayoutedWidget ltWidget)
  {
    ...
    if (((double) num != 0.0 || (double) num == 0.0 && (flipHorizontal || flipVertical)) && textBoxFormat.TextWrappingStyle != TextWrappingStyle.Tight && textBoxFormat.TextWrappingStyle != TextWrappingStyle.Through)
    {
      ...
      if ((double) num != 0.0 || flipVertical || flipHorizontal)
        this.PDFGraphics.Transform = this.GetTransformMatrix(ltWidget.Bounds, num, flipHorizontal, flipVertical);
    }
    ...
    if (textBoxFormat.VMLPathPoints == null)
    {
      if (textBoxFormat.FillEfects.Type == BackgroundType.Gradient)
      {
        ...
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) color2), ltWidget.Bounds);
      if (this.IsTexBoxHaveBackgroundPicture(textBox))
        this.PDFGraphics.DrawImage(this.GetPdfImage(textBoxFormat.FillEfects.Picture), ltWidget.Bounds);
    }
    else
    {
      path = new ShapePath(ltWidget.Bounds, (Dictionary<string, string>) null).GetVMLCustomShapePath(textBoxFormat.VMLPathPoints);
      if (path.PointCount > 0 && !color2.IsEmpty)
        this.PDFGraphics.DrawPath((PdfBrush) new PdfSolidBrush((PdfColor) color2), path);
    }
    ...
    if (!textBoxFormat.NoLine)
    {
      ...
      borders.Color = textBoxFormat.LineColor;
      PdfPen pen = this.GetPen(borders.Left, false);
      if (textBox.Shape != null)
        pen.LineJoin = this.GetLineJoin(this.GetPdfLineJoin(textBox.Shape.LineFormat.LineJoin));
      if (path != null && path.PointCount > 0)
        this.PDFGraphics.DrawPath(pen, path);
      else if (this.AutoTag)
      {
        string alternativeText = textBox.Shape != null ? textBox.Shape.AlternativeText : (string) null;
        string title = textBox.Shape != null ? textBox.Shape.Title : (string) null;
        PdfRectangle pdfRectangle = new PdfRectangle(ltWidget.Bounds);
        pdfRectangle.Pen = pen;
        pdfRectangle.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Figure, alternativeText, title, this.IsOverLappedShapeWidget(ltWidget));
        pdfRectangle.Draw(this.PDFGraphics);
      }
      else
        this.PDFGraphics.DrawRectangle(pen, ltWidget.Bounds.X, ltWidget.Bounds.Y, ltWidget.Bounds.Width, ltWidget.Bounds.Height);
    }
    this.PDFGraphics.ResetTransform();
  ...
  private void DrawBarTabStop(WParagraph paragraph, LayoutedWidget ltWidget)
  {
    ...
    for (int index = 0; index < tabs.Count; ++index)
    {
      if (tabs[index].Justification == Syncfusion.DocIO.DLS.TabJustification.Bar)
        this.PDFGraphics.DrawLine(new PdfPen(new PdfColor(Color.Black)), new PointF(tabs[index].Position + ltWidget.Bounds.X, ltWidget.Bounds.Y), new PointF(tabs[index].Position + ltWidget.Bounds.X, ltWidget.Bounds.Bottom + paragraph.ParagraphFormat.AfterSpacing));
    }
  }
...
label_31:
    ...
    {
      PdfImage pdfImage = this.GetPdfImage(listLevel.PicBullet.GetImage(listLevel.PicBullet.ImageBytes, false));
      if (this.AutoTag)
        pdfImage.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Paragraph, (string) null, (string) null, false);
      this.PDFGraphics.DrawImage(pdfImage, new RectangleF(x, num3, sizeF.Width, sizeF.Height));
  ...
  {
    this.PDFGraphics.DrawLine(new PdfPen(new PdfColor(Color.Black), 0.5f), new PointF(ltWidget.Bounds.X, ltWidget.Bounds.Y + ltWidget.Bounds.Height / 2f), new PointF(ltWidget.Bounds.Right, ltWidget.Bounds.Y + ltWidget.Bounds.Height / 2f));
  }

  internal void DrawEditableTextRange(WTextRange txtRange, LayoutedWidget ltWidget)
  {
    WTextFormField parentField = (this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField;
    PdfTextBoxField field = new PdfTextBoxField(this.PDFGraphics.Page, parentField.Name);
    field.Text = ((this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField).Text;
    field.ForeColor = (PdfColor) (this.m_editableTextFormTextRange.CharacterFormat.TextColor.IsEmpty ? parentField.CharacterFormat.TextColor : this.m_editableTextFormTextRange.CharacterFormat.TextColor);
    field.Font = (PdfFont) new PdfTrueTypeFont(this.m_editableTextFormTextRange.CharacterFormat.GetFontToRender(this.m_editableTextFormTextRange.ScriptType), this.m_editableTextFormTextRange.CharacterFormat.GetFontSizeToRender());
    field.Bounds = new RectangleF(this.m_editableTextFormBounds.X, this.m_editableTextFormBounds.Y, (float) Math.Round((double) this.m_editableTextFormBounds.Width), (float) Math.Round((double) this.m_editableTextFormBounds.Height));
    field.BackColor = (PdfColor) (this.m_editableTextFormTextRange.CharacterFormat.HighlightColor.IsEmpty ? parentField.CharacterFormat.HighlightColor : this.m_editableTextFormTextRange.CharacterFormat.HighlightColor);
    field.BorderColor = (PdfColor) Color.Empty;
    field.DefaultValue = ((this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField).DefaultText;
    field.MaxLength = ((this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField).MaximumLength;
    field.Visible = ((this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField).Enabled;
    if (this.AutoTag)
      field.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Form, "", "", this.IsOverLappedShapeWidget(ltWidget));
    (this.PDFGraphics.Page as PdfPage).Document.Form.Fields.Add((PdfField) field);
    (this.PDFGraphics.Page as PdfPage).Document.Form.SetDefaultAppearance(false);
  }
  ...
  internal void DrawTextRange(WTextRange txtRange, LayoutedWidget ltWidget, string text)
  {
    ...
    {
      if (txtRange.CharacterRange == CharacterRangeType.WordSplit && txtRange.CharacterFormat.Bidi && !this.EmbedFonts && !this.EmbedCompleteFonts)
      {
  ...
  internal void UpdateBookmarkTargetPosition(Entity ent, LayoutedWidget ltWidget)
  {
    ...
    if (PDFDrawingContext.BookmarkHyperlinksList.Count != 0)
    {
      for (int index = 0; index < PDFDrawingContext.BookmarkHyperlinksList.Count; ++index)
      {
        foreach (KeyValuePair<string, DocumentLayouter.BookmarkHyperlink> keyValuePair in PDFDrawingContext.BookmarkHyperlinksList[index])
        {
    ...
    PDFDrawingContext.BookmarkHyperlinksList.Add(dictionary);
  }
  ...
  {
    string text = char.ConvertFromUtf32((int) symbol.CharacterCode);
    ...
    else
    {
      ...
      if (clip)
      {
        this.PDFGraphics.SetClip(clipBounds, CombineMode.Replace);
        drawLines = true;
      }
      StringFormat format1 = new StringFormat(this.StringFormt);
      if (characterFormat.Bidi)
      ...
      {
        this.DrawStringBasedOnCharSpacing(FontScriptType.English, ltWidget != null ? ltWidget.CharacterRange : CharacterRangeType.LTR, font2, (Brush) this.GetBrush(this.GetTextColor(symbol.CharacterFormat)), bounds, text, format1, characterFormat);
      }
      else
      {
        string str = this.GetEmbedFontStyle(characterFormat);
        if (!string.IsNullOrEmpty(str))
          str = $"{symbol.FontName}_{str}";
        PdfFont font3 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(symbol.FontName) ? this.CreatePdfFont(font2, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, symbol.FontName, font2.Size, this.GetFontStyle(font2.Style));
        PdfStringFormat format2 = this.PDFGraphics.ConvertFormat(format1, symbol.CharacterFormat.ComplexScript || this.EnableComplexScript);
        PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(symbol.CharacterFormat));
        this.PDFGraphics.DrawString(text, font3, brush, textBounds, format2, true);
      }
      if (characterFormat.UnderlineStyle != UnderlineStyle.None || characterFormat.Strikeout || characterFormat.DoubleStrike)
      {
        ...
        this.AddLineToCollection(text, characterFormat.SubSuperScript != SubSuperScript.None, font1, characterFormat, drawLines, textBounds, isSameLine);
      }
      this.ResetTransform();
      if (!clip)
        return;
      this.PDFGraphics.ResetClip();
    }
  }
  ...
  internal void DrawPicture(WPicture picture, LayoutedWidget ltwidget)
  {
    if (picture != null)
    {
      ...
      if (this.RecreateNestedMetafile && !DocumentLayouter.IsAzureCompatible && image1 is Metafile)
      {
        Metafile metafile1 = image1 as Metafile;
        RectangleF frameRect = new RectangleF(0.0f, 0.0f, ltwidget.Bounds.Width, ltwidget.Bounds.Height);
        ...
        graphics2.DrawImage((Image) metafile1, 0.0f, 0.0f, ltwidget.Bounds.Width, ltwidget.Bounds.Height);
        graphics2.Dispose();
        metafile1.Dispose();
        memoryStream.Dispose();
        image1 = (Image) metafile2;
      }
      if (image1 == null)
        return;
      ...
      if (ltwidget.Widget.LayoutInfo.IsVerticalText && ltwidget.Widget is WPicture)
      {
        ...
        if (ownerParagraphValue.IsInCell)
        {
          ...
          if (cellLayoutedWidget != null)
          {
            ...
            {
              this.PDFGraphics.TranslateTransform(bounds2.X + bounds2.Y + bounds2.Width, bounds2.Y - bounds2.X);
              this.PDFGraphics.RotateTransform(90f);
            }
            else
            {
              this.PDFGraphics.TranslateTransform(bounds2.X - bounds2.Y, bounds2.X + bounds2.Y + bounds2.Height);
              this.PDFGraphics.RotateTransform(270f);
            }
          }
        }
        else if (ownerEntity1 is WTextBox)
        {
          ...
          float offsetY1 = (float) ((double) top - (double) bounds1.Y + ((double) bounds1.X - (double) left1)) + left2;
          if (wtextBox.TextBoxFormat.TextDirection == TextDirection.VerticalTopToBottom)
          {
            ...
            int verticalAlignment = (int) wtextBox.TextBoxFormat.TextVerticalAlignment;
            double cellLayoutedHeight = (double) boxContentHeight;
            rectangleF = wtextBox.TextLayoutingBounds;
            double height1 = (double) rectangleF.Height;
            float shiftVerticalText = this.GetWidthToShiftVerticalText((VerticalAlignment) verticalAlignment, (float) cellLayoutedHeight, (float) height1);
            PdfGraphics pdfGraphics = this.PDFGraphics;
            rectangleF = wtextBox.TextLayoutingBounds;
            double x1 = (double) rectangleF.X;
            rectangleF = wtextBox.TextLayoutingBounds;
            double y1 = (double) rectangleF.Y;
            double num = x1 + y1;
            rectangleF = wtextBox.TextLayoutingBounds;
            double height2 = (double) rectangleF.Height;
            double offsetX = num + height2 - (double) shiftVerticalText;
            rectangleF = wtextBox.TextLayoutingBounds;
            double y2 = (double) rectangleF.Y;
            rectangleF = wtextBox.TextLayoutingBounds;
            double x2 = (double) rectangleF.X;
            double offsetY2 = y2 - x2;
            pdfGraphics.TranslateTransform((float) offsetX, (float) offsetY2);
            this.PDFGraphics.RotateTransform(90f);
          }
          else
            this.PDFGraphics.TranslateTransform(bounds1.Y - top, offsetY1);
        }
        else if (ownerEntity1 is Shape)
        {
          ...
          float offsetY = (float) ((double) top - (double) bounds1.Y + ((double) bounds1.X - (double) left3)) + left4;
          if (shape.TextFrame.TextDirection == TextDirection.VerticalTopToBottom || shape.TextFrame.TextDirection == TextDirection.VerticalFarEast)
            this.PDFGraphics.TranslateTransform(this.GetCellHeightForVerticalText((Entity) picture) + left3 - bounds1.Y - bounds1.Width - left4, offsetY);
          else
            this.PDFGraphics.TranslateTransform(bounds1.Y - top, offsetY);
        }
        else if (ownerParagraphValue.Owner.Owner is ChildShape)
        {
          ...
          float offsetY = (float) ((double) top - (double) bounds1.Y + ((double) bounds1.X - (double) left5)) + left6;
          if (owner.TextFrame.TextDirection == TextDirection.VerticalTopToBottom || owner.TextFrame.TextDirection == TextDirection.VerticalFarEast)
            this.PDFGraphics.TranslateTransform(this.GetCellHeightForVerticalText((Entity) picture) + left5 - bounds1.Y - bounds1.Width - left6, offsetY);
          else
            this.PDFGraphics.TranslateTransform(bounds1.Y - top, offsetY);
        }
      }
      ...
      if ((ownerParagraphValue1 == null || (!picture.LayoutInCell || picture.Document.Settings.CompatibilityMode != CompatibilityMode.Word2013) && picture.TextWrappingStyle != TextWrappingStyle.Inline || !ownerParagraphValue1.IsInCell ? 1 : (!this.IsTableInTextBoxOrShape(ownerParagraphValue1.GetOwnerEntity(), true) ? 1 : 0)) != 0)
      {
        ...
        if (flag)
        {
          rectangleF = this.PDFGraphics.ClipBounds;
      ...
      if ((double) picture.FillRectangle.LeftOffset != 0.0 || (double) picture.FillRectangle.RightOffset != 0.0 || (double) picture.FillRectangle.TopOffset != 0.0 || (double) picture.FillRectangle.BottomOffset != 0.0)
      {
        ...
        {
          this.PDFGraphics.SetClip(bounds1, CombineMode.Replace);
      ...
      if (((double) rotation != 0.0 || (double) rotation == 0.0 && (picture.FlipHorizontal || picture.FlipVertical)) && !ltwidget.Widget.LayoutInfo.IsVerticalText && picture.TextWrappingStyle != TextWrappingStyle.Tight && picture.TextWrappingStyle != TextWrappingStyle.Through)
      {
        ...
        this.PDFGraphics.Transform = this.GetTransformMatrix(bounds1, rotation, picture.FlipHorizontal, picture.FlipVertical);
      }
      ...
      switch (ownerEntity4)
      {
        ...
        case WTextBox _:
          ...
          if ((double) wtextBox1.TextBoxFormat.Rotation != 0.0 && (!wtextBox1.IsShape || !wtextBox1.Shape.TextFrame.Upright))
            this.PDFGraphics.Transform = this.GetTransformMatrix(this.m_rotateTransform, wtextBox1.TextBoxFormat.Rotation, wtextBox1.TextBoxFormat.FlipHorizontal, wtextBox1.TextBoxFormat.FlipVertical);
        ...
        case Shape _:
          ...
          {
            this.PDFGraphics.Transform = this.GetTransformMatrix(this.m_rotateTransform, shape1.Rotation, shape1.FlipHorizontal, shape1.FlipVertical);
            break;
          }
          break;
      }
      ...
      if (image1 is Bitmap && picture.FillFormat.BlipFormat.BlipTransparency == BlipTransparency.GrayScale)
      {
        ...
        attributes = this.CreateImageAttributes();
      ...
      else
      {
        float lineWidth = this.GetLineWidth(picture);
        if (attributes != null)
          image1 = (Image) this.UpdateColoring(image1, attributes);
        PdfImage pdfImage = this.GetPdfImage(image1);
        if (this.AutoTag)
          pdfImage.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Figure, picture.AlternativeText, picture.Title, this.IsOverLappedShapeWidget(ltwidget));
        this.PDFGraphics.DrawImage(pdfImage, new RectangleF(bounds1.X + lineWidth, bounds1.Y + lineWidth, size.Width, size.Height));
        if ((double) lineWidth > 0.0)
        {
          PdfPen pen = this.CreatePen(picture.PictureShape);
          pen.Width = lineWidth;
          this.PDFGraphics.DrawRectangle(pen, bounds1.X + lineWidth / 2f, bounds1.Y + lineWidth / 2f, bounds1.Width - lineWidth, bounds1.Height - lineWidth);
        }
      }
      image1?.Dispose();
      if (this.currHyperlink != null)
  ...
  {
    PdfImage image = this.PDFGraphics.GetImage((Stream) stream, this.PdfDocument);
    switch (image)
    {
      case PdfBitmap _:
        (image as PdfBitmap).Quality = (long) this.ImageQuality;
        break;
      case PdfMetafile _:
        (image as PdfMetafile).IsEmbedCompleteFonts = this.EmbedCompleteFonts;
        break;
    }
    return image;
  }
    ...
    Image image)
  {
    ...
    if (this.AutoTag)
      pdfImage.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Figure, picture.AlternativeText, picture.Title, false);
    this.PDFGraphics.DrawImage(pdfImage, new RectangleF(bounds.X + lineWidth1, bounds.Y + lineWidth2, size.Width, size.Height));
    ...
    if ((double) lineWidth1 > 0.0)
    {
      ...
      this.PDFGraphics.DrawLine(pen, new PointF(bounds.Left + lineWidth1 / 2f, bounds.Top), new PointF(bounds.Left + lineWidth1 / 2f, bounds.Bottom));
    }
    if ((double) lineWidth2 > 0.0)
    {
      pen.Color = (PdfColor) color4;
      ...
      this.PDFGraphics.DrawLine(pen, new PointF(bounds.Left, bounds.Top + lineWidth2 / 2f), new PointF(bounds.Right, bounds.Top + lineWidth2 / 2f));
    }
    if ((double) lineWidth3 > 0.0)
    {
      pen.Color = (PdfColor) color3;
      ...
      this.PDFGraphics.DrawLine(pen, new PointF(bounds.Right - lineWidth3 / 2f, bounds.Top), new PointF(bounds.Right - lineWidth3 / 2f, bounds.Bottom));
    }
    if ((double) lineWidth4 <= 0.0)
      return;
    pen.Color = (PdfColor) color1;
    pen.Width = lineWidth4;
    this.PDFGraphics.DrawLine(this.GetPictureBorderPen(picture.PictureShape, picture.PictureShape.PictureDescriptor.BorderBottom, pen), new PointF(bounds.Left, bounds.Bottom - lineWidth4 / 2f), new PointF(bounds.Right, bounds.Bottom - lineWidth4 / 2f));
  }

  private PdfPen CreatePen(InlineShapeObject inlineShapeObject)
  {
    PdfPen pen = new PdfPen(new PdfColor(this.GetColorBorder(inlineShapeObject)));
    PdfPen dashAndLineStyle = this.GetDashAndLineStyle(inlineShapeObject, pen);
    dashAndLineStyle.LineJoin = this.GetLineJoin((PdfLineJoin) inlineShapeObject.ShapeContainer.GetPropertyValue(470));
    dashAndLineStyle.LineCap = this.GetLineCap((PdfLineCap) inlineShapeObject.ShapeContainer.GetPropertyValue(471));
  ...
  private PdfLineJoin GetLineJoin(PdfLineJoin lineJoin)
  {
    switch (lineJoin)
    {
      case PdfLineJoin.Miter:
        return PdfLineJoin.Miter;
      case PdfLineJoin.Bevel:
        return PdfLineJoin.Bevel;
      default:
        return PdfLineJoin.Round;
    }
  }

  private PdfPen GetPictureBorderPen(
    InlineShapeObject inlineShape,
    ...
    WCharacterFormat charFormat)
  {
    ...
    for (int index = 0; index < ltEQField.ChildEQFileds.Count; ++index)
    {
      if (ltEQField.ChildEQFileds[index] is TextEQField)
      {
        ...
        Font tempfont = childEqFiled.Font != null ? childEqFiled.Font : charFormat.GetFontToRender(scriptType);
        float num = 0.0f;
        string fontNameToRender = charFormat.GetFontNameToRender(scriptType);
        if (!this.IsListCharacter && (fontNameToRender == "DIN Offc" || fontNameToRender == "DIN OT") && this.currParagraph != null && this.currParagraph.IsContainDinOffcFont())
          num = childEqFiled.Bounds.Height * 0.2f;
        RectangleF rect = new RectangleF(childEqFiled.Bounds.X, childEqFiled.Bounds.Y + num, childEqFiled.Bounds.Width, childEqFiled.Bounds.Height);
        string str = this.GetEmbedFontStyle(charFormat);
        if (!string.IsNullOrEmpty(str))
          str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
        PdfFont font = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, childEqFiled.Text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
        PdfStringFormat format2 = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
        PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) textColor);
        this.PDFGraphics.DrawString(childEqFiled.Text, font, brush, rect, format2, true);
      }
      else if (ltEQField.ChildEQFileds[index] is LineEQField)
      {
        ...
        this.PDFGraphics.DrawLine(new PdfPen(new PdfColor(Color.Black), 0.5f), childEqFiled.Point1, childEqFiled.Point2);
      }
    ...
    StringAlignment arraySwitchAlignment)
  {
    ...
    this.GetBrush(textColor);
    StringFormat format1 = new StringFormat(this.StringFormt);
    format1.Alignment = arraySwitchAlignment;
    format1.FormatFlags &= ~StringFormatFlags.DirectionRightToLeft;
    for (int index = 0; index < ltEQField.ChildEQFileds.Count; ++index)
    {
      if (ltEQField.ChildEQFileds[index] is TextEQField)
  ...
  internal void DrawEditableDropDown(WDropDownFormField dropDownFormField, LayoutedWidget ltWidget)
  {
    ...
    float width = 15f + ltWidget.Bounds.Width;
    ltWidget.Bounds = new RectangleF(ltWidget.Bounds.X, ltWidget.Bounds.Y, width, ltWidget.Bounds.Height);
    PdfComboBoxField field = new PdfComboBoxField(this.PDFGraphics.Page, dropDownFormField.Name);
    field.Editable = true;
    field.Bounds = ltWidget.Bounds;
    field.ToolTip = dropDownFormField.StatusBarHelp;
    field.BorderColor = (PdfColor) Color.Empty;
    field.Visible = dropDownFormField.Enabled;
    field.Font = (PdfFont) new PdfTrueTypeFont(dropDownFormField.CharacterFormat.GetFontToRender(dropDownFormField.ScriptType));
    int index1 = 0;
    for (int index2 = dropDownFormField.DropDownItems.Count - 1; index1 <= index2; ++index1)
      field.Items.Add(new PdfListFieldItem(dropDownFormField.DropDownItems[index1].Text, dropDownFormField.DropDownItems[index1].Text));
    field.SelectedIndex = dropDownFormField.DropDownSelectedIndex;
    field.SelectedValue = dropDownFormField.DropDownValue;
    if (this.AutoTag)
      field.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Form, "", "", this.IsOverLappedShapeWidget(ltWidget));
    (this.PDFGraphics.Page as PdfPage).Document.Form.Fields.Add((PdfField) field);
  }
    ...
    LayoutedWidget ltWidget)
  {
    ...
    {
      format1 = new StringFormat(this.StringFormt);
    ...
    Color textColor = this.GetTextColor(charFormat);
    Brush brush1 = (Brush) this.GetBrush(textColor);
    ...
    else
    {
      ...
      if (!charFormat.TextBackgroundColor.IsEmpty && !flag3)
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) charFormat.TextBackgroundColor), bounds.X, y, bounds.Width, height);
      if (!charFormat.HighlightColor.IsEmpty && !flag3)
        this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(this.GetHightLightColor(charFormat.HighlightColor)), bounds.X, y, bounds.Width, height);
      ...
      {
        Font tempfont = !isSubSuperScriptNone ? charFormat.Document.FontSettings.GetFont(charFormat.GetFontNameToRender(scriptType), this.GetSubSuperScriptFontSize(charFormat.GetFontToRender(scriptType)), charFormat.GetFontToRender(scriptType).Style) : charFormat.GetFontToRender(scriptType);
        if (!charFormat.GetFontNameToRender(scriptType).Equals(tempfont.Name, StringComparison.OrdinalIgnoreCase) && this.m_privateFontStream != null && this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)))
          tempfont = this.GetPrivateFont(charFormat.GetFontNameToRender(scriptType), charFormat.GetFontSizeToRender(), tempfont.Style);
        string str = this.GetEmbedFontStyle(charFormat);
        if (!string.IsNullOrEmpty(str))
          str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
        PdfFont font2 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
        PdfStringFormat pdfStringFormat = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
        PdfBrush brush2 = (PdfBrush) new PdfSolidBrush(new PdfColor(Color.Gray));
        if (this.IsUnicode(text))
          this.PDFDrawString(text, font2, brush2, bounds.X - 0.2f, bounds.Y - 0.2f + num, pdfStringFormat);
        else
          this.PDFGraphics.DrawString(text, font2, brush2, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num, bounds.Width, bounds.Height), pdfStringFormat, true);
        textBounds = new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num, bounds.Width, bounds.Height);
      }
      if (charFormat.Engrave)
      {
        Font tempfont = !isSubSuperScriptNone ? charFormat.Document.FontSettings.GetFont(charFormat.GetFontNameToRender(scriptType), charFormat.GetFontSizeToRender() / 1.5f, charFormat.GetFontToRender(scriptType).Style) : charFormat.GetFontToRender(scriptType);
        if (!charFormat.GetFontNameToRender(scriptType).Equals(tempfont.Name, StringComparison.OrdinalIgnoreCase) && this.m_privateFontStream != null && this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)))
          tempfont = this.GetPrivateFont(charFormat.GetFontNameToRender(scriptType), tempfont.Size, tempfont.Style);
        PdfStringFormat pdfStringFormat = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
        PdfBrush brush3 = (PdfBrush) new PdfSolidBrush(new PdfColor(Color.Gray));
        if (this.IsUnicode(text))
        {
          ...
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          if (this.m_privateFontStream != null && (this.m_privateFontStream.ContainsKey(str) || this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType))))
          {
            pdfFont1 = this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
          }
          else
          {
            PdfFont pdfFont2 = this.CreatePdfFont(tempfont, true, this.EmbedCompleteFonts, text);
            this.PDFDrawString(text, pdfFont2, brush3, bounds.X - 0.2f, bounds.Y - 0.2f, pdfStringFormat);
          }
        }
        else
        {
          ...
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          PdfFont font3 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
          this.PDFGraphics.DrawString(text, font3, brush3, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f, bounds.Width, bounds.Height), pdfStringFormat, true);
        }
        textBounds = new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num, bounds.Width, bounds.Height);
      }
      ...
      if (!this.IsOwnerParagraphEmpty(text))
      {
        ...
        if (characterRangeType == CharacterRangeType.RTL)
          this.DrawRTLText(scriptType, characterRangeType, text, charFormat, font1, brush1, bounds, format1);
        else if (this.IsUnicode(text))
        {
          this.DrawChineseText(scriptType, characterRangeType, text, charFormat, font1, brush1, bounds, format1);
        }
        else
        {
          ...
          {
            this.DrawSmallCapString(scriptType, characterRangeType, text, charFormat, bounds, font1, format1, brush1, (double) charFormat.CharacterSpacing != 0.0);
          }
          else
          {
            ...
            {
              this.DrawStringBasedOnCharSpacing(scriptType, characterRangeType, font4, brush1, bounds, text, format1, charFormat);
            }
            else
            {
              string str = this.GetEmbedFontStyle(charFormat);
              if (!string.IsNullOrEmpty(str))
                str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
              PdfFont font5 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font4, this.EmbedFonts || this.EmbedCompleteFonts || this.currParagraph != null && this.currParagraph.ParagraphFormat.Bidi && ltWidget.Widget.LayoutInfo is ParagraphLayoutInfo, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font4.Size, this.GetFontStyle(font4.Style));
              PdfStringFormat format2 = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
              PdfBrush brush4 = (PdfBrush) new PdfSolidBrush((PdfColor) textColor);
              this.PDFGraphics.DrawString(text, font5, brush4, new RectangleF(bounds.X, bounds.Y + num, bounds.Width, bounds.Height), format2, true);
            }
      ...
      if ((this.ExportBookmarks & ExportBookmarkType.Headings) != ExportBookmarkType.None && (this.currParagraph != null || paraFormat != null) && !this.currParagraph.IsInCell)
      {
        ...
        {
          PDFDrawingContext.Bookmarks.Add(new BookmarkPosition(this.IsListCharacter ? $"{text}\t{this.currParagraph.GetDisplayText(this.currParagraph.Items)}" : this.currParagraph.GetDisplayText(this.currParagraph.Items), DocumentLayouter.PageNumber, bounds, headingLevel));
...
label_24:
    ...
    PdfFontStyle style)
  {
    ...
    if (this.m_privateFontStream.ContainsKey(styleName))
    {
      str = styleName;
      stream = this.m_privateFontStream[styleName];
    }
    else if (this.m_privateFontStream.ContainsKey(fontName))
    {
      str = fontName;
      stream = this.m_privateFontStream[fontName];
    }
    string key = $"{str};{size.ToString((IFormatProvider) CultureInfo.InvariantCulture)};{style.ToString()}";
    if (stream == null || !stream.CanRead)
      return (PdfFont) null;
    PdfFont pdfFont;
    if (this.SubstitutedFonts.ContainsKey(key))
    {
      pdfFont = this.SubstitutedFonts[key];
    }
    else
    {
      stream.Position = 0L;
      ...
      pdfFont = (PdfFont) new PdfTrueTypeFont((Stream) new MemoryStream(buffer), size, true, style);
      this.SubstitutedFonts.Add(key, pdfFont);
    }
    return pdfFont;
  }
  ...
  internal string GetEmbedFontStyle(WCharacterFormat format)
  {
    ...
    if (format.GetBoldToRender())
      return "Bold";
    return format.GetItalicToRender() ? "Italic" : "Regular";
  }
    ...
    for (int index = 0; index < charList.Count; ++index)
    {
      if (charList[index] == 'c')
      {
        ...
        else
        {
          ...
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          PdfFont font3 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font2, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font2.Size, this.GetFontStyle(font2.Style));
          PdfStringFormat format1 = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
          PdfBrush pdfBrush = (PdfBrush) this.GetPDFBrush(this.GetTextColor(charFormat));
          this.PDFGraphics.DrawString(stringList[index], font3, pdfBrush, new RectangleF(bounds.X + num1, bounds.Y + num4, bounds.Width, bounds.Height), format1, true);
        }
        num1 += sizeF.Width;
      }
      else
      {
        ...
        Font font4 = charFormat == null || charFormat.SubSuperScript == SubSuperScript.None ? font1 : charFormat.Document.FontSettings.GetFont(font1.Name, this.GetSubSuperScriptFontSize(font1), font1.Style);
        if (isCharacterSpacing)
        {
          this.DrawStringBasedOnCharSpacing(scriptType, characterRangeType, font4, textBrush, new RectangleF(bounds.X + num1, bounds.Y + (num2 - num3), sizeF.Width, bounds.Height), stringList[index], format, charFormat);
        }
        else
        {
          string str = this.GetEmbedFontStyle(charFormat);
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          PdfFont font5 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? (PdfFont) new PdfTrueTypeFont(font4, true, false, this.EmbedCompleteFonts) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font4.Size, this.GetFontStyle(font4.Style));
          PdfStringFormat format2 = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
          PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
          this.PDFGraphics.DrawString(stringList[index], font5, brush, new RectangleF(bounds.X + num1, bounds.Y + (num2 - num3) + num4, bounds.Width, bounds.Height), format2, true);
        }
        num1 += sizeF.Width;
      }
    }
    ...
    WCharacterFormat charFormat)
  {
    ...
    foreach (char ch in text)
    {
      ...
      if (!string.IsNullOrEmpty(str))
        str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
      PdfFont font2 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font.Size, this.GetFontStyle(font.Style));
      PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
      PdfStringFormat format1 = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
      if ((double) bounds.X <= (double) bounds.X + (double) num3)
      {
        this.PDFGraphics.DrawString(ch.ToString(), font2, brush, new RectangleF(bounds.X + num3, bounds.Y + num2, width2, bounds.Height), format1, true);
        if ((double) width1 > 0.0)
          this.PDFGraphics.DrawString(" ", font2, brush, new RectangleF(bounds.X + num3 + width2, bounds.Y + num2, width1, bounds.Height), format1, true);
      }
      else
        this.PDFGraphics.DrawString(ch.ToString(), font2, brush, new RectangleF(bounds.X, bounds.Y + num2, width2, bounds.Height), format1, true);
      num3 += width2 + width1;
    }
  }
    ...
    WParagraph ownerParagraph)
  {
    ...
    if (ownerParagraph.IsInCell)
    {
      ...
      if (ownerEntity2.CellFormat.TextDirection == TextDirection.VerticalTopToBottom)
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(translatePoints.X, translatePoints.Y);
        this.PDFGraphics.RotateTransform(rotationAngle);
      }
      else
      {
    ...
    else if (ownerEntity1 is WTextBox)
    {
      ...
      if (wtextBox.TextBoxFormat.TextDirection == TextDirection.VerticalTopToBottom || wtextBox.TextBoxFormat.TextDirection == TextDirection.VerticalFarEast)
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(wtextBox.TextLayoutingBounds.X + wtextBox.TextLayoutingBounds.Y + wtextBox.TextLayoutingBounds.Height - shiftVerticalText, wtextBox.TextLayoutingBounds.Y - wtextBox.TextLayoutingBounds.X);
        this.PDFGraphics.RotateTransform(90f);
      }
      else
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(wtextBox.TextLayoutingBounds.X - wtextBox.TextLayoutingBounds.Y + shiftVerticalText, wtextBox.TextLayoutingBounds.X + wtextBox.TextLayoutingBounds.Y + wtextBox.TextLayoutingBounds.Width);
        this.PDFGraphics.RotateTransform(270f);
      }
    }
    else if (ownerEntity1 is Shape)
    {
      ...
      if (shape.TextFrame.TextDirection == TextDirection.VerticalTopToBottom || shape.TextFrame.TextDirection == TextDirection.VerticalFarEast)
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(shape.TextLayoutingBounds.X + shape.TextLayoutingBounds.Y + shape.TextLayoutingBounds.Height, shape.TextLayoutingBounds.Y - shape.TextLayoutingBounds.X);
        this.PDFGraphics.RotateTransform(90f);
      }
      else
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(shape.TextLayoutingBounds.X - shape.TextLayoutingBounds.Y, shape.TextLayoutingBounds.Width + shape.TextLayoutingBounds.X + shape.TextLayoutingBounds.Y);
        this.PDFGraphics.RotateTransform(270f);
      }
    }
    else
    {
      ...
      if (owner.TextFrame.TextDirection == TextDirection.VerticalTopToBottom || owner.TextFrame.TextDirection == TextDirection.VerticalFarEast)
      {
        ...
        if (isNeedToScale)
          return;
        this.PDFGraphics.TranslateTransform(owner.TextLayoutingBounds.X + owner.TextLayoutingBounds.Y + owner.TextLayoutingBounds.Height, owner.TextLayoutingBounds.Y - owner.TextLayoutingBounds.X);
        this.PDFGraphics.RotateTransform(90f);
      }
      else
      {
        ...
        double offsetY = num2 + y5;
        pdfGraphics.TranslateTransform((float) offsetX, (float) offsetY);
        this.PDFGraphics.RotateTransform(270f);
      }
    }
  }
    ...
    StringFormat format)
  {
    ...
    else if (!charFormat.Bidi)
    {
      ...
      else
      {
        ...
        if (!string.IsNullOrEmpty(str))
          str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
        PdfFont font4 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? (PdfFont) new PdfTrueTypeFont(font3, true, false, this.EmbedCompleteFonts) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font3.Size, this.GetFontStyle(font3.Style));
        PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
        PdfStringFormat stringFormat = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
        string[] strArray = text.Split(' ');
        if (strArray.Length > 0)
        {
          float width1 = this.MeasureString(" ", font, format, charFormat, false, true).Width;
          for (int index = strArray.Length - 1; index >= 0; --index)
          {
            string text1 = strArray[index];
            float width2 = this.MeasureString(strArray[index], font, format, charFormat, false, true).Width;
            if (index != 0)
            {
              text1 = ' '.ToString() + text1;
              width2 += width1;
            }
            this.PDFDrawString(text1, font4, brush, bounds.X, bounds.Y + num, stringFormat);
            bounds.X += width2;
          }
        }
        else
          this.PDFDrawString(text, font4, brush, bounds.X, bounds.Y + num, stringFormat);
      }
    }
    else
    {
      ...
      if (strArray.Length > 0)
      {
        float width3 = this.MeasureString(" ", font, format, charFormat, false, true).Width;
        for (int index = strArray.Length - 1; index >= 0; --index)
        {
          string text2 = strArray[index];
          float width4 = this.MeasureString(text2, font, format, charFormat, false, true).Width;
          if (index != 0)
          {
            text2 = ' '.ToString() + text2;
            width4 += width3;
          }
          this.DrawUnicodeString(scriptType, characterRangeType, text2, charFormat, font, textBrush, new RectangleF(bounds.X, bounds.Y, width4, bounds.Height), format);
          bounds.X += width4;
        }
      }
      else
      ...
      if (font.Name != font2.Name && this.IsUnicodeText(text))
      {
        ...
        for (int index = 0; index < charArray.Length; ++index)
        {
          if (!this.IsUnicodeText(charArray[index].ToString()))
          {
            ...
            if (text2 != null)
            {
              ...
              {
                Font tempfont = charFormat == null || charFormat.SubSuperScript == SubSuperScript.None ? font2 : charFormat.Document.FontSettings.GetFont(font2.Name, this.GetSubSuperScriptFontSize(font2), font2.Style);
                if (!charFormat.GetFontNameToRender(scriptType).Equals(tempfont.Name, StringComparison.OrdinalIgnoreCase) && this.m_privateFontStream != null && this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)))
                  tempfont = this.GetPrivateFont(charFormat.GetFontNameToRender(scriptType), tempfont.Size, tempfont.Style);
                string str = this.GetEmbedFontStyle(charFormat);
                if (!string.IsNullOrEmpty(str))
                  str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
                PdfFont font3 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, true, this.EmbedCompleteFonts, text2) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
                PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
                PdfStringFormat stringFormat = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
                this.PDFDrawString(text2, font3, brush, bounds.X, bounds.Y - num3 + num1, stringFormat);
              }
              bounds.X += width;
              text2 = (string) null;
            }
        ...
        if (text2 != null)
        {
          ...
          else
          {
            ...
            if (!string.IsNullOrEmpty(str))
              str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
            PdfFont font4 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, true, this.EmbedCompleteFonts, text2) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
            PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
            PdfStringFormat stringFormat = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
            this.PDFDrawString(text2, font4, brush, bounds.X, bounds.Y - num3 + num1, stringFormat);
          }
        }
        else
        {
  ...
  {
    this.PDFGraphics.DrawString(text, font, brush, new RectangleF(x, y, 0.0f, 0.0f), stringFormat, true);
  }
    ...
    StringFormat format)
  {
    ...
    for (int index = 0; index < charArray.Length; ++index)
    {
      if (charArray[index] < 'ÿ')
      {
        ...
        if (text2 != null)
        {
          ...
          {
            Font tempfont = charFormat == null || charFormat.SubSuperScript == SubSuperScript.None ? font1 : charFormat.Document.FontSettings.GetFont(font1.Name, this.GetSubSuperScriptFontSize(font1), font1.Style);
            if (tempfont.Name == "Arial Narrow" && tempfont.Style == FontStyle.Bold)
            ...
            if (!string.IsNullOrEmpty(str))
              str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
            PdfFont font2 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, false, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
            PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
            PdfStringFormat stringFormat = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
            this.PDFDrawString(text2, font2, brush, bounds.X, bounds.Y + num, stringFormat);
          }
          bounds.X += width;
          text2 = (string) null;
        }
    ...
    if (text2 != null)
    {
      ...
      {
        Font tempfont = charFormat == null || charFormat.SubSuperScript == SubSuperScript.None ? font : charFormat.Document.FontSettings.GetFont(font.Name, this.GetSubSuperScriptFontSize(font), font.Style);
        if (tempfont.Name == "Arial Narrow" && tempfont.Style == FontStyle.Bold)
          text2 = text2.Replace(ControlChar.NonBreakingSpaceChar, ControlChar.SpaceChar);
        string str = this.GetEmbedFontStyle(charFormat);
        if (!string.IsNullOrEmpty(str))
        ...
        PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
        PdfStringFormat stringFormat = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
        this.PDFDrawString(text2, font3, brush, bounds.X, bounds.Y + num, stringFormat);
      }
    }
    else
    {
    ...
    StringFormat format)
  {
    ...
    else
    {
      ...
      if (tempfont.Name == "Arial Narrow" && tempfont.Style == FontStyle.Bold)
        text = text.Replace(ControlChar.NonBreakingSpaceChar, ControlChar.SpaceChar);
      if (charFormat != null && !charFormat.GetFontNameToRender(scriptType).Equals(tempfont.Name, StringComparison.OrdinalIgnoreCase) && this.PrivateFonts != null && this.HasPrivateFont(charFormat.GetFontNameToRender(scriptType)))
        tempfont = this.GetPrivateFont(charFormat.GetFontNameToRender(scriptType), tempfont.Size, tempfont.Style);
      RectangleF rect = new RectangleF(bounds.X, bounds.Y + num, bounds.Width, bounds.Height);
      string str = this.GetEmbedFontStyle(charFormat);
      if (!string.IsNullOrEmpty(str))
        str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
      PdfFont font1 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, true, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
      PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
      PdfStringFormat format1 = this.PDFGraphics.ConvertFormat(format, charFormat.ComplexScript || this.EnableComplexScript);
      this.PDFGraphics.DrawString(text, font1, brush, rect, format1, true);
    }
  }
    ...
    LayoutedWidget ltWidget)
  {
    ...
    if (!charFormat.TextBackgroundColor.IsEmpty)
      this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) charFormat.TextBackgroundColor), bounds);
    if (ltWidget.Owner.ChildWidgets[0] == ltWidget)
    {
      strArray = text.TrimStart().Split(' ');
      SizeF sizeF1 = this.MeasureString(text, font1, font2, format1, charFormat);
      SizeF sizeF2 = this.MeasureString(text.TrimStart(), font1, font2, format1, charFormat);
      if (sizeF1 != sizeF2)
      {
    ...
    {
      Color hightLightColor = this.GetHightLightColor(charFormat.HighlightColor);
      this.CreateSolidBrush(hightLightColor);
      SizeF sizeF3 = new SizeF();
      SizeF sizeF4 = !flag2 ? this.MeasureString(text, font2, format1, charFormat, false, true) : this.MeasureString(text, font1, font2, format1, charFormat);
      this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) hightLightColor), bounds.X, bounds.Y, bounds.Width, sizeF4.Height);
    }
    this.ResetTransform();
    ...
    else
    {
      ...
      if (charFormat.Emboss)
      {
        ...
        if (!string.IsNullOrEmpty(str))
          str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
        PdfFont font3 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
        PdfBrush brush2 = (PdfBrush) new PdfSolidBrush((PdfColor) Color.Gray);
        PdfStringFormat pdfStringFormat = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
        if (this.IsUnicode(text))
          this.PDFDrawString(text, font3, brush2, bounds.X + 0.2f, bounds.Y + 0.2f + num2, pdfStringFormat);
        else
          this.PDFGraphics.DrawString(text, font3, brush2, new RectangleF(bounds.X + 0.2f, bounds.Y + 0.2f + num2, bounds.Width, bounds.Height), pdfStringFormat, true);
      ...
      if (charFormat.Engrave)
      {
        ...
        if (!string.IsNullOrEmpty(str))
          str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
        PdfFont font4 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(tempfont, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), tempfont.Size, this.GetFontStyle(tempfont.Style));
        PdfBrush brush3 = (PdfBrush) new PdfSolidBrush((PdfColor) Color.Gray);
        PdfStringFormat pdfStringFormat = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
        if (this.IsUnicode(text))
          this.PDFDrawString(text, font4, brush3, bounds.X - 0.2f, bounds.Y - 0.2f + num2, pdfStringFormat);
        else
          this.PDFGraphics.DrawString(text, font4, brush3, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num2, bounds.Width, bounds.Height), pdfStringFormat, true);
        textBounds = new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num2, bounds.Width, bounds.Height);
      }
      if (charFormat.AllCaps)
        text = text.ToUpper();
      SizeF sizeF5 = new SizeF();
      SizeF sizeF6 = !flag2 ? this.MeasureString(text, font2, format1, charFormat, false, false) : this.MeasureString(text, font1, font2, format1, charFormat);
      ...
      StringFormat format2 = new StringFormat(StringFormat.GenericTypographic);
      format1.FormatFlags &= ~StringFormatFlags.LineLimit;
      format2.FormatFlags &= ~StringFormatFlags.LineLimit;
      format2.FormatFlags |= StringFormatFlags.MeasureTrailingSpaces;
      format2.FormatFlags |= StringFormatFlags.NoClip;
      if (txtRange.CharacterRange == CharacterRangeType.WordSplit && charFormat.Bidi && (this.EmbedFonts || this.EmbedCompleteFonts))
      ...
      if (flag2)
      {
        sizeF7 = this.MeasureString(text, font1, font2, format1, charFormat);
        width2 = sizeF7.Width;
      }
      else
      {
        sizeF7 = this.MeasureString(text, font2, format1, charFormat, false, true);
        width2 = sizeF7.Width;
      }
      float num4 = 0.0f;
      for (int index = 0; index < strArray.Length; ++index)
      {
        if (flag2)
        {
          ...
          sizeF7 = this.MeasureString(strArray[index], font1, font2, format1, charFormat);
        ...
        else
        {
          ...
          sizeF7 = this.MeasureString(strArray[index], font2, format1, charFormat, false, true);
      ...
      for (int index = 0; index < strArray.Length; ++index)
      {
        ...
        if (ltWidget.CharacterRange == CharacterRangeType.RTL)
        {
          format2.FormatFlags |= StringFormatFlags.DirectionRightToLeft;
          x2 -= width6;
          this.DrawRTLText(scriptType, ltWidget.CharacterRange, strArray[index], charFormat, font2, brush1, new RectangleF(x2, bounds.Y, width6, bounds.Height), format2);
        }
        else if (this.IsUnicode(strArray[index]))
        {
          this.DrawChineseText(scriptType, ltWidget.CharacterRange, strArray[index], charFormat, font1, brush1, new RectangleF(bounds.X, bounds.Y, width6, bounds.Height), format2);
          flag4 = true;
        }
        else
        {
          ...
          if (charFormat.SmallCaps)
          {
            this.DrawSmallCapString(scriptType, ltWidget.CharacterRange, strArray[index], charFormat, new RectangleF(bounds.X, bounds.Y, width6, bounds.Height), font2, format2, brush1, false);
          }
          else
          {
            ...
            if ((double) charFormat.CharacterSpacing != 0.0)
            {
              this.DrawStringBasedOnCharSpacing(scriptType, ltWidget.CharacterRange, font5, brush1, new RectangleF(bounds.X, bounds.Y, width6, bounds.Height), strArray[index], format1, charFormat);
            }
            else
            {
              string str = this.GetEmbedFontStyle(charFormat);
              if (!string.IsNullOrEmpty(str))
                str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
              PdfFont font6 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font5, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font5.Size, this.GetFontStyle(font5.Style));
              PdfBrush brush4 = (PdfBrush) new PdfSolidBrush((PdfColor) textColor);
              PdfStringFormat format3 = this.PDFGraphics.ConvertFormat(format2, charFormat.ComplexScript || this.EnableComplexScript);
              this.PDFGraphics.DrawString(strArray[index], font6, brush4, new RectangleF(bounds.X, bounds.Y + num2, width6, bounds.Height), format3, true);
            }
          }
        }
        Font font7 = flag4 ? font1 : font2;
        float num13 = num7 + ltWidget.WordSpace / num8;
        if (font7.Underline && index != strArray.Length - 1)
        {
          string text1 = "";
          double num14;
          double width8;
          for (; (double) num13 > 0.0; num13 = (float) (num14 - width8))
          {
            text1 += " ";
            num14 = (double) num13;
            sizeF7 = this.MeasureString(" ", font2, format1);
            width8 = (double) sizeF7.Width;
          }
          sizeF7 = this.MeasureString(text1, font7, format1);
          float width9 = sizeF7.Width;
          Font font8 = !isSubSuperScriptNone ? charFormat.Document.FontSettings.GetFont(font7.Name, font7.Size / 1.5f, font7.Style) : font7;
          ...
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          PdfFont font9 = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font1, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font8.Size, this.GetFontStyle(font8.Style));
          PdfBrush brush5 = (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat));
          PdfStringFormat format4 = this.PDFGraphics.ConvertFormat(format1, charFormat.ComplexScript || this.EnableComplexScript);
          this.PDFGraphics.DrawString(text1, font9, brush5, new RectangleF(bounds.X + width6, bounds.Y + num2, width9, bounds.Height), format4, true);
        }
        if (!ltWidget.IsLastLine && index != strArray.Length - 1)
        {
          ...
          Font font10 = !isSubSuperScriptNone ? charFormat.Document.FontSettings.GetFont(font7.Name, font7.Size / 1.5f, font7.Style) : font7;
          if (!charFormat.GetFontNameToRender(scriptType).Equals(font10.Name, StringComparison.OrdinalIgnoreCase) && this.m_privateFontStream != null && this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)))
            font10 = this.GetPrivateFont(charFormat.GetFontNameToRender(scriptType), font10.Size, font10.Style);
          ...
          if (!string.IsNullOrEmpty(str))
            str = $"{charFormat.GetFontNameToRender(scriptType)}_{str}";
          this.PDFGraphics.DrawString(" ", this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str) && !this.m_privateFontStream.ContainsKey(charFormat.GetFontNameToRender(scriptType)) ? this.CreatePdfFont(font1, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, text) : this.CreatePdfFont(str, charFormat.GetFontNameToRender(scriptType), font10.Size, this.GetFontStyle(font10.Style)), (PdfBrush) new PdfSolidBrush((PdfColor) this.GetTextColor(charFormat)), layoutRectangle);
  ...
  private void ScaleTransformMatrix(float scaleFactor, PointF translatePoints, float rotationAngle)
  {
    ...
    this.PDFGraphics.Transform = matrix;
  }
  ...
  {
    RectangleF clipBounds = this.PDFGraphics.ClipBounds;
    this.PDFGraphics.ResetClip();
    this.PDFGraphics.DrawLine(new PdfPen(new PdfColor(lineColor), lineWidth), start, end);
    this.PDFGraphics.SetClip(clipBounds, CombineMode.Replace);
  }
  ...
  private void DrawBorder(Border border, PointF start, PointF end)
  {
    if (border.BorderType == BorderStyle.Cleared || (border.BorderType != BorderStyle.None || border.HasNoneStyle) && border.BorderType == BorderStyle.None)
      return;
    this.PDFGraphics.DrawLine(this.GetPen(border, false), start, end);
  }
  ...
  private void DrawParagraphBorder(Border border, PointF start, PointF end)
  {
    if (border.BorderType == BorderStyle.Cleared || (border.BorderType != BorderStyle.None || border.HasNoneStyle) && border.BorderType == BorderStyle.None)
      return;
    this.PDFGraphics.DrawLine(this.GetPen(border, true), start, end);
  }

  private void DrawBorder(CellLayoutInfo.CellBorder border, PointF start, PointF end)
  {
    if (this.PDFGraphics.Tag == null && this.PDFGraphics.IsTaggedPdf)
      this.PDFGraphics.Tag = (PdfTag) new PdfArtifact();
    this.PDFGraphics.DrawLine(this.GetPen(border.BorderType, border.RenderingLineWidth, border.BorderColor), start, end);
  }
  ...
  internal virtual void DrawTable(WTable table, LayoutedWidget ltWidget)
  {
    if ((double) table.TableFormat.CellSpacing > 0.0)
    {
      ...
      if (table.TableFormat.TextureStyle != TextureStyle.TextureNone)
        ...
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) table.TableFormat.BackColor), ltWidget.Bounds);
  ...
  internal virtual void DrawTableCell(WTableCell cell, LayoutedWidget ltWidget)
  {
    ...
    else if (!cell.CellFormat.BackColor.IsEmpty && cell.Owner is WTableRow)
    {
      if ((double) ltWidget.Bounds.Height < (double) layoutInfo.UpdatedTopPadding && cell.GridSpan > (short) 1 && (cell.Owner as WTableRow).HeightType == TableRowHeightType.Exactly)
        ...
        this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(cell.CellFormat.BackColor), bounds);
    }
    if (flag1)
      this.ResetTransform();
    if (!flag3)
      ...
      if (layoutedWidget.Widget is WTableCell)
      {
        ...
        if ((double) layoutedWidget.Bounds.X < (double) ltWidget.Bounds.Right && (double) layoutedWidget.Bounds.Right > (double) ltWidget.Bounds.X)
        {
          ...
          if (ltWidget.Widget is WTableCell)
            this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush((ltWidget.Widget as WTableCell).CellFormat.BackColor), bounds);
        }
      }
    }
  }
    ...
    RectangleF bounds)
  {
    ...
    if (textureStyle.ToString().Contains("Percent"))
    {
      ...
      this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(this.GetForeColor(foreColor, backColor, percent)), bounds);
    }
    ...
    RectangleF bounds)
  {
    ...
    switch (textureStyle)
    {
      case TextureStyle.TextureSolid:
        this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(foreColor), bounds);
        break;
      case TextureStyle.TextureDarkHorizontal:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.Horizontal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDarkVertical:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.DarkVertical, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDarkDiagonalDown:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.DarkDownwardDiagonal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDarkDiagonalUp:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.DarkUpwardDiagonal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDarkDiagonalCross:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.DiagonalCross, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureHorizontal:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.LightHorizontal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureVertical:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.LightVertical, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDiagonalDown:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.LightDownwardDiagonal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureDiagonalUp:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.LightUpwardDiagonal, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
      case TextureStyle.TextureCross:
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfHatchBrush(PdfHatchStyle.Cross, (PdfColor) foreColor, (PdfColor) backColor), bounds);
        break;
    ...
    bool isLastCell)
  {
    ...
    PdfPen pen1 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[0], leftBorder.BorderColor);
    PointF point1_1 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
    PointF point2_1 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
    if (cellLayoutInfo.PrevCellTopBorder != null && cellLayoutInfo.PrevCellTopBorder.BorderType == leftBorder.BorderType && (double) cellLayoutInfo.PrevCellTopBorder.RenderingLineWidth == (double) leftBorder.RenderingLineWidth)
      point1_1 = new PointF(point1_1.X, point1_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
    if (isLastRow && cellLayoutInfo.PrevCellBottomBorder != null && cellLayoutInfo.PrevCellBottomBorder.BorderType != leftBorder.BorderType && (double) cellLayoutInfo.PrevCellBottomBorder.RenderingLineWidth != (double) leftBorder.RenderingLineWidth && cellLayoutInfo.BottomBorder != null && cellLayoutInfo.BottomBorder.BorderType == leftBorder.BorderType && (double) cellLayoutInfo.BottomBorder.RenderingLineWidth == (double) leftBorder.RenderingLineWidth || isFirstCell && cellLayoutInfo.BottomBorder != null && cellLayoutInfo.BottomBorder.BorderType == leftBorder.BorderType && (double) cellLayoutInfo.BottomBorder.RenderingLineWidth == (double) leftBorder.RenderingLineWidth)
      point2_1 = new PointF(point2_1.X, point2_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PointF point1_2 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    PdfPen pen2 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.BorderColor);
    ...
    if (cellBorder != null && cellBorder.BorderType == leftBorder.BorderType && (double) cellBorder.RenderingLineWidth == (double) leftBorder.RenderingLineWidth)
      point1_2 = new PointF(point1_2.X, point1_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
  }
    ...
    bool isLastCell)
  {
    ...
    PdfPen pen1 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[0], rightBorder.BorderColor);
    ...
    PointF point1_2 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    PdfPen pen2 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.BorderColor);
    ...
    if (isLastRow && cellLayoutInfo.NextCellBottomBorder != null && cellLayoutInfo.NextCellBottomBorder.BorderType != rightBorder.BorderType && (double) cellLayoutInfo.NextCellBottomBorder.RenderingLineWidth != (double) rightBorder.RenderingLineWidth && cellLayoutInfo.BottomBorder != null && cellLayoutInfo.BottomBorder.BorderType == rightBorder.BorderType && (double) cellLayoutInfo.BottomBorder.RenderingLineWidth == (double) rightBorder.RenderingLineWidth || isLastCell && cellLayoutInfo.BottomBorder != null && cellLayoutInfo.BottomBorder.BorderType == rightBorder.BorderType && (double) cellLayoutInfo.BottomBorder.RenderingLineWidth == (double) rightBorder.RenderingLineWidth)
      point2_2 = new PointF(point2_2.X, point2_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
  }
    ...
    bool isBiDiTable)
  {
    ...
    PdfPen pen1 = this.GetPen(cellLayoutInfo.BottomBorder.BorderType, borderLineWidthArray[0], cellLayoutInfo.BottomBorder.BorderColor);
    PointF point1_1 = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
    PointF point2_1 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
    if (!isBiDiTable && cellLayoutInfo.LeftBorder != null && cellLayoutInfo.LeftBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && (double) cellLayoutInfo.LeftBorder.RenderingLineWidth == (double) cellLayoutInfo.BottomBorder.RenderingLineWidth)
      point1_1 = new PointF(point1_1.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1_1.Y);
      ...
      point1_1 = new PointF(point1_1.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1_1.Y);
    if (isBiDiTable && cellLayoutInfo.LeftBorder != null && cellLayoutInfo.LeftBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && (double) cellLayoutInfo.LeftBorder.RenderingLineWidth == (double) cellLayoutInfo.BottomBorder.RenderingLineWidth)
      point2_1 = new PointF(point2_1.X + borderLineWidthArray[0], point2_1.Y);
    else if (!isBiDiTable && cellLayoutInfo.RightBorder != null && cellLayoutInfo.RightBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && (double) cellLayoutInfo.RightBorder.RenderingLineWidth == (double) cellLayoutInfo.BottomBorder.RenderingLineWidth)
      point2_1 = new PointF(point2_1.X + borderLineWidthArray[0], point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(cellLayoutInfo.BottomBorder.BorderType, borderLineWidthArray[2], cellLayoutInfo.BottomBorder.BorderColor);
    PointF point1_2 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    PointF point2_2 = new PointF(end.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    if (!isBiDiTable && cellLayoutInfo.RightBorder != null && cellLayoutInfo.RightBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && (double) cellLayoutInfo.RightBorder.RenderingLineWidth == (double) cellLayoutInfo.BottomBorder.RenderingLineWidth)
      point2_2 = new PointF(point2_2.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2], point2_2.Y);
    else if (isBiDiTable && cellLayoutInfo.RightBorder != null && cellLayoutInfo.LeftBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && (double) cellLayoutInfo.LeftBorder.RenderingLineWidth == (double) cellLayoutInfo.BottomBorder.RenderingLineWidth)
      point2_2 = new PointF(point2_2.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2], point2_2.Y);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
  }
    ...
    bool isEnd)
  {
    ...
    PdfPen pen1 = this.GetPen(topBorder.BorderType, borderLineWidthArray[0], topBorder.BorderColor);
    PointF point1_1 = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
    PointF point2_1 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
    ...
    if (topBorder.AdjCellRightBorder != null && topBorder.AdjCellRightBorder.BorderType == topBorder.BorderType && (double) topBorder.AdjCellRightBorder.RenderingLineWidth == (double) topBorder.RenderingLineWidth)
      point2_1 = new PointF(point2_1.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(topBorder.BorderType, borderLineWidthArray[2], topBorder.BorderColor);
    PointF point1_2 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    PointF point2_2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    if (isStart && cellLayoutInfo.LeftBorder != null && cellLayoutInfo.LeftBorder.BorderType == topBorder.BorderType && (double) cellLayoutInfo.LeftBorder.RenderingLineWidth == (double) topBorder.RenderingLineWidth)
      point1_2 = new PointF(point1_2.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1_2.Y);
    if (isEnd && cellLayoutInfo.RightBorder != null && cellLayoutInfo.RightBorder.BorderType == topBorder.BorderType && (double) cellLayoutInfo.RightBorder.RenderingLineWidth == (double) topBorder.RenderingLineWidth)
      point2_2 = new PointF(point2_2.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), point2_2.Y);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
  }
    ...
    bool isBottomBorderSame)
  {
    ...
    PdfPen pen1 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[0], leftBorder.Color);
    PointF point1_1 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
    PointF point2_1 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
    if (isBottomBorderSame)
      point2_1 = new PointF(point2_1.X, point2_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.Color);
    PointF point1_2 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    ...
    bool isBottomBorderSame)
  {
    ...
    PointF point2_1 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
    if (isBottomBorderSame)
      point2_1 = new PointF(point2_1.X, point2_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.Color);
    PointF point1_2 = new PointF(start.X + (float) ((double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF(end.X + (float) ((double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    if (isTopBorderSame)
      point1_2 = new PointF(point1_2.X, point1_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
    if (isBottomBorderSame)
      point2_2 = new PointF(point2_2.X, point2_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
    PdfPen pen3 = this.GetPen(leftBorder.BorderType, borderLineWidthArray[4], leftBorder.Color);
    PointF point1_3 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0), start.Y);
    PointF point2_3 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0), end.Y);
    if (isTopBorderSame || isBetweenBorderSame)
      point1_3 = new PointF(point1_3.X, point1_3.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3]);
    this.PDFGraphics.DrawLine(pen3, point1_3, point2_3);
  }
    ...
    bool isBottomBorderSame)
  {
    ...
    PdfPen pen1 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[0], rightBorder.Color);
    PointF point1_1 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
    PointF point2_1 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
    if (isTopBorderSame || isBetweenBorderSame)
      point1_1 = new PointF(point1_1.X, point1_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.Color);
    PointF point1_2 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    if (isBottomBorderSame)
      point2_2 = new PointF(point2_2.X, point2_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    ...
    bool isBottomBorderSame)
  {
    ...
    PointF point2_1 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
    if (isTopBorderSame || isBetweenBorderSame)
      point1_1 = new PointF(point1_1.X, point1_1.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3]);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.Color);
    PointF point1_2 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), start.Y);
    PointF point2_2 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0), end.Y);
    ...
    if (isBottomBorderSame)
      point2_2 = new PointF(point2_2.X, point2_2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
    PdfPen pen3 = this.GetPen(rightBorder.BorderType, borderLineWidthArray[4], rightBorder.Color);
    PointF point1_3 = new PointF((float) ((double) start.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + ((double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0)), start.Y);
    PointF point2_3 = new PointF((float) ((double) end.X + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0), end.Y);
    if (isBottomBorderSame)
      point2_3 = new PointF(point2_3.X, point2_3.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
    this.PDFGraphics.DrawLine(pen3, point1_3, point2_3);
  }
    ...
    bool isRightBorderSame)
  {
    ...
    this.PDFGraphics.DrawLine(this.GetPen(topBorder.BorderType, borderLineWidthArray[0], topBorder.Color), new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f), new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f));
    PdfPen pen = this.GetPen(topBorder.BorderType, borderLineWidthArray[2], topBorder.Color);
    PointF point1 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    PointF point2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    if (isLeftBorderSame)
      point1 = new PointF(point1.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1.Y);
    if (isRightBorderSame)
      point2 = new PointF(point2.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), point2.Y);
    this.PDFGraphics.DrawLine(pen, point1, point2);
  }
    ...
    bool isRightBorderSame)
  {
    float[] borderLineWidthArray = this.GetBorderLineWidthArray(topBorder.BorderType, topBorder.LineWidth);
    this.PDFGraphics.DrawLine(this.GetPen(topBorder.BorderType, borderLineWidthArray[0], topBorder.Color), new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f), new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f));
    PdfPen pen1 = this.GetPen(topBorder.BorderType, borderLineWidthArray[2], topBorder.Color);
    PointF point1_1 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    PointF point2_1 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    if (isLeftBorderSame)
      point1_1 = new PointF(point1_1.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1_1.Y);
    if (isRightBorderSame)
      point2_1 = new PointF(point2_1.X - (borderLineWidthArray[3] + borderLineWidthArray[4]), point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(topBorder.BorderType, borderLineWidthArray[4], topBorder.Color);
    PointF point1_2 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0));
    PointF point2_2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] + (double) borderLineWidthArray[3] + (double) borderLineWidthArray[4] / 2.0));
    if (isLeftBorderSame)
      point1_2 = new PointF(point1_2.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3], point1_2.Y);
    if (isRightBorderSame)
      point2_2 = new PointF(point2_2.X - (borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]), point2_2.Y);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
  }
    ...
    bool isRightBorderSame)
  {
    ...
    PdfPen pen = this.GetPen(bottomBorder.BorderType, borderLineWidthArray[0], bottomBorder.Color);
    PointF point1 = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
    ...
    PointF end)
  {
    ...
    this.PDFGraphics.DrawLine(this.GetPen(borderType, borderLineWidthArray[0], borderColor), new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f), new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f));
    this.PDFGraphics.DrawLine(this.GetPen(borderType, borderLineWidthArray[2], borderColor), new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0)), new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0)));
  }
  ...
  {
    float[] borderLineWidthArray = this.GetBorderLineWidthArray(bottomBorder.BorderType, bottomBorder.LineWidth);
    PdfPen pen1 = this.GetPen(bottomBorder.BorderType, borderLineWidthArray[0], bottomBorder.Color);
    PointF point1_1 = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
    PointF point2_1 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
    if (isLeftBorderSame)
      point1_1 = new PointF(point1_1.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3], point1_1.Y);
    if (isRightBorderSame)
      point2_1 = new PointF(point2_1.X - (borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]), point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(bottomBorder.BorderType, borderLineWidthArray[2], bottomBorder.Color);
    PointF point1_2 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    PointF point2_2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray[0] + (double) borderLineWidthArray[1] + (double) borderLineWidthArray[2] / 2.0));
    if (isLeftBorderSame)
      point1_2 = new PointF(point1_2.X + borderLineWidthArray[0] + borderLineWidthArray[1], point1_2.Y);
    ...
    bool isOverlapRight)
  {
    ...
    PdfPen pen1 = this.GetPen(betweenBorder.BorderType, borderLineWidthArray1[0], betweenBorder.Color);
    PointF point1_1 = new PointF(start.X, start.Y + borderLineWidthArray1[0] / 2f);
    PointF point2_1 = new PointF(end.X, end.Y + borderLineWidthArray1[0] / 2f);
    if (isLeftBorderSame)
      point1_1 = new PointF(point1_1.X + borderLineWidthArray1[0] + borderLineWidthArray1[1], point1_1.Y);
    else if (borderLineWidthArray2 != null)
      point1_1 = new PointF(point1_1.X + this.GetLeftRightLineWidht(borderLineWidthArray2, true), point1_1.Y);
    if (isRightBorderSame)
      point2_1 = new PointF(point2_1.X - (borderLineWidthArray1[1] + borderLineWidthArray1[2]), point2_1.Y);
    else if (borderLineWidthArray3 != null)
      point2_1 = new PointF(point2_1.X - this.GetLeftRightLineWidht(borderLineWidthArray3, false), point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(betweenBorder.BorderType, borderLineWidthArray1[2], betweenBorder.Color);
    PointF point1_2 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] / 2.0));
    PointF point2_2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] / 2.0));
    if (isLeftBorderSame)
      point1_2 = new PointF(point1_2.X + borderLineWidthArray1[0] + borderLineWidthArray1[1], point1_2.Y);
    else if (borderLineWidthArray2 != null)
      point1_2 = new PointF(point1_2.X + this.GetLeftRightLineWidht(borderLineWidthArray2, true), point1_2.Y);
    if (isRightBorderSame)
      point2_2 = new PointF(point2_2.X - (borderLineWidthArray1[1] + borderLineWidthArray1[2]), point2_2.Y);
    else if (borderLineWidthArray3 != null)
      point2_2 = new PointF(point2_2.X - this.GetLeftRightLineWidht(borderLineWidthArray3, false), point2_2.Y);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
    if (!isOverlapRight && !isOverlapLeft)
      return;
    if (isOverlapLeft)
    {
      PointF point1_3 = new PointF(start.X + borderLineWidthArray1[0] / 2f, start.Y);
      PointF point2_3 = new PointF(start.X + borderLineWidthArray1[0] / 2f, start.Y + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2]);
      this.PDFGraphics.DrawLine(pen1, point1_3, point2_3);
    }
    if (!isOverlapRight)
      return;
    PointF point1_4 = new PointF(end.X - borderLineWidthArray1[0] / 2f, end.Y);
    PointF point2_4 = new PointF(end.X - borderLineWidthArray1[0] / 2f, end.Y + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2]);
    this.PDFGraphics.DrawLine(pen1, point1_4, point2_4);
  }
    ...
    bool isOverlapRight)
  {
    ...
    PointF point2_1 = new PointF(end.X, end.Y + borderLineWidthArray1[0] / 2f);
    if (isLeftBorderSame)
      point1_1 = new PointF(point1_1.X + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3], point1_1.Y);
    else if (borderLineWidthArray2 != null)
      point1_1 = new PointF(point1_1.X + this.GetLeftRightLineWidht(borderLineWidthArray2, true), point1_1.Y);
    if (isRightBorderSame)
      point2_1 = new PointF(point2_1.X - (borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3] + borderLineWidthArray1[4]), point2_1.Y);
    else if (borderLineWidthArray3 != null)
      point2_1 = new PointF(point2_1.X - this.GetLeftRightLineWidht(borderLineWidthArray3, false), point2_1.Y);
    this.PDFGraphics.DrawLine(pen1, point1_1, point2_1);
    PdfPen pen2 = this.GetPen(betweenBorder.BorderType, borderLineWidthArray1[2], betweenBorder.Color);
    ...
    PointF point2_2 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] / 2.0));
    if (isLeftBorderSame)
      point1_2 = new PointF((float) ((double) point1_2.X + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (isOverlapLeft ? 0.0 : (double) borderLineWidthArray1[2] + (double) borderLineWidthArray1[3])), point1_2.Y);
    else if (borderLineWidthArray2 != null)
      point1_2 = new PointF(point1_2.X + this.GetLeftRightLineWidht(borderLineWidthArray2, true), point1_2.Y);
    if (isRightBorderSame)
      point2_2 = new PointF(point2_2.X - ((isOverlapRight ? 0.0f : borderLineWidthArray1[1] + borderLineWidthArray1[2]) + borderLineWidthArray1[3] + borderLineWidthArray1[4]), point2_2.Y);
    else if (borderLineWidthArray3 != null)
      point2_2 = new PointF(point2_2.X - this.GetLeftRightLineWidht(borderLineWidthArray3, false), point2_2.Y);
    this.PDFGraphics.DrawLine(pen2, point1_2, point2_2);
    PdfPen pen3 = this.GetPen(betweenBorder.BorderType, borderLineWidthArray1[4], betweenBorder.Color);
    PointF point1_3 = new PointF(start.X, (float) ((double) start.Y + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] + (double) borderLineWidthArray1[3] + (double) borderLineWidthArray1[4] / 2.0));
    PointF point2_3 = new PointF(end.X, (float) ((double) end.Y + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] + (double) borderLineWidthArray1[3] + (double) borderLineWidthArray1[4] / 2.0));
    if (isLeftBorderSame)
      point1_3 = new PointF(point1_3.X + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3], point1_3.Y);
    else if (borderLineWidthArray2 != null)
      point1_3 = new PointF(point1_3.X + this.GetLeftRightLineWidht(borderLineWidthArray2, true), point1_3.Y);
    if (isRightBorderSame)
      point2_3 = new PointF(point2_3.X - (borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3] + borderLineWidthArray1[4]), point2_3.Y);
    else if (borderLineWidthArray3 != null)
      point2_3 = new PointF(point2_3.X - this.GetLeftRightLineWidht(borderLineWidthArray3, false), point2_3.Y);
    this.PDFGraphics.DrawLine(pen3, point1_3, point2_3);
    if (!isOverlapRight && !isOverlapLeft)
      return;
    if (isOverlapLeft)
    {
      PointF point1_4 = new PointF(start.X + borderLineWidthArray1[0] / 2f, start.Y);
      PointF point2_4 = new PointF(point1_4.X, start.Y + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3] + borderLineWidthArray1[4]);
      this.PDFGraphics.DrawLine(pen1, point1_4, point2_4);
      point1_4 = new PointF((float) ((double) start.X + (double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] / 2.0), start.Y);
      point2_4 = new PointF(point1_4.X, point2_4.Y);
      this.PDFGraphics.DrawLine(pen1, point1_4, point2_4);
    }
    if (!isOverlapRight)
      return;
    PointF point1_5 = new PointF(end.X - borderLineWidthArray1[0] / 2f, end.Y);
    PointF point2_5 = new PointF(point1_5.X, end.Y + borderLineWidthArray1[0] + borderLineWidthArray1[1] + borderLineWidthArray1[2] + borderLineWidthArray1[3] + borderLineWidthArray1[4]);
    this.PDFGraphics.DrawLine(pen1, point1_5, point2_5);
    point1_5 = new PointF(end.X - (float) ((double) borderLineWidthArray1[0] + (double) borderLineWidthArray1[1] + (double) borderLineWidthArray1[2] / 2.0), end.Y);
    point2_5 = new PointF(point1_5.X, point2_5.Y);
    this.PDFGraphics.DrawLine(pen1, point1_5, point2_5);
  }
  ...
  {
    this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(bgColor), (RectangleF) new Rectangle(0, 0, width, height));
  }

  internal void DrawBackgroundImage(Image image, WPageSetup pageSetup)
  {
    this.PDFGraphics.DrawImage(this.GetPdfImage(image), 0.0f, 0.0f, pageSetup.PageSize.Width, pageSetup.PageSize.Height);
    if (image == null)
      return;
    image.Dispose();
    image = (Image) null;
  ...
  internal void Draw(Page page, ref int autoTagsCount)
  {
    ...
    this.PDFGraphics.DocIOPageBounds = new RectangleF(0.0f, 0.0f, (float) (int) page.Setup.PageSize.Width, (float) (int) page.Setup.PageSize.Height);
    this.autoTagCount = autoTagsCount;
    bool autoTag = this.AutoTag;
    bool flag1 = false;
    ...
    if (page.PageWidgets.Count != 0)
    {
      ...
      if (page.DocSection.Document.TrackChangesBalloonCount > 0)
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) Color.FromArgb(240 /*0xF0*/, 240 /*0xF0*/, 240 /*0xF0*/)), new RectangleF(x, 0.0f, 250f, page.DocSection.PageSetup.PageSize.Height));
    }
    bool flag2 = true;
    bool flag3 = false;
    for (int index1 = 0; index1 < page.PageWidgets.Count; ++index1)
    {
      ...
      if (pageWidget.Widget is HeaderFooter && autoTag)
        this.AutoTag = false;
      ...
      if (!(pageWidget.Widget is HeaderFooter))
      {
        ...
        for (int index4 = 0; index4 < page.TrackChangesMarkups.Count; ++index4)
        {
          ...
          RectangleF bounds = new RectangleF(x + 20f, trackChangesMarkup.LtWidget.Bounds.Y, 210f, trackChangesMarkup.LtWidget.Bounds.Height);
          float num = trackChangesMarkup is CommentsMarkups ? (trackChangesMarkup as CommentsMarkups).ExtraSpacing : 0.0f;
          PointF point1 = new PointF(trackChangesMarkup.Position.X, trackChangesMarkup.Position.Y - num);
          PointF pointF = new PointF(x - 5f, trackChangesMarkup.Position.Y - num);
          PointF point2 = new PointF(x + 20f, trackChangesMarkup.LtWidget.Bounds.Y + 10f);
          ...
          PdfPen pen1 = new PdfPen(color, 1f);
          PdfPen pen2 = new PdfPen(color, 0.5f);
          pen2.DashStyle = PdfDashStyle.Dot;
          this.PDFGraphics.DrawLine(pen2, point1, pointF);
          this.PDFGraphics.DrawLine(pen2, pointF, point2);
          this.PDFGraphics.DrawRoundedRectangle(bounds, 5, pen1, trackChangesMarkup is CommentsMarkups ? (PdfBrush) new PdfSolidBrush((PdfColor) this.GetRevisionFillColor(revisionOptions.CommentColor, isResolvedComment)) : (PdfBrush) new PdfSolidBrush((PdfColor) Color.White));
          if (!(trackChangesMarkup is CommentsMarkups))
    ...
    autoTagsCount = this.autoTagCount;
  }

  private void DrawMarkupTriangles(PointF position, Color revisionColor)
  {
    PdfBrush brush = (PdfBrush) new PdfSolidBrush((PdfColor) revisionColor);
    PdfPath path = new PdfPath();
    ...
    path.AddLine(pointF2, position);
    path.AddLine(position, pointF1);
    path.CloseFigure();
    this.PDFGraphics.DrawPath(brush, path);
  }
...
label_10:
  ...
  internal void Draw(BookmarkStart bookmarkStart, LayoutedWidget ltWidget)
  {
    if (bookmarkStart.Name.StartsWithExt("_") || (this.ExportBookmarks & ExportBookmarkType.Bookmarks) == ExportBookmarkType.None)
      return;
    PDFDrawingContext.Bookmarks.Add(new BookmarkPosition(bookmarkStart.Name, DocumentLayouter.PageNumber, ltWidget.Bounds));
  }
  ...
  internal void DrawEmptyString(LayoutedWidget layoutedWidget, WParagraph paragraph)
  {
    ...
    string str1 = " ";
    string str2 = this.GetEmbedFontStyle(breakCharacterFormat);
    if (!string.IsNullOrEmpty(str2))
      str2 = $"{breakCharacterFormat.GetFontNameToRender(FontScriptType.English)}_{str2}";
    Font fontToRender = breakCharacterFormat.GetFontToRender(FontScriptType.English);
    PdfFont font = this.m_privateFontStream == null || !this.m_privateFontStream.ContainsKey(str2) && !this.m_privateFontStream.ContainsKey(breakCharacterFormat.GetFontNameToRender(FontScriptType.English)) ? this.CreatePdfFont(fontToRender, this.EmbedFonts || this.EmbedCompleteFonts, this.EmbedCompleteFonts, str1) : this.CreatePdfFont(str2, breakCharacterFormat.GetFontNameToRender(FontScriptType.English), fontToRender.Size, this.GetFontStyle(fontToRender.Style));
    this.PDFGraphics.DrawString(str1, font, PdfBrushes.Black, new PointF(layoutedWidget.Bounds.X, layoutedWidget.Bounds.Y));
    this.PDFGraphics.ReSetTag();
  }
  ...
  internal void Draw(LayoutedWidget layoutedWidget, bool isHaveToInitLayoutInfo)
  {
    ...
    if (this.AutoTag)
    {
      WParagraph ownerParagraph = this.GetOwnerParagraph(layoutedWidget);
      if (ownerParagraph != null && layoutedWidget.ChildWidgets.Count > 0 && this.GetOwnerParagraph(layoutedWidget.ChildWidgets[0]) == ownerParagraph && layoutedWidget.ChildWidgets[0].ChildWidgets.Count > 0 && (!(layoutedWidget.ChildWidgets[0].ChildWidgets[0].Widget is Break) || (layoutedWidget.ChildWidgets[0].ChildWidgets[0].Widget as Break).BreakType != BreakType.ColumnBreak))
      {
        this.PDFGraphics.SetTag(this.CreatePdfStructureElement(ownerParagraph, layoutedWidget));
        if (this.IsTextRange(ownerParagraph))
          flag1 = true;
        else
          flag2 = true;
      }
    ...
    if (this.AutoTag && flag2)
    {
      WParagraph ownerParagraph = this.GetOwnerParagraph(layoutedWidget);
      this.DrawEmptyString(layoutedWidget, ownerParagraph);
    }
    int index1 = 0;
    for (int count = layoutedWidget.ChildWidgets.Count; index1 < count; ++index1)
    {
      ...
      if (this.AutoTag && childWidget != null)
        this.CreatePdfStructureElement(childWidget, ownerLayoutedWidget, cellWidget);
      ...
      else if (childWidget != null)
      {
          ...
          case ChildShape _:
            ...
            switch (ownerLayoutedWidget)
            {
              ...
              case Shape _:
                ...
                num = shape.Rotation;
                flag3 = shape.FlipHorizontal;
                flag4 = shape.FlipVertical;
                break;
              case ChildShape _:
                ChildShape childShape = ownerLayoutedWidget as ChildShape;
                num = childShape.RotationToRender;
                flag3 = childShape.FlipHorizantalToRender;
                flag4 = childShape.FlipVerticalToRender;
                break;
            }
            if ((double) num != 0.0 || (double) num == 0.0 && (flag3 || flag4))
            {
        ...
        if (this.AutoTag)
        {
          if (childWidget.Widget is WTableRow)
            this.pdfStructureTable.Pop();
          else if (childWidget.Widget is WTable)
          {
            this.pdfStructureTable.Pop();
            if (this.pdfStructureTable.Count == 0)
            {
              this.pdfStructureTable.Clear();
              this.pdfStructureTable = (Stack<PdfStructureElement>) null;
            }
          }
        }
      }
    }
    if (this.AutoTag && flag1)
      this.PDFGraphics.ReSetTag();
  ...
  private void DrawUnderLine(WCharacterFormat characterFormat, RectangleF textBounds)
  {
    ...
    this.PDFGraphics.DrawLine(this.CreatePen(characterFormat, lineWidth), new PointF(x, y2), new PointF(x + textBounds.Width, y2));
  }
  ...
  private void DrawStrikeThrough(WCharacterFormat characterFormat, RectangleF textBounds)
  {
    ...
    if (characterFormat.Strikeout)
    {
      ...
      this.PDFGraphics.DrawLine(this.CreatePen(characterFormat, lineWidth), new PointF(x1, y2), new PointF(x1 + textBounds.Width, y2));
    }
    else
    {
  ...
  private void SetClip(RectangleF clippingBounds)
  {
    if (!(clippingBounds != RectangleF.Empty))
      return;
    this.PDFGraphics.SetClip(this.GetClipBounds(clippingBounds, clippingBounds.Width, 0.0f), CombineMode.Replace);
  }

  private void ResetClip() => this.PDFGraphics.ResetClip();

  internal void ResetTransform() => this.PDFGraphics.ResetTransform();

  private void SetRotateTransform(RectangleF rotateBounds, float rotation, bool flipV, bool flipH)
  {
    this.PDFGraphics.Transform = this.GetTransformMatrix(rotateBounds, rotation, flipH, flipV);
  }
    ...
    bool isResolvedComment)
  {
    if ((double) bounds.Width <= 0.0)
      return;
    this.PDFGraphics.DrawRectangle((PdfBrush) this.GetPDFBrush(this.GetRevisionFillColor(revisionOptions.CommentColor, isResolvedComment)), bounds);
  }
  ...
  private void DrawLineBackGroundColors(WParagraph paragraph, LayoutedWidget ltWidget)
  {
    ...
    {
      for (int index = 0; index < colorRenderingBounds.Count; ++index)
        this.PDFGraphics.DrawRectangle((PdfBrush) new PdfSolidBrush((PdfColor) paragraphFormat.BackColor), colorRenderingBounds[index]);
    }
  ...
  internal void DrawChart(WChart chart, LayoutedWidget widget)
  {
    ...
    {
      PdfImage pdfImage = this.GetPdfImage(memoryStream);
      if (this.AutoTag)
      {
        pdfImage.PdfTag = (PdfTag) this.CreateAutoTag(PdfTagType.Figure, chart.AlternativeText, chart.Title, this.IsOverLappedShapeWidget(widget));
        this.PDFGraphics.DrawImage(pdfImage, bounds);
        this.PDFGraphics.ResetTransform();
      }
      else
      {
  ...
  {
    if (this.PreserveFormFields)
      this.DrawEditableCheckbox(checkBox, ltWidget);
    else
  ...
  {
    if (this.PreserveFormFields)
      this.DrawEditableDropDown(dropDownFormField, ltWidget);
    else
  ...
  internal void Draw(IStringWidget stringWidget, LayoutedWidget ltWidget, string text)
  {
    if (this.PreserveFormFields)
    {
      ...
      if (this.m_editableTextFormTextRange != null)
      {
        ...
        string str = ((this.m_editableTextFormTextRange.PreviousSibling as WFieldMark).ParentField as WTextFormField).Text.Replace(" ", string.Empty);
        this.m_editableTextFormText = this.m_editableTextFormText.Replace(" ", string.Empty);
        if (!(this.m_editableTextFormText == str))
          return;
        this.DrawEditableTextRange(stringWidget as WTextRange, ltWidget);
    ...
    WPageSetup pageSetup)
  {
    ...
    this.CreateImageAttributes().SetColorMatrix(colorMatrix, ColorMatrixFlag.Default, ColorAdjustType.Bitmap);
    if (textWatermark.Rotation != 0)
    {
      bounds = new RectangleF(bounds.X, bounds.Y, bounds.Width, bounds.Height);
      this.PDFGraphics.Transform = this.GetTransformMatrix(bounds, (float) textWatermark.Rotation);
    ...
    stream.Dispose();
    this.PDFGraphics.DrawImage(pdfImage, (RectangleF) rectangle);
    if (textWatermark.Rotation != 0)
      this.ResetTransform();
    bitmap.Dispose();
  }
  ...
  private Bitmap ConvertAsImage(TextWatermark textWatermark, Font font)
  {
    ...
    System.Drawing.Graphics graphics2 = System.Drawing.Graphics.FromImage((Image) bitmap);
    graphics2.TextRenderingHint = TextRenderingHint.AntiAlias;
    graphics2.SmoothingMode = SmoothingMode.HighQuality;
    graphics2.InterpolationMode = InterpolationMode.HighQualityBicubic;
    Color color = textWatermark.Color;
    if (textWatermark.Semitransparent)
      color = this.ChangeColorBrightness(color, 0.5f);
  ...
  private float GetFontSize(TextWatermark textWatermark)
  {
    ...
    System.Drawing.Graphics graphics = System.Drawing.Graphics.FromImage((Image) new Bitmap(1, 1));
    ...
    WPageSetup pageSetup)
  {
    ...
    if (pictureWatermark.Washout)
    {
      for (int x = 0; x < bitmap.Width; ++x)
      {
        ...
        {
          Color color = this.ChangeColorBrightness(bitmap.GetPixel(x, y), 0.8f);
          bitmap.SetPixel(x, y, color);
        }
      }
      this.PDFGraphics.DrawImage(this.GetPdfImage((Image) bitmap), (RectangleF) new Rectangle((int) bounds.X, (int) bounds.Y, (int) bounds.Width, (int) bounds.Height));
    }
    else
      this.PDFGraphics.DrawImage(this.GetPdfImage(pictureWatermark.Picture), (RectangleF) new Rectangle((int) bounds.X, (int) bounds.Y, (int) bounds.Width, (int) bounds.Height));
  }
  ...
  internal void DrawEditableCheckbox(WCheckBox checkbox, LayoutedWidget ltWidget)
  {
    ...
    PdfCheckBoxField field = new PdfCheckBoxField(this.PDFGraphics.Page, checkbox.Name);
    field.Bounds = ltWidget.Bounds;
    field.Checked = checkbox.Checked;
    field.ToolTip = checkbox.StatusBarHelp;
    field.Visible = checkbox.Enabled;
    field.ForeColor = (PdfColor) checkbox.CharacterFormat.HighlightColor;
    field.BackColor = (PdfColor) checkbox.CharacterFormat.TextColor;
  ...
  {
    PdfPen pen = new PdfPen(new PdfColor(this.GetTextColor(checkbox.CharacterFormat)));
    pen.Width = 0.7f;
    ...
    this.PDFGraphics.DrawRectangle(pen, rectangleF.X, rectangleF.Y, rectangleF.Width, rectangleF.Height);
    if (!checkbox.Checked)
      return;
    PointF point1 = new PointF(rectangleF.X, rectangleF.Y);
    PointF point2 = new PointF(rectangleF.Right, rectangleF.Bottom);
    this.PDFGraphics.DrawLine(pen, point1, point2);
    point1 = new PointF(rectangleF.Right, rectangleF.Top);
  ...
  internal void DrawShape(Shape shape, LayoutedWidget ltWidget)
  {
    ...
    PdfStructureElement structureElement = (PdfStructureElement) null;
    if (this.AutoTag)
      structureElement = this.CreateAutoTag(PdfTagType.Figure, shape.AlternativeText, shape.Title, this.IsOverLappedShapeWidget(ltWidget));
    ...
    if (((double) rotation != 0.0 || (double) rotation == 0.0 && (flipH || flipV)) && shape.WrapFormat.TextWrappingStyle != TextWrappingStyle.Tight && shape.WrapFormat.TextWrappingStyle != TextWrappingStyle.Through)
    {
      ...
      if ((double) rotation != 0.0 || flipV || flipH)
        this.PDFGraphics.Transform = this.GetTransformMatrix(rectangleF1, rotation, flipH, flipV);
    }
    PdfPath graphicsPath = this.GetGraphicsPath(shape, rectangleF1, ref pen);
    graphicsPath.PdfTag = (PdfTag) structureElement;
    ...
    if (graphicsPath.PointCount > 0)
    {
      if (shape.FillFormat.Fill && this.IsShapeNeedToBeFill(shape.AutoShapeType))
      {
        ...
        if (color != Color.Empty)
        {
          ...
          if (this.AutoTag)
          {
            ...
            this.PDFGraphics.DrawPath(brush, graphicsPath);
          }
          else
            this.PDFGraphics.DrawPath(brush, graphicsPath);
        }
      ...
      {
        pen.LineJoin = this.GetLineJoin(this.GetPdfLineJoin(shape.LineFormat.LineJoin));
        if (shape.LineFormat.Style == LineStyle.ThinThin && (shape.AutoShapeType == AutoShapeType.Line || shape.AutoShapeType == AutoShapeType.StraightConnector) && !shape.FlipHorizontal && !shape.FlipVertical && !this.IsArrowPreserved(shape))
          ...
          this.PDFGraphics.DrawPath(pen, graphicsPath);
      }
    }
    else if (this.AutoTag)
    {
  ...
  private PdfLineJoin GetPdfLineJoin(Syncfusion.DocIO.ReaderWriter.DataStreamParser.Escher.LineJoin lineJoin)
  {
    switch (lineJoin)
    {
  ...
  private void DrawLineShapeBasedOnLineType(Shape shape, RectangleF bounds, PdfPen pen)
  {
    ...
    path1.AddLines(pointsBasedOnFlip);
    this.PDFGraphics.DrawPath(pen, path1);
    ...
    path2.AddLines(pointsBasedOnFlip);
    ...
    RectangleF rect)
  {
    ...
    if ((double) ang <= 0.0 && !flipV && !flipH)
      return;
    this.PDFGraphics.Transform = this.GetTransformMatrix(rect, ang, flipV, flipH);
  }
  ...
  private PdfPath GetGraphicsPath(Shape shape, RectangleF bounds, ref PdfPen pen)
  {
    ...
    switch (shape.AutoShapeType)
    {
      ...
      case AutoShapeType.SmileyFace:
        PdfPath[] smileyFacePath = shapePath.GetSmileyFacePath();
        int num = 0;
        ...
        foreach (PdfPath path2 in smileyFacePath)
        {
          ...
          if (color1 != Color.Empty)
            this.PDFGraphics.DrawPath((PdfBrush) new PdfSolidBrush((PdfColor) color1), path2);
          this.PDFGraphics.DrawPath(pen, path2);
        }
        break;
      ...
      case AutoShapeType.Arc:
        ...
        if (color2 != Color.Empty && shape.FillFormat.Fill && !shape.FillFormat.IsDefaultFill)
          this.PDFGraphics.DrawPath((PdfBrush) new PdfSolidBrush((PdfColor) color2), arcPath[1]);
        if (shape.LineFormat.Line)
        {
      ...
      case AutoShapeType.HorizontalScroll:
        ...
        foreach (PdfPath path3 in horizontalScroll)
        {
          if (color3 != Color.Empty)
            this.PDFGraphics.DrawPath((PdfBrush) new PdfSolidBrush((PdfColor) color3), path3);
      ...
      case AutoShapeType.StraightConnector:
        ...
        PointF[] pointsBasedOnFlip = this.GetLinePointsBasedOnFlip(shape.FlipHorizontal, shape.FlipVertical, bounds);
        this.DrawArrowHead(shape, pen, bounds, ref isArrowHeadExist, ref path1, pointsBasedOnFlip);
        if (!isArrowHeadExist)
          path1.AddLines(pointsBasedOnFlip);
        pen.Width = width;
        return path1;
    ...
    return shape.Path2DList != null && shape.Path2DList.Count > 0 ? shapePath.GetCustomGeomentryPath(bounds, path1, shape) : path1;
  }
  ...
  private PdfPath GetGraphicsPath(ChildShape shape, RectangleF bounds, ref PdfPen pen)
  {
    ...
    switch (autoShapeType)
    {
      ...
      case AutoShapeType.StraightConnector:
        ...
        return path1;
      case AutoShapeType.ElbowConnector:
        PdfPath bentConnectorPath = shapePath.GetBentConnectorPath();
        if (shape.FlipVertical || shape.FlipHorizantal)
          this.PDFGraphics.Transform = this.GetTransformMatrix(bounds, 0.0f, shape.FlipHorizantal, shape.FlipVertical);
...
label_19:
  ...
  private System.Drawing.Graphics GetGraphicsFromImage(Bitmap bmp)
  {
    return System.Drawing.Graphics.FromImage((Image) bmp);
  }
  ...
  private void AddCloseArrowHeadPoints(PointF[] points, PdfPen pen)
  {
    ...
    path.CloseFigure();
    this.Graphics.FillPolygon((Brush) new SolidBrush((Color) pen.Color), points1);
    this.PDFGraphics.DrawPath(pen, path);
  }
  ...
  private PdfSolidBrush GetPDFBrush(Color color) => new PdfSolidBrush((PdfColor) color);
  ...
  private PdfPen GetPen(Border border, bool isParagraphBorder)
  {
    ...
    if (!border.Color.IsEmpty && border.Color.ToArgb() != 0)
      color = border.Color;
    PdfPen pen = new PdfPen(new PdfColor(color), border.LineWidth);
    pen.DashStyle = PdfDashStyle.Custom;
    switch (border.BorderType)
    {
      case BorderStyle.Double:
    ...
    if (!isParagraphBorder)
    {
      pen.StartCap = PdfLineCap.Square;
      pen.EndCap = PdfLineCap.Square;
    }
    return pen;
  }
  ...
  private PdfPen GetPen(BorderStyle borderType, float borderLineWidth, Color borderColor)
  {
    ...
    if (!borderColor.IsEmpty && borderColor.ToArgb() != 0)
      color = borderColor;
    PdfPen pen = new PdfPen(new PdfColor(color), borderLineWidth);
    pen.DashStyle = PdfDashStyle.Custom;
    switch (borderType)
    {
      case BorderStyle.Dot:
  ...
  private PdfPen GetPen(UnderlineStyle underlineStyle, float lineWidth, Color lineColor)
  {
    Color color = Color.Black;
    if (!lineColor.IsEmpty && lineColor.ToArgb() != 0)
      color = lineColor;
    PdfPen pen = new PdfPen(new PdfColor(color), lineWidth);
    pen.DashStyle = PdfDashStyle.Custom;
  ...
  private Image ScaleImage(Image srcImage, float width, float height)
  {
    ...
    this.PDFGraphics.DrawImage(this.GetPdfImage((Image) bitmap), (RectangleF) new Rectangle(x, y, width2, height2));
    return (Image) bitmap;
  }
  ...
  {
    foreach (BookmarkPosition bookmark in PDFDrawingContext.Bookmarks)
    {
  ...
  {
    return text != null && Encoding.UTF8.GetByteCount(text) != text.Length;
  }
  ...
  public new void Close()
  {
    ...
    {
      if (this.m_pdfgraphics != null)
        this.m_pdfgraphics.Clear();
      this.m_graphics = (System.Drawing.Graphics) null;
    }
    if (this.m_graphicsBmp != null)
    {
      this.m_graphicsBmp.Dispose();
      this.m_graphicsBmp = (System.Drawing.Graphics) null;
    }
