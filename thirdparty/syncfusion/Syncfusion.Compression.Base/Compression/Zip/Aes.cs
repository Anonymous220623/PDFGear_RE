...
// Type: Syncfusion.Compression.Zip.Aes
...
internal class Aes
{
  ...
  private void SetKey(Aes.KeySize keySize, byte[] keyBytes)
  {
  ...
  private byte[] Cipher(byte[] input, byte[] output)
  {
    ...
    for (int index = 0; index < 4 * this.Nb; ++index)
      output[index] = this.State[index % 4, index / 4];
    return output;
  }
  ...
  private static byte gfmultby01(byte value) => value;

  private static byte gfmultby02(byte value)
  {
    return value < (byte) 128 /*0x80*/ ? (byte) ((uint) value << 1) : (byte) ((int) value << 1 ^ 27);
  }

  private static byte gfmultby03(byte value)
  {
    return (byte) ((uint) Aes.gfmultby02(value) ^ (uint) value);
  }

  private static byte gfmultby09(byte value)
  {
    return (byte) ((uint) Aes.gfmultby02(Aes.gfmultby02(Aes.gfmultby02(value))) ^ (uint) value);
  }
  ...
  {
    return (byte) ((uint) Aes.gfmultby02(Aes.gfmultby02(Aes.gfmultby02(value))) ^ (uint) Aes.gfmultby02(Aes.gfmultby02(value)) ^ (uint) value);
  }

  private static byte gfmultby0e(byte value)
  {
    return (byte) ((uint) Aes.gfmultby02(Aes.gfmultby02(Aes.gfmultby02(value))) ^ (uint) Aes.gfmultby02(Aes.gfmultby02(value)) ^ (uint) Aes.gfmultby02(value));
  }
  ...
  private void Dump()
  {
    Console.WriteLine($"Nb = {(object) this.Nb} Nk = {(object) this.Nk} Nr = {(object) this.Nr}");
    Console.WriteLine("\nThe key is \n" + this.DumpKey());
  ...
  {
    if (this.m_salt == null)
      this.m_salt = ZipArchiveItem.CreateRandom(this.m_saltBytes);
    RFC2898Key rfC2898Key = new RFC2898Key(this.m_password, this.m_salt, 1000);
    this.m_encryptionKey = rfC2898Key.GetBytes(this.m_keyBytes);
    this.m_macKey = rfC2898Key.GetBytes(this.m_keyBytes);
    this.m_passVer = rfC2898Key.GetBytes(2);
  }

  private void GenerateMAC(byte[] data)
  {
    RFC2898Key rfC2898Key = new RFC2898Key(this.m_macKey, this.m_salt, 1000);
    rfC2898Key.ComputeHash(data);
    this.m_compMac = new byte[10];
    Array.Copy((Array) rfC2898Key.Hash, (Array) this.m_compMac, 10);
  }
  ...
  private bool Compare(byte[] input1, byte[] input2)
  {
    ...
    {
      if ((int) input1[index] != (int) input2[index])
        return false;
    }
    return true;
  }
  ...
  private void SetKeyBytes(EncryptionAlgorithm type)
  {
    switch (type)
    {
      case EncryptionAlgorithm.AES128:
        this.m_keyBytes = 16 /*0x10*/;
        this.m_saltBytes = 8;
        this.mKeySize = Aes.KeySize.Bits128;
        break;
      case EncryptionAlgorithm.AES192:
        this.m_keyBytes = 24;
        this.m_saltBytes = 12;
        this.mKeySize = Aes.KeySize.Bits192;
        break;
      case EncryptionAlgorithm.AES256:
        this.m_keyBytes = 32 /*0x20*/;
        this.m_saltBytes = 16 /*0x10*/;
        this.mKeySize = Aes.KeySize.Bits256;
        break;
    }
  }
  ...
  {
    this.m_salt = new byte[this.m_saltBytes];
    Buffer.BlockCopy((Array) cipherData, 0, (Array) this.m_salt, 0, this.m_saltBytes);
    this.GenerateCryptographicKey();
    this.SetKey(this.mKeySize, this.m_encryptionKey);
    byte[] numArray1 = new byte[2];
    Buffer.BlockCopy((Array) cipherData, this.m_saltBytes, (Array) numArray1, 0, 2);
    if (!this.Compare(numArray1, this.m_passVer))
      throw new Exception("Incorrect Password");
    this.m_storedmac = new byte[10];
    Buffer.BlockCopy((Array) cipherData, cipherData.Length - 10, (Array) this.m_storedmac, 0, 10);
    byte[] numArray2 = new byte[cipherData.Length - this.m_saltBytes - 2 - 10];
    Buffer.BlockCopy((Array) cipherData, this.m_saltBytes + 2, (Array) numArray2, 0, numArray2.Length);
    byte[] numArray3 = this.AesEncryptDecrypt(numArray2, this.m_encryptionKey);
    this.GenerateMAC(numArray2);
    if (!this.Compare(this.m_compMac, this.m_storedmac))
  ...
  internal byte[] Encrypt(byte[] plainData)
  {
    this.GenerateCryptographicKey();
    this.SetKey(this.mKeySize, this.m_encryptionKey);
    byte[] data = this.AesEncryptDecrypt(plainData, this.m_encryptionKey);
    this.GenerateMAC(data);
    byte[] dst = new byte[data.Length + this.m_saltBytes + 2 + 10];
    this.m_salt.CopyTo((Array) dst, 0);
    this.m_passVer.CopyTo((Array) dst, this.m_saltBytes);
    data.CopyTo((Array) dst, this.m_saltBytes + 2);
    Buffer.BlockCopy((Array) this.m_compMac, 0, (Array) dst, dst.Length - 10, 10);
    return dst;
  }
  ...
  private byte[] AesEncryptDecrypt(byte[] input, byte[] aesKey)
  {
    ...
    for (; index < input.Length; ++index)
    {
      if (num2 == num1)
      {
        ...
        Buffer.BlockCopy((Array) BitConverter.GetBytes(num3), 0, (Array) numArray2, 0, BitConverter.GetBytes(num3).Length);
        output = this.Cipher(numArray2, output);
        num2 = 0;
      }
      numArray1[index] = (byte) ((int) input[index] ^ (int) output[num2++]);
    }
    return numArray1;
  }
