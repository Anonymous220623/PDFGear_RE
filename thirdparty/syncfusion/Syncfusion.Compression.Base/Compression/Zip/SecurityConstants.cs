// Decompiled with JetBrains decompiler
// Type: Syncfusion.Compression.Zip.SecurityConstants
// Assembly: Syncfusion.Compression.Base, Version=19.3460.0.57, Culture=neutral, PublicKeyToken=3d67ed1f87d44c89
// MVID: A9A7FF4E-A031-4867-8B1F-3311AA2A62FF
// Assembly location: C:\Program Files\PDFgear\Syncfusion.Compression.Base.dll

#nullable disable
namespace Syncfusion.Compression.Zip;

internal sealed class SecurityConstants
{
  internal const int AesPassVerLength = 2;
  internal const int AesMacLength = 10;
  internal const int ZipCryptoPassLength = 12;
  internal const int ZipCryptoPassVerPos = 11;
  internal const int PKZipEncryptionHeader = 23;
  internal const CompressionMethod AES = CompressionMethod.PPMd | CompressionMethod.Shrunk;
  internal const int Rfc2898BlockSize = 20;
  internal const int PassIterations = 1000;
  internal const int AesBlockSize = 16 /*0x10*/;
  internal static readonly byte[] AesEncryptionHeader = new byte[8]
  {
    (byte) 1,
    (byte) 153,
    (byte) 7,
    (byte) 0,
    (byte) 2,
    (byte) 0,
    (byte) 65,
    (byte) 69
  };
}
