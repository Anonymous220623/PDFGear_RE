...
// Type: Syncfusion.OfficeChartToImageConverter.ChartCommon
...
internal class ChartCommon
{
  ...
  internal void SetChartSize(IOfficeChart _chart)
  {
    ...
    {
      this.ChartWidth = (int) (_chart.Width * 96.0 / 72.0);
      this.ChartHeight = (int) (_chart.Height * 96.0 / 72.0);
    }
    else
    {
    ...
    bool condition)
  {
    ...
    if (!oppositeAxis.IsAutoCross)
    {
      ...
      if (serieValAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_Low && serieValAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_High)
        sfaxis.ShowAxisNextToOrigin = true;
      else
        sfaxis.ShowOrigin = true;
      if (serieValAxis.ParentChart.ChartType == OfficeChartType.Combination_Chart && serieValAxis.IsAutoCross && serieValAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_NextToAxis && oppositeAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_None)
      {
        sfaxis.ShowOrigin = true;
    ...
    {
      sfaxis.Origin = 0.0;
      if ((categoryAxisImpl != null ? (categoryAxisImpl.IsChartBubbleOrScatter ? 1 : 0) : (oppositeAxis.AxisType != OfficeAxisType.Value || !(serieValAxis is ChartCategoryAxisImpl) ? 0 : ((serieValAxis as ChartCategoryAxisImpl).IsChartBubbleOrScatter ? 1 : 0))) != 0 && serieValAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_Low)
        sfaxis.ShowAxisNextToOrigin = true;
    }
    ...
    if (condition && ((serieValAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_High || oppositeAxis.ReversePlotOrder) && (serieValAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_High || !oppositeAxis.ReversePlotOrder) || oppositeAxis.IsMaxCross))
    {
      if (!oppositeAxis.IsMaxCross && oppositeAxis.IsAutoCross && !this.IsChart3D && serieValAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_High)
  ...
  private string GetSourceNumberFormat(ChartValueAxisImpl axis)
  {
    ...
    IEnumerable<IOfficeChartSerie> source = axis.ParentChart.Series.Where<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (item => !item.IsFiltered && item.UsePrimaryAxis == axis.IsPrimary));
    if (source != null && source.Count<IOfficeChartSerie>() > 0)
    {
      ChartSerieImpl firstSerie = source.First<IOfficeChartSerie>() as ChartSerieImpl;
      if (axis is ChartCategoryAxisImpl)
      {
        IRange categoryLabelsIrange = firstSerie.CategoryLabelsIRange;
        if (this.CheckIfValidValueRange(categoryLabelsIrange))
          sourceNumberFormat = this.TryAndGetFirstCellNumberFormat(categoryLabelsIrange);
        else if (firstSerie.CategoriesFormatCode != null && firstSerie.CategoriesFormatCode != "General")
        {
          string categoriesFormatCode = source.All<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (x => (x as ChartSerieImpl).CategoriesFormatCode == firstSerie.CategoriesFormatCode)) ? firstSerie.CategoriesFormatCode : (string) null;
      ...
      else
      {
        ...
        if (this.CheckIfValidValueRange(valuesIrange))
          sourceNumberFormat = this.TryAndGetFirstCellNumberFormat(valuesIrange);
        else if (firstSerie.FormatCode != null && firstSerie.FormatCode != "General")
        {
          string formatCode = source.All<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (x => (x as ChartSerieImpl).FormatCode == firstSerie.FormatCode)) ? firstSerie.FormatCode : (string) null;
          if (formatCode != null)
...
label_9:
  ...
  protected bool IsRowOrColumnIsHidden(IRange valueCell, WorksheetImpl workSheet) => false;
  ...
  protected double GetDisplayUnitValue(ChartValueAxisImpl valueAxis)
  {
    ...
    {
      case OfficeChartDisplayUnit.None:
        return 1.0;
      case OfficeChartDisplayUnit.Hundreds:
        return 100.0;
      case OfficeChartDisplayUnit.Thousands:
        return 1000.0;
      case OfficeChartDisplayUnit.TenThousands:
        return 10000.0;
      case OfficeChartDisplayUnit.HundredThousands:
        return 100000.0;
      case OfficeChartDisplayUnit.Millions:
        return 1000000.0;
      case OfficeChartDisplayUnit.TenMillions:
        return 10000000.0;
      case OfficeChartDisplayUnit.HundredMillions:
        return 100000000.0;
      case OfficeChartDisplayUnit.ThousandMillions:
        return 1000000000.0;
      case OfficeChartDisplayUnit.MillionMillions:
  ...
  private DataTemplate GetLabelTemplate(ChartAxisImpl chartAxisImpl, bool isLabel)
  {
    ...
    if (chartAxisImpl.Font.Underline == OfficeUnderline.Single)
    ...
    if (!(chartAxisImpl.FrameFormat as ChartFrameFormatImpl).IsAutomaticFormat && chartAxisImpl.FrameFormat.Interior.Pattern != OfficePattern.None)
      frameworkElementFactory.SetValue(TextBlock.BackgroundProperty, (object) this.GetBrushFromDataFormat((IOfficeChartFillBorder) chartAxisImpl.FrameFormat));
    ...
    string seriesType)
  {
    ...
    if (!frameFormat.IsAutomaticFormat && frameFormat.Interior.Pattern != OfficePattern.None)
    {
      fillBrush = this.GetBrushFromDataFormat((IOfficeChartFillBorder) frameFormat);
    ...
    bool condition)
  {
    ...
    if (!xlsioValAxis.IsAutoCross)
    {
      if (this.IsStacked100AxisFormat((ChartAxisImpl) xlsioCatAxis) && xlsioValAxis.CrossesAt != 0.0)
      {
        ...
        if ((xlsioValAxis.CrossesAt < num2 || xlsioValAxis.CrossesAt == num2) && (xlsioValAxis.CrossesAt > num1 || xlsioValAxis.CrossesAt == num1))
        {
          ...
          if (xlsioCatAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_Low)
    ...
    if (this.IsBarChartAxis((ChartAxisImpl) xlsioValAxis) && ((xlsioCatAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_High || xlsioValAxis.ReversePlotOrder) && (xlsioCatAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_High || !xlsioValAxis.ReversePlotOrder) || xlsioValAxis.IsMaxCross))
    {
      sfCatAxis.OpposedPosition = true;
      sfCatAxis.ShowAxisNextToOrigin = false;
    }
    if (condition && ((xlsioCatAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_High || xlsioValAxis.ReversePlotOrder) && (xlsioCatAxis.TickLabelPosition != OfficeTickLabelPosition.TickLabelPosition_High || !xlsioValAxis.ReversePlotOrder) || xlsioValAxis.IsMaxCross))
    {
      if (!xlsioValAxis.IsMaxCross && xlsioValAxis.IsAutoCross && !this.IsChart3D && xlsioCatAxis.TickLabelPosition == OfficeTickLabelPosition.TickLabelPosition_High)
    ...
    else
    {
      ...
      AxisLabelConverter axisLabelConverter = new AxisLabelConverter();
      if (sfCatAxis is CustomDateTimeAxis || sfCatAxis is DateTimeAxis3D)
      {
  ...
  private void SfTickLines(ChartAxisImpl xlsioAxis, ChartAxis sfAxis)
  {
    ...
    {
      case OfficeTickMark.TickMark_None:
        sfAxis.MinorTickLineStyle = new Style()
        {
          TargetType = typeof (Line)
        };
        break;
      case OfficeTickMark.TickMark_Inside:
        sfAxis.TickLinesPosition = AxisElementPosition.Inside;
        sfAxis.MinorTickLineStyle = sfAxis.AxisLineStyle;
        break;
      case OfficeTickMark.TickMark_Outside:
      case OfficeTickMark.TickMark_Cross:
    ...
    out bool isVertical)
  {
    ...
    else
    {
      ...
      textBlock.Text = axis.Title != null ? axis.Title : "Axis Title";
    }
    ...
    out ChartLegend emptyLegend)
  {
    ...
    {
      this.SetLegendPosition((IOfficeChart) xlsioChart, chartLegend1);
      if (xlsioChart.Legend.IsVerticalLegend)
        chartLegend1.Orientation = ChartOrientation.Vertical;
    }
    if (!(xlsioChart.Legend.FrameFormat as ChartFrameFormatImpl).IsAutomaticFormat && xlsioChart.Legend.FrameFormat.Interior.Pattern != OfficePattern.None)
      chartLegend1.Background = this.GetBrushFromDataFormat((IOfficeChartFillBorder) xlsioChart.Legend.FrameFormat);
    double num1 = !(xlsioChart.Legend as ChartLegendImpl).IsDefaultTextSettings || xlsioChart.DefaultTextProperty == null ? xlsioChart.Legend.TextArea.Size : xlsioChart.Font.Size;
    ...
    double num2 = ApplicationImpl.ConvertUnitsStatic(num1, MeasureUnits.Point, MeasureUnits.Pixel);
    chartLegend1.FontSize = num2;
    ...
    if (border.LinePattern != OfficeChartLinePattern.None)
    {
      ...
      if (border.IsAutoLineColor && !border.HasGradientFill && this.parentWorkbook.Version == OfficeVersion.Excel97to2003)
      {
    ...
    {
      chartLegend1.IconWidth = num2 * 2.0;
      chartLegend1.IconHeight = num2;
    }
    else if (!str.Contains("Combination"))
      chartLegend1.IconHeight = chartLegend1.IconWidth = num2 / 2.0;
    ...
    if (this.IsChartEx)
    {
      ...
      chartLegend1.LegendPosition = LegendPosition.Inside;
    sortedLegendOrders = sfchart.VisibleSeries.Count > 1 ? this.GetOrderOfLegendItems(sfchart, (IOfficeChart) xlsioChart, chartLegend1) : (int[]) null;
    ...
    {
      List<IOfficeChartSerie> officeChartSerieList = xlsioChart.Series.OrderByType();
      IEnumerable<IGrouping<int, IOfficeChartSerie>> groupings = xlsioChart.Series.Where<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (x => !x.IsFiltered)).GroupBy<IOfficeChartSerie, int, IOfficeChartSerie>((Func<IOfficeChartSerie, int>) (x => (x as ChartSerieImpl).ChartGroup), (Func<IOfficeChartSerie, IOfficeChartSerie>) (x => x));
      ...
      foreach (KeyValuePair<int, ChartLegendEntryImpl> keyValuePair in hashEntries)
      {
        ...
        {
          int num3 = 0;
          foreach (IGrouping<int, IOfficeChartSerie> source in groupings)
          {
            int num4 = source.Count<IOfficeChartSerie>();
            num3 += num4;
            if (keyValuePair.Key < num3)
            {
              int index1 = keyValuePair.Key - (num3 - num4);
              IOfficeChartSerie officeChartSerie = source.ElementAt<IOfficeChartSerie>(index1);
              if (officeChartSerie != null)
              {
                for (int index2 = 0; index2 < officeChartSerieList.Count; ++index2)
                {
                  if (officeChartSerieList[index2].Name == officeChartSerie.Name)
                  {
                    index1 = index2;
                    break;
                  }
    ...
    else
    {
      ...
      if (chartLegend1.DockPosition != ChartDock.Left && chartLegend1.DockPosition != ChartDock.Right || flag)
      {
        ...
        double num6 = chartLegend1.IconWidth + 10.0 + 2.0 * chartLegend1.BorderThickness.Left;
        totalWidth += (double) totalCount * num6;
        if ((double) this.ChartWidth < totalWidth || flag)
        {
          ...
          frameworkElementFactory.SetValue(FrameworkElement.WidthProperty, (object) (serieWidth + num6));
          ...
          if (flag)
            chartLegend1.ItemsPanel.VisualTree.SetValue(FrameworkElement.WidthProperty, (object) (serieWidth + num6 * num5));
          else
            chartLegend1.ItemsPanel.VisualTree.SetValue(FrameworkElement.WidthProperty, (object) ((serieWidth + num6) * (double) (int) ((double) this.ChartWidth / (serieWidth + num6 * num5))));
        }
      }
  ...
  private int[] GetOrderOfLegendItems(ChartBase sfchart, IOfficeChart chart, ChartLegend legend)
  {
  ...
  internal void SfTextBlock(TextBlock sfTextArea, ChartTextAreaImpl textArea)
  {
    ...
    if (textArea.Underline == OfficeUnderline.Single)
    ...
    ChartAxisImpl axis)
  {
    ...
    if (!(textArea.FrameFormat as ChartFrameFormatImpl).IsAutomaticFormat && textArea.FrameFormat.Interior.Pattern != OfficePattern.None)
      sfTextArea.Background = this.GetBrushFromDataFormat((IOfficeChartFillBorder) textArea.FrameFormat);
    return isVertical;
  }

  internal Border SfChartTitle(IOfficeChart xlsioChart, out RectangleF manualRect)
  {
    ...
    if (!(xlsioChart is ChartImpl))
    {
      ChartImpl chartObject = (xlsioChart as ChartShapeImpl).ChartObject;
    }
    manualRect = new RectangleF(-1f, -1f, 0.0f, 0.0f);
    if (layout.IsManualLayout)
    {
    ...
    else
    {
      ...
      textBlock.Text = xlsioChart.ChartTitle != null ? xlsioChart.ChartTitle : (flag ? (this.Names.ContainsKey((xlsioChart.Series[0] as ChartSerieImpl).Index) ? this.Names[(xlsioChart.Series[0] as ChartSerieImpl).Index] : this.GetSerieName(xlsioChart.Series[0] as ChartSerieImpl)) : "Chart Title");
    }
  ...
  internal void SetTextBlockInlines(ChartTextAreaImpl textArea, TextBlock block)
  {
    ...
    for (int index = 0; index < formattingRuns.Length; ++index)
    {
      ...
      {
        IOfficeFont font = innerFonts[fontIndex];
  ...
  internal void SetFormattingsInRun(Run run, IOfficeFont font)
  {
    if (font.Bold)
      run.FontWeight = FontWeights.Bold;
    if (font.Italic)
      run.FontStyle = FontStyles.Italic;
    if (font.Underline == OfficeUnderline.Single)
  ...
  {
    ChartDataLabelsImpl chartDataLabelsImpl = (ChartDataLabelsImpl) null;
    ...
    if (chartDataLabelsImpl != null && !chartDataLabelsImpl.IsDelete && (chartDataLabelsImpl.IsCategoryName || chartDataLabelsImpl.IsSeriesName || chartDataLabelsImpl.IsValue || chartDataLabelsImpl.IsPercentage || chartDataLabelsImpl.IsValueFromCells) || serie.SerieFormat.IsMarkerSupported && (serie.SerieFormat as ChartSerieDataFormatImpl).MarkerFormat.MarkerType != OfficeChartMarkerType.None || flag2)
    {
      ...
      {
        info.LabelRotationAngle = (double) chartDataLabelsImpl.TextRotationAngle;
        if (chartDataLabelsImpl.IsCategoryName || chartDataLabelsImpl.IsSeriesName || chartDataLabelsImpl.IsValue || chartDataLabelsImpl.IsPercentage || chartDataLabelsImpl.IsValueFromCells)
          info.ShowLabel = true;
        this.SetLabelPosition(sfChartSeries, info, chartDataLabelsImpl);
      }
      ...
      if (chartDataLabelsImpl != null && !chartDataLabelsImpl.IsDelete || flag2)
      {
        if ((chartDataLabelsImpl == null || chartDataLabelsImpl.IsDelete) && flag2)
          chartDataLabelsImpl = serie.DataPoints.DefaultDataPoint.DataLabels as ChartDataLabelsImpl;
        ...
        System.Drawing.Color color = !chartDataLabelsImpl.IsAutoColor || !this.TryAndGetColorBasedOnElement(ChartElementsEnum.ChartAreaFill, serie.ParentChart, out color) || (int) color.R != (int) color.G || (int) color.B != (int) color.R || color.R != (byte) 0 ? chartDataLabelsImpl.RGBColor : System.Drawing.Color.FromArgb(0, (int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue);
        dataTemplateInfo.SetValues(chartDataLabelsImpl.FontName, chartDataLabelsImpl.Size, chartDataLabelsImpl.Bold, chartDataLabelsImpl.Italic, chartDataLabelsImpl.Underline != OfficeUnderline.None, chartDataLabelsImpl.Strikethrough, this.SfColor(color));
        System.Windows.Media.Brush fillBrush;
        info.LabelTemplate = this.SfDataTemplate(dataTemplateInfo, chartDataLabelsImpl.FrameFormat as ChartFrameFormatImpl, out fillBrush, out borderBrush, out borderThickness, sfChartSeries.GetType().ToString());
        ...
        DataLabelSetting dataLabelSetting1 = new DataLabelSetting(chartDataLabelsImpl, isCircularSeries);
        ...
        bool flag3 = sfChartSeries is CandleSeries || sfChartSeries is HiLoSeries;
        bool flag4 = itemsSource.Count <= 0 || (flag3 ? !(itemsSource[0].Size is string) : !(itemsSource[0].Close is string));
        for (int index = 0; index < itemsSource.Count; ++index)
          labelConvertor.CategoryNames[index] = !flag4 ? (!flag3 ? itemsSource[index].Close : itemsSource[index].Size) : itemsSource[index].X;
        if (isCircularSeries)
        {
        ...
        if (chartDataLabelsImpl.IsSourceLinked && serie.ValuesIRange != null && !(serie.ValuesIRange is ExternalRange) && !(serie.ValuesIRange is NameImpl))
        {
          dataLabelSetting1.NumberFormat = serie.ValuesIRange.Cells[0].NumberFormat;
          dataLabelSetting1.IsSourceLinked = true;
        }
        else if (chartDataLabelsImpl.IsSourceLinked && serie.ValuesIRange != null && serie.ValuesIRange is NameImpl && (serie.ValuesIRange as NameImpl).AddressLocal != null)
        {
          dataLabelSetting1.NumberFormat = serie.ValuesIRange.Cells[0].NumberFormat;
          dataLabelSetting1.IsSourceLinked = true;
        }
        else
          dataLabelSetting1.NumberFormat = chartDataLabelsImpl.NumberFormat == null || !(chartDataLabelsImpl.NumberFormat.ToLower() != "General".ToLower()) ? "General" : chartDataLabelsImpl.NumberFormat;
        ColorConverter colorConverter1 = this.GetNumberFormatColorConverter(dataLabelSetting1.NumberFormat, chartDataLabelsImpl.RGBColor);
        if (dictionary.Count > 0)
        {
          FontConverter fontConverter1 = new FontConverter();
          fontConverter1.Names = new Dictionary<int, string>(dictionary.Count);
          fontConverter1.DefaultName = chartDataLabelsImpl.FontName;
          FontConverter fontConverter2 = new FontConverter();
          fontConverter2.Sizes = new Dictionary<int, double>(dictionary.Count);
          fontConverter2.DefaultSize = chartDataLabelsImpl.Size;
          FontConverter fontConverter3 = new FontConverter();
          fontConverter3.Bolds = new Dictionary<int, bool>(dictionary.Count);
          fontConverter3.DefaultBold = chartDataLabelsImpl.Bold;
          FontConverter fontConverter4 = new FontConverter();
          fontConverter4.Italics = new Dictionary<int, bool>(dictionary.Count);
          fontConverter4.DefaultItalic = chartDataLabelsImpl.Italic;
          FontConverter fontConverter5 = new FontConverter();
          fontConverter5.Underlines = new Dictionary<int, bool>(dictionary.Count);
          fontConverter5.DefaultUnderline = chartDataLabelsImpl.Underline != OfficeUnderline.None;
          FontConverter fontConverter6 = new FontConverter();
          fontConverter6.Strikethroughs = new Dictionary<int, bool>(dictionary.Count);
          fontConverter6.DefaultStrikethrough = chartDataLabelsImpl.Strikethrough;
          ...
          ChartFrameFormatImpl frameFormat1 = chartDataLabelsImpl.FrameFormat as ChartFrameFormatImpl;
          ...
          if (!frameFormat1.IsAutomaticFormat && frameFormat1.Interior.Pattern != OfficePattern.None)
            colorConverter3.DefaultBrush = this.GetBrushFromDataFormat((IOfficeChartFillBorder) frameFormat1);
          ...
          colorConverter1.DefaultBrush = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(chartDataLabelsImpl.RGBColor));
          foreach (KeyValuePair<int, ChartDataPointImpl> keyValuePair in dictionary)
          {
            if (keyValuePair.Key < itemsSource.Count)
            {
              ...
              {
                colorConverter3.LabelBrushes.Add(keyValuePair.Value.Index, frameFormat2.Interior.Pattern != OfficePattern.None ? this.GetBrushFromDataFormat((IOfficeChartFillBorder) frameFormat2) : (System.Windows.Media.Brush) null);
              ...
              if (dataLabels.ShowTextProperties)
              {
                ...
                fontConverter5.Underlines.Add(keyValuePair.Value.Index, dataLabels.Underline != OfficeUnderline.None);
              }
              ...
              dataLabelSetting2.NumberFormat = !dataLabelSetting2.IsSourceLinked || serie.ValuesIRange == null || serie.ValuesIRange is ExternalRange || dataLabels.NumberFormat == null ? dataLabelSetting1.NumberFormat : serie.ValuesIRange.Cells[keyValuePair.Value.Index].NumberFormat;
  ...
  {
    IEnumerable<\u003C\u003Ef__AnonymousType1<int, ChartSerieDataFormatImpl>> source = (chartSerieImpl.DataPoints as ChartDataPointsCollection).m_hashDataPoints.Select(point => new
    {
      point = point,
    ...
    if (dataFormat.MarkerFormatOrNull != null && !dataFormat.IsAutoMarker && (dataFormat.MarkerFormat.MarkerType == OfficeChartMarkerType.None ? (source.Count() == 0 ? 1 : 0) : 0) != 0)
      return;
    ...
    if (dataFormat.MarkerFormat.MarkerType != OfficeChartMarkerType.None && markerSettings1.FillBrush != null && (markerSettings1.FillBrush as SolidColorBrush).Color.ToString() == "#00FFFFFF" && (chartSerieImpl.SerieType == OfficeChartType.Scatter_Markers || markerSettings1.BorderBrush != null && (markerSettings1.BorderBrush as SolidColorBrush).Color.ToString() == "#FF000000"))
    ...
    bool isvaryColor)
  {
    ...
    bool isBinary = this.parentWorkbook.Version == OfficeVersion.Excel97to2003;
    ...
    markerSettings.BorderThickness = 0.75;
    return markerSettings;
  }
    ...
    ChartDataLabelsImpl label)
  {
    ...
    else
    {
      ...
      info.HorizontalAlignment = HorizontalAlignment.Center;
      if (label.Position != OfficeDataLabelPosition.Automatic && flag)
      {
        switch (label.Position)
        {
          case OfficeDataLabelPosition.Outside:
            ...
            break;
          case OfficeDataLabelPosition.Inside:
          case OfficeDataLabelPosition.BestFit:
            if (circularSeriesBase != null)
            {
              circularSeriesBase.EnableSmartLabels = false;
            ...
            break;
          case OfficeDataLabelPosition.OutsideBase:
            if (circularSeriesBase != null)
              circularSeriesBase.EnableSmartLabels = false;
            else if (circularSeriesBase3D != null)
              circularSeriesBase3D.EnableSmartLabels = false;
            ...
            break;
          case OfficeDataLabelPosition.Above:
  ...
  internal void SfPloatArea(SfChartExt sfChart, ChartFrameFormatImpl chartArea, ChartImpl chart)
  {
    ...
    {
      sfChart.AreaBackground = this.GetBrushFromDataFormat((IOfficeChartFillBorder) chartArea);
    }
    else
    {
      ...
      if (this.TryAndGetColorBasedOnElement(ChartElementsEnum.PlotAreaFill, chart, out color))
        sfChart.AreaBackground = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(color, chart.Style < 33 || chart.Style > 100 && chart.Style < 133 ? transparency : 0.0));
      else
    ...
    if (border.LinePattern != OfficeChartLinePattern.None)
    {
      ...
      else if (this.parentWorkbook.Version == OfficeVersion.Excel97to2003)
  ...
  internal void SfChartArea(SfChartExt sfChart, ChartFrameFormatImpl chartArea, ChartImpl chart)
  {
    System.Drawing.Color color;
    if (!chartArea.IsAutomaticFormat)
      sfChart.Background = this.GetBrushFromDataFormat((IOfficeChartFillBorder) chartArea);
    else if (this.TryAndGetColorBasedOnElement(ChartElementsEnum.ChartAreaFill, chart, out color))
    {
      double num = 1.0;
      sfChart.Background = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(color, chart.Style < 41 || chart.Style > 100 && chart.Style < 141 ? num : 0.0));
    }
    else
      sfChart.Background = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(byte.MaxValue, byte.MaxValue, byte.MaxValue, 1.0));
    ChartBorderImpl border = chartArea.Border as ChartBorderImpl;
    if (border.LinePattern != OfficeChartLinePattern.None)
    {
      double uniformLength = 0.0;
      ...
      {
        if (this.TryAndGetColorBasedOnElement(ChartElementsEnum.ChartAreaLine, chart, out color))
    ...
    ChartImpl chart)
  {
    ...
    System.Drawing.Color color;
    if (!chartArea.IsAutomaticFormat)
      brush = this.GetBrushFromDataFormat((IOfficeChartFillBorder) chartArea);
    else if (this.TryAndGetColorBasedOnElement(ChartElementsEnum.ChartAreaFill, chart, out color))
    {
      double num = 1.0;
      sfChart.Background = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(color, chart.Style < 41 || chart.Style > 100 && chart.Style < 141 ? num : 0.0));
  ...
  internal void SfChartTrendLine(ChartSerieImpl serieImpl, CartesianSeries sfSerie)
  {
    ...
    for (int iIndex = 0; iIndex < serieImpl.TrendLines.Count; ++iIndex)
    {
      ...
      if (trendLine.Type == OfficeTrendLineType.Moving_Average)
        trendline.Type = TrendlineType.Linear;
      else
        trendline.Type = (TrendlineType) Enum.Parse(typeof (TrendlineType), trendLine.Type.ToString());
      if (trendLine.LegendEntry != null && trendLine.LegendEntry.IsDeleted)
        trendline.VisibilityOnLegend = Visibility.Collapsed;
      trendline.Label = trendLine.Name;
      if (trendLine.Type == OfficeTrendLineType.Polynomial && trendLine.Order >= 2 && trendLine.Order <= 6)
        trendline.PolynomialOrder = trendLine.Order;
      else if (trendLine.Type != OfficeTrendLineType.Polynomial && trendLine.Order < 3)
        trendline.PolynomialOrder = 3;
      else if (trendLine.Type != OfficeTrendLineType.Polynomial && trendLine.Order >= 3 && trendLine.Order <= 6)
        trendline.PolynomialOrder = trendLine.Order;
      ...
      if (this.GetStrokeDashArrayValues((short) trendLine.Border.LinePattern, out patternValues))
        trendline.StrokeDashArray = patternValues;
      trendline.LegendIcon = ChartLegendIcon.StraightLine;
      trendline.ForwardForecast = trendLine.Forward;
      trendline.BackwardForecast = trendLine.Backward;
      ...
      if (border.AutoFormat && this.TryAndGetColorBasedOnElement(ChartElementsEnum.OtherLines, serieImpl.ParentChart, out color))
        trendline.Stroke = (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(color));
      else
        trendline.Stroke = this.GetBrushFromBorder(border);
      if (border.AutoFormat)
      {
        trendline.StrokeThickness = 1.0;
      }
      else
      {
        trendline.StrokeThickness = this.GetBorderThickness(border);
        if (trendline.StrokeThickness != 0.0 && trendline.StrokeThickness < 1.0)
          trendline.StrokeThickness = 1.0;
      }
    ...
    ChartSeriesBase chartSerie)
  {
    ...
    if (chartSerie is RadarSeries)
    {
      ...
      this.SfSecondaryAxisCommon((IOfficeChart) xlsioChart, radarSeries1.XAxis, radarSeries1.YAxis);
    }
    else
    {
      ...
      this.SfSecondaryAxisCommon((IOfficeChart) xlsioChart, cartesianSeries1.XAxis, cartesianSeries1.YAxis);
    }
  }

  protected void SfSecondaryAxisCommon(
    IOfficeChart xlsioChart,
  ...
  {
    return serie.ParentChart.SeriesNameLevel == OfficeSeriesNameLevel.SeriesNameLevelAll ? serie.Name : "Series" + (serie.Index + 1).ToString();
  }
  ...
  internal void SetGapWidthandOverlap(ChartSeriesBase seriesBase, ChartSerieImpl serie)
  {
    ...
    if (!this.IsChart3D)
    {
      if (!this.SetGapWidthOnSingleSeries(seriesBase, serie, out overlap, false))
      {
        ...
        if (overlap > 0.0)
          num = num - overlap == 1.0 ? 0.9 : (num - overlap > 0.0 ? num - overlap : num);
    ...
    bool isChart3D)
  {
    ...
    bool flag1 = serie.ParentChart.IsStacked || serie.ParentChart.ChartType == OfficeChartType.Combination_Chart && serieType.ToString().Contains("Stacked");
    bool flag2 = serie.ParentChart.IsClustered && serie.ParentChart.Series.Count == 1 || !isChart3D && serie.ParentSeries.Count > 1 && num1 == 100.0 && serie.SerieFormat.CommonSerieOptions.GapWidth == 0 || serie.ParentChart.ChartType == OfficeChartType.Combination_Chart && serie.ParentSeries.Count<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (x => x.SerieType == serieType && x.UsePrimaryAxis == serie.UsePrimaryAxis)) == 1;
    ...
    if (gapWidth == 0.0)
    {
      ...
      {
        int num2 = serie.ParentSeries.Count<IOfficeChartSerie>((Func<IOfficeChartSerie, bool>) (x => x.SerieType == serieType));
  ...
  internal System.Windows.Media.Brush GetBrushFromDataFormat(IOfficeChartFillBorder fillFormat)
  {
    ...
    bool isAxisLine)
  {
    ...
    if (isAxisLine)
    {
      if (border.LinePattern != OfficeChartLinePattern.None)
      {
        ...
        {
          System.Drawing.Color color;
          brush = this.parentWorkbook.Version != OfficeVersion.Excel97to2003 ? (!this.TryAndGetColorBasedOnElement(elementEnum, chart, out color) ? (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor((byte) 134, (byte) 134, (byte) 134)) : (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor(color))) : (System.Windows.Media.Brush) new SolidColorBrush(this.SfColor((byte) 0, (byte) 0, (byte) 0));
          num = 1.0;
        }
        else
    ...
    bool isRadar)
  {
    ...
    if (axisImpl.Visible && flag && axisImpl.HasDisplayUnitLabel && axisImpl.DisplayUnit != OfficeChartDisplayUnit.None && (!flag || !this.IsChartEx || !(axisImpl.DisplayUnit == OfficeChartDisplayUnit.Custom & axisImpl.ParentChart.ChartType.ToString().Contains("Pareto")) || axisImpl.DisplayUnitLabel.Text != null))
  ...
  private string GetTextFromDisplayUnits(ChartValueAxisImpl axis)
  {
    ...
    if (axis.DisplayUnitLabel.Text == null)
    {
      ...
      fromDisplayUnits = (axis.DisplayUnitLabel as ChartTextAreaImpl).Text;
    ...
    out System.Drawing.Color color)
  {
    ...
    switch (inputelement)
    {
      ...
      case ChartElementsEnum.UpBarFill:
        ...
        int num1 = style;
        int color_Index1 = 4 + (num1 >= 9 ? (num1 >= 17 ? (num1 >= 25 ? (num1 >= 33 ? num1 - 43 : num1 - 27) : num1 - 19) : num1 - 11) : num1 - 3);
        color = chart.GetChartThemeColorByColorIndex(color_Index1);
        color = this.GetPercentageTintOrShadeOfColor(color, 0.25, mixColor1);
        break;
      ...
      case ChartElementsEnum.DownBarLine:
        ...
        int color_Index2 = 4 + (style - 35);
        color = chart.GetChartThemeColorByColorIndex(color_Index2);
      ...
      case ChartElementsEnum.DownBarFill:
        switch (style)
        {
          ...
          default:
            int num2 = style;
            int color_Index3 = 4 + (num2 >= 9 ? (num2 >= 17 ? (num2 >= 25 ? (num2 >= 33 ? (num2 >= 41 ? num2 - 43 : num2 - 35) : num2 - 27) : num2 - 19) : num2 - 11) : num2 - 3);
            color = chart.GetChartThemeColorByColorIndex(color_Index3);
            color = this.GetPercentageTintOrShadeOfColor(color, 0.5, mixColor2);
            break;
        }
        break;
      ...
      case ChartElementsEnum.PlotAreaFill:
        ...
        {
          int color_Index4 = 4 + (style - 35);
          color = chart.GetChartThemeColorByColorIndex(color_Index4);
          color = this.GetPercentageTintOrShadeOfColor(color, 0.2, mixColor1);
          break;
        }
        color = System.Drawing.Color.FromArgb(0, 70, 70, 70);
    ...
    out System.Drawing.Color color)
  {
    ...
    if ((isLine ? (style != 34 ? 1 : 0) : (Array.IndexOf<int>(array, style) == -1 ? 1 : 0)) != 0)
    {
      ...
      {
        int num1 = style;
        int color_Index = 4 + (num1 >= 9 ? (num1 >= 17 ? (num1 >= 25 ? (num1 >= 33 ? (num1 >= 41 ? num1 - 43 : num1 - 35) : num1 - 27) : num1 - 19) : num1 - 11) : num1 - 3);
        color = chart.GetChartThemeColorByColorIndex(color_Index);
        double num2 = 0.0;
