...
// Type: Syncfusion.Presentation.Drawing.Storage
...
internal class Storage
{
    ...
    foreach (string key1 in oleObjectCollection.Keys)
    {
      if (oleObjectCollection[key1].Streams.Count == this.Streams.Count)
      {
        foreach (string key2 in this.Streams.Keys)
        {
          ...
          {
            flag = this.CompareArray(byteArray1, byteArray2);
          }
          else
          {
            flag = false;
  ...
  internal void UpdateGuid(Syncfusion.CompoundFile.Presentation.Net.CompoundFile cmpFile, int storageIndex, string storageName)
  {
