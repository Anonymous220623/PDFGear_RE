...
// Type: Syncfusion.Presentation.Net.StreamHelper
...
#nullable disable
namespace Syncfusion.Presentation.Net;
...
internal static class StreamHelper
{
  ...
  public static string GetAsciiString(Stream stream, int roundedSize)
  {
    ...
    return StreamHelper.RemoveLastZero(Encoding.Default.GetString(numArray, 0, roundedSize));
  }
  ...
  public static string GetUnicodeString(Stream stream, int roundedSize)
  {
    ...
    if (stream.Read(numArray, 0, count) != count)
      throw new IOException();
    string unicodeString = StreamHelper.RemoveLastZero(Encoding.Unicode.GetString(numArray, 0, count));
    int num = count % 4;
    if (num != 0)
