...
// Type: Syncfusion.CompoundFile.Presentation.Net.CompoundFile
...
internal class CompoundFile : ICompoundFile, IDisposable
{
  ...
  {
    Syncfusion.CompoundFile.Presentation.Net.CompoundFile compoundFile = new Syncfusion.CompoundFile.Presentation.Net.CompoundFile();
    ...
    using (FileStream fileStream = new FileStream("test.bin", FileMode.Open, FileAccess.Read, FileShare.Read))
    {
      ...
      Syncfusion.CompoundFile.Presentation.Net.CompoundFile.WriteDirectory(path, compoundFile.Directory);
      Syncfusion.CompoundFile.Presentation.Net.CompoundFile.WriteStorage(path, root);
    }
  }
  ...
  private static void WriteStorage(string path, ICompoundStorage storage)
  {
    foreach (string stream in storage.Streams)
      Syncfusion.CompoundFile.Presentation.Net.CompoundFile.WriteStream(path, stream, storage);
    foreach (string storage1 in storage.Storages)
    {
      ...
      using (ICompoundStorage storage2 = storage.OpenStorage(storage1))
        Syncfusion.CompoundFile.Presentation.Net.CompoundFile.WriteStorage(path1, storage2);
    }
  }
  ...
  public CompoundFile(string fileName, bool create)
  {
    ...
    {
      this.m_stream = (Stream) new FileStream(fileName, FileMode.Create, FileAccess.ReadWrite, FileShare.None);
      this.InitializeVariables();
    }
  }
  ...
  internal void ReadSector(byte[] buffer, int offset, int sectorIndex, FileHeader header)
  {
    ...
    this.m_stream.Position = Syncfusion.CompoundFile.Presentation.Net.CompoundFile.GetSectorOffset(sectorIndex, sectorShift);
