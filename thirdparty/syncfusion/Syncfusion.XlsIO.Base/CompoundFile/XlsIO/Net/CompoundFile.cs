...
// Type: Syncfusion.CompoundFile.XlsIO.Net.CompoundFile
...
public class CompoundFile : ICompoundFile, IDisposable
{
  ...
  public static void Main()
  {
    ...
    using (FileStream fileStream = new FileStream("test.bin", FileMode.Open, FileAccess.Read, FileShare.Read))
    {
      ...
      Syncfusion.CompoundFile.XlsIO.Net.CompoundFile.WriteDirectory(path, compoundFile.Directory);
      Syncfusion.CompoundFile.XlsIO.Net.CompoundFile.WriteStorage(path, root);
    }
  }
  ...
  private static void WriteStorage(string path, ICompoundStorage storage)
  {
    foreach (string stream in storage.Streams)
      Syncfusion.CompoundFile.XlsIO.Net.CompoundFile.WriteStream(path, stream, storage);
    foreach (string storage1 in storage.Storages)
    {
      ...
      using (ICompoundStorage storage2 = storage.OpenStorage(storage1))
        Syncfusion.CompoundFile.XlsIO.Net.CompoundFile.WriteStorage(path1, storage2);
    }
  }
  ...
  internal void ReadSector(byte[] buffer, int offset, int sectorIndex, FileHeader header)
  {
    ...
    this.m_stream.Position = Syncfusion.CompoundFile.XlsIO.Net.CompoundFile.GetSectorOffset(sectorIndex, sectorShift);
