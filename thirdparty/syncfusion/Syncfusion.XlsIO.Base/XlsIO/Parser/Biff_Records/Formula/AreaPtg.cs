...
// Type: Syncfusion.XlsIO.Parser.Biff_Records.Formula.AreaPtg
...
[CLSCompliant(false)]
public class AreaPtg : Ptg, IRangeGetterToken, IRangeGetter, IToken3D, IRectGetter
{
  ...
  protected internal byte FirstOptions
  {
    get => this.m_firstOptions;
    set => this.m_firstOptions = value;
  }

  protected internal byte LastOptions
  {
  ...
  public override void InfillPTG(DataProvider provider, ref int offset, ExcelVersion version)
  {
    ...
    else
    {
      ...
      offset += 4;
      short num1 = provider.ReadInt16(offset);
      this.m_iFirstColumn = (int) (ushort) ((uint) num1 << 2) >> 2;
      this.m_firstOptions = (byte) ((uint) (ushort) ((uint) num1 >> 14) << 6);
      offset += 2;
      short num2 = provider.ReadInt16(offset);
      this.m_iLastColumn = (int) (ushort) ((uint) num2 << 2) >> 2;
      this.m_lastOptions = (byte) ((uint) (ushort) ((uint) num2 >> 14) << 6);
      offset += 2;
    }
  }
  ...
  public override byte[] ToByteArray(ExcelVersion version)
  {
    ...
    if (version == ExcelVersion.Excel97to2003)
    {
      if (!(this is Area3DPtg) && (this.m_iFirstRow > (int) ushort.MaxValue || this.m_iLastRow > (int) ushort.MaxValue || this.m_iFirstColumn > (int) byte.MaxValue || this.m_iLastColumn > (int) byte.MaxValue))
      {
        FormulaToken formulaToken = !(this is AreaNPtg) ? this.GetCorrespondingErrorCode() : (this as AreaNPtg).GetErrorCode();
        byteArray[0] = (byte) formulaToken;
      }
    ...
    else if (version != ExcelVersion.Excel97to2003)
    {
      ...
      int index7 = index6 + 4;
      BitConverter.GetBytes((short) ((int) (short) ((int) (short) this.m_iFirstColumn | (int) (short) ((XlsbDataHolder.ReadBit(this.m_firstOptions, 6) ? 1 : 0) << 14)) | (int) (short) ((XlsbDataHolder.ReadBit(this.m_firstOptions, 7) ? 1 : 0) << 15))).CopyTo((Array) byteArray, index7);
      int index8 = index7 + 2;
      BitConverter.GetBytes((short) ((int) (short) ((int) (short) this.m_iLastColumn | (int) (short) ((XlsbDataHolder.ReadBit(this.m_lastOptions, 6) ? 1 : 0) << 14)) | (int) (short) ((XlsbDataHolder.ReadBit(this.m_lastOptions, 7) ? 1 : 0) << 15))).CopyTo((Array) byteArray, index8);
      int num = index8 + 2;
      return byteArray;
    }
    ...
    WorkbookImpl book)
  {
    ...
    if (iCurSheetIndex == iSourceSheetIndex && (Ptg.RectangleContains(rectSource, iTokenRow, iTokenColumn) || Ptg.RectangleIntersects(rectSource, this.FirstRow, this.FirstColumn, this.LastRow, this.LastColumn) || Ptg.RectangleIntersects(rectDest, this.FirstRow, this.FirstColumn, this.LastRow, this.LastColumn)))
  ...
  public AreaPtg ConvertFullRowColumnAreaPtgs(bool bFromExcel07To97)
  {
    int iRows1;
    int iColumns1;
    UtilityMethods.GetMaxRowColumnCount(out iRows1, out iColumns1, ExcelVersion.Excel2007);
    int iRows2;
    int iColumns2;
    UtilityMethods.GetMaxRowColumnCount(out iRows2, out iColumns2, ExcelVersion.Excel97to2003);
    if (bFromExcel07To97)
    {
