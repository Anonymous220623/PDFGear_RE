...
// Type: Syncfusion.XlsIO.Parser.Biff_Records.Formula.AreaNPtg
...
public class AreaNPtg : AreaPtg
{
    ...
    bool isForSerialization)
  {
    ...
    int row1 = this.IsFirstRowRelative ? this.GetUpdatedRowIndex(iRow, this.FirstRow, parentWorkbook) : this.FirstRow;
    int row2 = this.IsLastRowRelative ? this.GetUpdatedRowIndex(iRow, this.LastRow, parentWorkbook) : this.LastRow;
    int column1 = this.IsFirstColumnRelative ? this.GetUpdatedColumnIndex(iColumn, (int) this.FirstColumn, parentWorkbook) : (int) this.FirstColumn;
    int column2 = this.IsLastColumnRelative ? this.GetUpdatedColumnIndex(iColumn, (int) this.LastColumn, parentWorkbook) : (int) this.LastColumn;
  ...
  private int GetUpdatedRowIndex(int iRow, int row, WorkbookImpl book)
  {
    if (iRow == 0 && row == 0)
      return 0;
    return book == null ? (row + iRow > 65536 /*0x010000*/ ? iRow - 65536 /*0x010000*/ + row - 1 : row + iRow - 1) : (row + iRow > book.MaxRowCount ? iRow - book.MaxRowCount + row - 1 : row + iRow - 1);
  }
  ...
  private int GetUpdatedColumnIndex(int iColumn, int column, WorkbookImpl book)
  {
    if (iColumn == 0 && column == 0)
      return 0;
    return book == null ? (column + iColumn > 256 /*0x0100*/ ? iColumn - 256 /*0x0100*/ + column - 1 : column + iColumn - 1) : (column + iColumn > book.MaxColumnCount ? iColumn - book.MaxColumnCount + column - 1 : column + iColumn - 1);
  }

  internal FormulaToken GetErrorCode()
  {
    return AreaErrorPtg.IndexToCode(AreaNPtg.CodeToIndex(this.TokenCode));
  }
}
