...
// Type: Syncfusion.XlsIO.Parser.Biff_Records.RKRecord
...
[Biff(TBIFFRecord.RK)]
[CLSCompliant(false)]
public class RKRecord : CellPositionBase, IDoubleValue, IValueHolder
{
  ...
  {
    double num = (value & 2) <= 0 ? (ApplicationImpl.UseUnsafeCodeStatic ? RKRecord.UnsafeGetDouble(value) : RKRecord.SafeGetDouble(value)) : (double) (value >> 2);
    if ((value & 1) > 0)
      num /= 100.0;
    return num;
  }

  private static unsafe double UnsafeGetDouble(int value)
  {
    double num = 0.0;
    int* numPtr = (int*) &num;
    numPtr[1] = (int) ((long) value & 4294967292L);
    *numPtr = 0;
    return num;
  }
