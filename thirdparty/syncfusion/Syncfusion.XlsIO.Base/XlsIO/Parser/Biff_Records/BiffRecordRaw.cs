...
// Type: Syncfusion.XlsIO.Parser.Biff_Records.BiffRecordRaw
...
public abstract class BiffRecordRaw : ICloneable, IBiffStorage
{
  ...
  public virtual byte[] Data
  {
    get
    {
      ...
      this.InfillInternalData((DataProvider) new ByteArrayDataProvider(arrData), 0, ExcelVersion.Excel97to2003);
      return arrData;
    }
    set
    {
      ...
      this.ParseStructure((DataProvider) new ByteArrayDataProvider(value), 0, length, ExcelVersion.Excel97to2003);
    }
  }
  ...
  {
    this.FillRecord(reader, (DataProvider) null, (IDecryptor) null, (byte[]) null);
    ...
    byte[] arrBuffer)
  {
    ...
    try
    {
      ...
      if (num - (long) this.m_iLength < 0L)
        throw new ApplicationException("Unexpected end of records stream. Record data cannot be readed - reached end of stream.");
      provider.Read(reader, 0, this.m_iLength, (byte[]) null);
      if (decryptor != null && this.NeedDecoding)
      {
      ...
      this.ParseStructure(provider, 0, this.m_iLength, ExcelVersion.Excel97to2003);
    ...
    int streamPosition)
  {
    ...
    int iSize = this.m_iLength + 4;
    if (this.m_iLength > 0)
      this.InfillInternalData(provider, 4, ExcelVersion.Excel97to2003);
    byte[] internalBuffer = ((ByteArrayDataProvider) provider).InternalBuffer;
    if (encryptor != null && this.NeedDecoding)
    {
      int startDecodingOffset = this.StartDecodingOffset;
    ...
    provider.WriteInto(writer, 0, iSize, internalBuffer);
    return iSize;
  }
