...
// Type: Syncfusion.XlsIO.Parser.Biff_Records.DVRecord
...
[Biff(TBIFFRecord.DV)]
[CLSCompliant(false)]
public class DVRecord : BiffRecordRawWithArray, ICloneable
{
  ...
  {
    get => this.m_choiceTokens;
    set => this.m_choiceTokens = value;
  }

  internal bool IsFormulaOrChoice
  {
    get => this.m_isFormulaOrChoice;
    set => this.m_isFormulaOrChoice = value;
  }
  ...
  public override void ParseStructure()
  {
    ...
    this.m_arrFirstFormulaTokens = FormulaUtil.ParseExpression((DataProvider) provider, (int) uint16_1, ExcelVersion.Excel97to2003);
    provider.SetBuffer(bytes2);
    this.m_arrSecondFormulaTokens = FormulaUtil.ParseExpression((DataProvider) provider, (int) uint16_2, ExcelVersion.Excel97to2003);
  ...
  public override void InfillInternalData(ExcelVersion version)
  {
    ...
    byte[] src = FormulaUtil.PtgArrayToByteArray(this.m_arrFirstFormulaTokens, version);
    if (this.m_bStrListExplicit && src != null && this.m_isFormulaOrChoice)
    {
      string replace = "2C00";
      string replacewith = "0000";
      src = this.ReplaceBytes(src, replace, replacewith);
    }
    if (this.m_bStrListExplicit && src != null && !this.m_isFormulaOrChoice)
    {
      string replace = BitConverter.ToString(Encoding.Unicode.GetBytes(CultureInfo.CurrentCulture.TextInfo.ListSeparator)).Replace("-", "");
      string replacewith = "0000";
      src = this.ReplaceBytes(src, replace, replacewith);
    }
    ushort length1 = src != null ? (ushort) src.Length : (ushort) 0;
    ...
    if (length1 > (ushort) 0)
    {
      this.SetBytes(offset3, src, 0, (int) length1);
      offset3 += (int) length1;
    }
    byte[] byteArray = FormulaUtil.PtgArrayToByteArray(this.m_arrSecondFormulaTokens, version);
    ushort length2 = byteArray != null ? (ushort) byteArray.Length : (ushort) 0;
    ...
    if (length2 > (ushort) 0)
    {
      this.SetBytes(offset5, byteArray, 0, (int) length2);
      offset5 += (int) length2;
    }
  ...
  {
    string str1 = BitConverter.ToString(src).Replace("-", "").Replace(replace, replacewith);
    string str2 = string.Empty;
    if (this.m_choiceTokens != null && CultureInfo.CurrentCulture.TextInfo.ListSeparator != ",")
    {
      string[] strArray = this.splitString(this.m_choiceTokens);
      for (int index1 = 0; index1 < strArray.Length; ++index1)
      {
        if (index1 > 0)
          str2 += CultureInfo.CurrentCulture.TextInfo.ListSeparator;
        string str3 = string.Empty;
        string str4 = strArray[index1];
        if (str4.StartsWith("\""))
        {
          for (int index2 = 1; index2 < str4.Length - 1; ++index2)
            str3 += (string) (object) str4[index2];
          str3 = str3.Replace("\"\"", "\"");
        }
        str2 = str3 == null || str3 == string.Empty ? str2 + strArray[index1] : str2 + str3;
      }
      str1 = BitConverter.ToString(this.StringArrayToByteArray(str2, src)).Replace("-", "").Replace(BitConverter.ToString(Encoding.Unicode.GetBytes(CultureInfo.CurrentCulture.TextInfo.ListSeparator)).Replace("-", ""), replacewith);
    }
    int length = str1.Length;
    byte[] numArray = new byte[length / 2];
    for (int startIndex = 0; startIndex < length; startIndex += 2)
      numArray[startIndex / 2] = Convert.ToByte(str1.Substring(startIndex, 2), 16 /*0x10*/);
    return numArray;
  }
  ...
  private byte[] StringArrayToByteArray(string value, byte[] arrFstCondFormula1)
  {
    ...
    byteArray[0] = arrFstCondFormula1[0];
    byteArray[1] = (byte) value.Length;
    byteArray[2] = arrFstCondFormula1[2];
    bytes.CopyTo((Array) byteArray, 3);
    return byteArray;
  }
  ...
  public override int GetHashCode()
  {
    ...
    return this.IsStrListExplicit.GetHashCode() ^ this.IsEmptyCell.GetHashCode() ^ this.IsSuppressArrow.GetHashCode() ^ this.IsShowPromptBox.GetHashCode() ^ this.IsShowErrorBox.GetHashCode() ^ this.DataType.GetHashCode() ^ this.ErrorStyle.GetHashCode() ^ this.Condition.GetHashCode() ^ this.PromtBoxTitle.GetHashCode() ^ this.ErrorBoxTitle.GetHashCode() ^ this.PromtBoxText.GetHashCode() ^ this.ErrorBoxText.GetHashCode() ^ length1.GetHashCode() ^ length2.GetHashCode();
  }
}
