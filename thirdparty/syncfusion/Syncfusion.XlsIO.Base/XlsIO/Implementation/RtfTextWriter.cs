...
// Type: Syncfusion.XlsIO.Implementation.RtfTextWriter
...
public class RtfTextWriter : TextWriter
{
  ...
  private void WriteFontInTable(Font font)
  {
    ...
    this.Escape = false;
    this.Write($"{{\\f{num}\\fnil\\fcharset{font.GdiCharSet} {font.Name};}}");
    this.Escape = true;
  }
  ...
  public int AddFont(Font font)
  {
    ...
    int count = this.m_hashFonts.Count;
    this.m_hashFonts.Add(font, count);
    return count;
  }
  ...
  private void WriteEndFont(IFont font)
  {
    ...
    this.WriteEndFontItalicBoldStriked(font.GenerateNativeFont());
    this.WriteEndUnderline(font1);
    this.WriteEndSubSuperScript(font1);
  }
  ...
  private void WriteEndFontItalicBoldStriked(Font font)
  {
    if (font == null)
      throw new ArgumentNullException(nameof (font));
    if (font.Italic)
      this.WriteTag(RtfTags.ItalicOff);
    if (font.Bold)
      this.WriteTag(RtfTags.BoldOff);
    if (!font.Strikeout)
      return;
    this.WriteStrikeThrough(StrikeThroughStyle.SingleOff);
  }
  ...
  private void WriteEndUnderline(FontImpl font)
  {
    ...
    switch (font.Underline)
    {
      ...
      case ExcelUnderline.SingleAccounting:
      case ExcelUnderline.DoubleAccounting:
        this.WriteUnderlineAttribute(UnderlineStyle.None);
        break;
    }
  }
  ...
  private void WriteEndSubSuperScript(FontImpl font)
  {
    ...
    if (!font.Subscript && !font.Superscript)
      return;
    this.WriteTag(RtfTags.SubSuperOff);
  }
}
