...
// Type: Syncfusion.XlsIO.Implementation.Charts.ChartExDataCache
...
internal class ChartExDataCache
{
  ...
  internal void CopyProperties(ChartSerieImpl serie, WorkbookImpl workbook)
  {
    ...
    if (this.m_categoryFormula != null)
    {
      IRange range = ChartParser.GetRange(workbook, this.m_categoryFormula);
      serie.CategoryLabels = !(range is IName name) || name.RefersToRange == null ? range : name.RefersToRange;
    }
    if (this.m_seriesFormula != null)
    {
      IRange range = ChartParser.GetRange(workbook, this.m_seriesFormula);
      serie.Values = !(range is IName name) || name.RefersToRange == null ? range : name.RefersToRange;
    }
