...
// Type: Syncfusion.XlsIO.Implementation.Charts.ChartSeriesCollection
  ...
  IEnumerable
{
  ...
  private System.Collections.Generic.List<IChartSerie> m_additionOrder = new System.Collections.Generic.List<IChartSerie>();
  ...
  public new IChartSerie this[int index]
  {
    get => this.List[index];
  ...
  public IChartSerie this[string name]
  {
    get
    {
      ...
      {
        IChartSerie chartSerie = this.List[index];
        if (chartSerie.Name == name)
          return chartSerie;
      }
      return (IChartSerie) null;
    }
  }
  ...
  public IChartSerie Add(ExcelChartType serieType)
  {
    ...
    return (IChartSerie) chartSerieImpl;
  }

  public IChartSerie Add(string name, ExcelChartType serieType)
  {
    ChartSerieImpl chartSerieImpl = (ChartSerieImpl) this.Add(name);
    chartSerieImpl.ChangeSeriesType(serieType, true);
    (chartSerieImpl.SerieFormat as ChartSerieDataFormatImpl).HasMarkerProperties = true;
    return (IChartSerie) chartSerieImpl;
  }
  ...
  public new void RemoveAt(int index)
  {
    ...
    if (this.GetCountOfSeriesWithSameDrawingOrder(chartSerieImpl.ChartGroup) == 0 && count != 1)
      this.m_chart.RemoveFormat((IChartFormat) chartSerieImpl.GetCommonSerieFormat());
    this.UpdateSerieIndexAfterRemove(index);
    if (!this.HasSecondary())
      this.m_chart.RemoveSecondaryAxes();
    if (index != 0 || this.List.Count <= 0 || !this.m_chart.IsHistogramOrPareto || (this.List[0].SerieFormat as ChartSerieDataFormatImpl).HistogramAxisFormatProperty == null)
      return;
    (this.m_chart.PrimaryCategoryAxis as ChartCategoryAxisImpl).HistogramAxisFormatProperty.Clone((this.List[0].SerieFormat as ChartSerieDataFormatImpl).HistogramAxisFormatProperty);
  }
  ...
  public void Serialize(OffsetArrayList records)
  {
    ...
    foreach (ChartSerieImpl chartSerieImpl in (IEnumerable<IChartSerie>) this.List)
    {
  ...
  public IChartSerie Add(ChartSerieImpl serieToAdd)
  {
    ...
    this.Add((IChartSerie) serieToAdd);
    ...
    if (!this.m_chart.ParentWorkbook.Loading)
    {
      ...
      this.m_additionOrder.Add((IChartSerie) serieToAdd);
    return (IChartSerie) serieToAdd;
  }
  ...
  {
    ChartSeriesCollection parent1 = new ChartSeriesCollection(this.Application, parent);
    System.Collections.Generic.List<IChartSerie> innerList = this.InnerList;
    int index = 0;
  ...
  internal void ClearSeriesForChangeChartType(bool preserveFormats)
  {
    ...
    for (int index = 0; index < this.Count; ++index)
    {
      if (chartDataLabelsImplList[index] != null)
        (this.List[index].DataPoints.DefaultDataPoint as ChartDataPointImpl).DataLabels = (IChartDataLabels) chartDataLabelsImplList[index];
    }
  ...
  [Obsolete("This method is obsolete and will be removed soon. Please use UpdateDataPointForCylConePyrChartType(ExcelBaseFormat baseFormat, ExcelTopFormat topFormat) method. Sorry for inconvenience.")]
    ...
    ExcelTopFormat topFormat)
  {
    for (int index = 0; index < this.Count; ++index)
    {
      IChartSerieDataFormat dataFormat = ((ChartSerieImpl) this.List[index]).DataPoints.DefaultDataPoint.DataFormat;
      dataFormat.BarShapeBase = baseFormat;
      dataFormat.BarShapeTop = topFormat;
    }
  }

  public void UpdateDataPointForCylConePyrChartType(
    ExcelBaseFormat baseFormat,
    ExcelTopFormat topFormat)
  {
    for (int index = 0; index < this.Count; ++index)
  ...
  {
    return CollectionBaseEx<IChartSerie>.GenerateDefaultName((ICollection<IChartSerie>) this.List, "Serie");
  }
  ...
  public string GetDefSerieName(int iSerieIndex)
  {
    ...
    IList<IChartSerie> namesCollection;
    ...
    {
      namesCollection = (IList<IChartSerie>) new System.Collections.Generic.List<IChartSerie>(iSerieIndex);
      for (int index = 0; index < iSerieIndex; ++index)
        namesCollection.Add(this.List[index]);
    }
    return CollectionBaseEx<IChartSerie>.GenerateDefaultName((ICollection<IChartSerie>) namesCollection, "Serie");
  }
  ...
  public void AssignTrendDataLabel(ChartTextAreaImpl area)
  {
    ...
    {
      IChartTrendLines trendLines = this.List[index].TrendLines;
      int iIndex = 0;
  ...
  internal void ResortSeries(Dictionary<int, int> dictSeriesAxis, System.Collections.Generic.List<int> markerSeriesList)
  {
    ...
    for (int index1 = 0; index1 < count; ++index1)
    {
      ...
      if (sortedList.ContainsKey(index2) && (chartSerieImpl.IsFiltered || sortedList[index2].IsFiltered))
        intList.Add(index1);
      else
    ...
    if (intList.Count > 0)
    {
      ...
      {
        ChartSerieImpl chartSerieImpl = (ChartSerieImpl) innerList[intList[index]];
        while (sortedList.ContainsKey(key) && key < (int) ushort.MaxValue)
          ++key;
        chartSerieImpl.Index = key;
        sortedList.Add(key, chartSerieImpl);
      }
    }
    ...
    for (int index3 = 0; index3 < count; ++index3)
    {
      ...
      innerList[index3] = (IChartSerie) chartSerieImpl;
  ...
  internal System.Collections.Generic.List<IChartSerie> AdditionOrder => this.m_additionOrder;
