...
// Type: Syncfusion.XlsIO.Implementation.Shapes.TextBoxShapeBase
...
public class TextBoxShapeBase : ShapeImpl
{
    ...
    bool addToCollections)
  {
    ...
    {
      if (index < this.m_strText.Text.Length && this.m_strText.GetFont(index, true) is FontWrapper font)
      {
        FontImpl fontToAdd = font.Wrapped.Clone((object) workbook.InnerFonts);
        FontImpl fontImpl = workbook.AddFont((IFont) fontToAdd) as FontImpl;
        parent1.m_strText.TextObject.SetFontByIndex(index, fontImpl.Index);
      }
    }
  ...
  {
    return this.GetClientTextBoxRecord(parent, this.VAlignment);
  }
