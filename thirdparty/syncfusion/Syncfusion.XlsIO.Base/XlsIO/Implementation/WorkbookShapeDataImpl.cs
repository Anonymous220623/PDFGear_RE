...
// Type: Syncfusion.XlsIO.Implementation.WorkbookShapeDataImpl
...
public class WorkbookShapeDataImpl : CommonObject, ICloneParent
{
      ...
      for (int count = objects.Count; index < count; ++index)
      {
        ...
        if (shapeCollectionBase.ShapesCount != 0)
        {
          ...
          if (worksheet != null && worksheet.InnerDVTable != null)
            shapesCount += (uint) (worksheet.InnerDVTable.Count + 1);
          if (shapeIds == null)
  ...
  {
    lock (WorkbookShapeDataImpl.m_lockObj)
    {
      if (image == null)
      ...
      if (num1 == -1 && num2 == -1)
      {
        ...
        if (image.RawFormat.Guid.ToString() == ImageFormat.Jpeg.Guid.ToString() && Enum.GetName(typeof (PixelFormat), (object) image.PixelFormat) == null || image.RawFormat.Guid.ToString() == ImageFormat.Emf.Guid.ToString() || image.RawFormat.Guid.ToString() == ImageFormat.MemoryBmp.Guid.ToString())
          image.Save((Stream) memoryStream1, ImageFormat.Bmp);
        else
          image.Save((Stream) memoryStream1, image.RawFormat);
        image = Image.FromStream((Stream) memoryStream1);
      ...
      if (num2 == -1)
      {
        if (memoryStream1 != null && memoryStream1.Length == memoryStream1.Position)
        {
          pictureRecord.PictureStream = (Stream) memoryStream1;
        }
        else
        {
          ImageFormat rawFormat = image.RawFormat;
          MemoryStream memoryStream2 = new MemoryStream();
          image.Save((Stream) memoryStream2, rawFormat);
          pictureRecord.PictureStream = (Stream) memoryStream2;
        }
      }
