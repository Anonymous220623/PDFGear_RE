...
// Type: Syncfusion.XlsIO.Implementation.RangesOperations
...
public class RangesOperations
{
  ...
  internal void AddRanges(Rectangle rect) => this.CellList.Add(rect);
  ...
  internal void Remove(Rectangle[] arrRanges, bool isCF)
  {
    ...
    int length = arrRanges.Length;
    for (int index = 0; index < length; ++index)
      this.Remove(arrRanges[index], isCF);
  }
  ...
  private int Remove(Rectangle rect, bool isCF)
  {
    ...
    for (int index1 = 0; index1 < num; ++index1)
    {
      ...
      if (isCF ? object.Equals((object) rectangle1, (object) rect) : UtilityMethods.Intersects(rectangle1, rect))
      {
        int index2 = num - 1;
