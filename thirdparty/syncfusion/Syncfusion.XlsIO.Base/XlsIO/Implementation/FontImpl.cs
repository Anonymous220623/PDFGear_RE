...
// Type: Syncfusion.XlsIO.Implementation.FontImpl
  ...
  IDisposable
{
  ...
  internal readonly double MIN_FONT_SIZE = 1.0;
  internal readonly double MAX_FONT_SIZE = 409.5;
  ...
  public double Size
  {
    ...
    set
    {
      if (value < this.MIN_FONT_SIZE || value > this.MAX_FONT_SIZE)
        throw new ArgumentOutOfRangeException("Font.Size", "Font.Size out of range. Size must be less then 409.5 and greater than 1.");
  ...
  {
    get => this.m_font.SuperSubscript == ExcelFontVertialAlignment.Subscript;
    set
    {
      ...
      if (value)
      {
        this.m_font.SuperSubscript = ExcelFontVertialAlignment.Subscript;
        if (this.BaseLine >= 0)
          this.BaseLine = -25000;
      }
      else if (this.m_font.SuperSubscript == ExcelFontVertialAlignment.Subscript)
      {
        this.m_font.SuperSubscript = ExcelFontVertialAlignment.Baseline;
        if (this.BaseLine < 0)
          this.BaseLine = 0;
      }
      this.SetChanged();
  ...
  {
    get => this.m_font.SuperSubscript == ExcelFontVertialAlignment.Superscript;
    set
    {
      ...
      if (value)
      {
        this.m_font.SuperSubscript = ExcelFontVertialAlignment.Superscript;
        if (this.BaseLine <= 0)
          this.BaseLine = 30000;
      }
      else if (this.m_font.SuperSubscript == ExcelFontVertialAlignment.Superscript)
      {
        this.m_font.SuperSubscript = ExcelFontVertialAlignment.Baseline;
        if (this.BaseLine > 0)
          this.BaseLine = 0;
  ...
  protected Graphics BookGraphics => this.ParentWorkbook.InnerGraphics;
  ...
  {
    get => this.m_canDispose;
    set => this.m_canDispose = value;
  }
  ...
  {
    this.m_color = new ColorObject((ExcelKnownColors) this.m_font.PaletteColorIndex);
    this.m_color.AfterChange += new ColorObject.AfterChangeHandler(this.UpdateRecord);
  }
  ...
  private void UpdateColor()
  {
    if (this.m_color.ColorType == ColorType.RGB || this.m_color.ColorType == ColorType.Theme)
      ...
      this.m_color.SetIndexed((ExcelKnownColors) this.m_font.PaletteColorIndex);
  }

  private void UpdateColor(ColorObject obj)
  {
    if (obj.ColorType == ColorType.RGB || obj.ColorType == ColorType.Theme)
      this.m_color.SetRGB(obj.GetRGB((IWorkbook) this.m_book));
    else
      this.m_color.SetIndexed((ExcelKnownColors) this.m_font.PaletteColorIndex);
  }
  ...
  public System.Drawing.Font GenerateNativeFont(float size)
  {
    ...
    if (this.Underline != ExcelUnderline.None)
  ...
  public void Parse(System.Drawing.Font nativeFont)
  {
    ...
    this.Underline = nativeFont.Underline ? ExcelUnderline.Single : ExcelUnderline.None;
    this.UpdateColor();
  }
  ...
  public SizeF MeasureStringSpecial(string strValue)
  {
    ...
    if (strValue.Length <= 32000)
      format.SetMeasurableCharacterRanges(FontImpl.characterRanges);
    lock (this.BookGraphics)
    {
      Graphics bookGraphics = this.BookGraphics;
      TextRenderingHint textRenderingHint = bookGraphics.TextRenderingHint;
      bookGraphics.TextRenderingHint = TextRenderingHint.AntiAlias;
      double size = this.Size;
      if (!this.ParentWorkbook.IsConverting)
      {
        if (this.Bold)
          size *= this.Size >= 10.0 ? 1.0700000524520874 : 1.1499999761581421;
        else if (this.Size <= 10.0)
          size *= 1.0700000524520874;
      }
      System.Drawing.Font nativeFont = this.GenerateNativeFont((float) Math.Ceiling(size));
      sizeF = bookGraphics.MeasureString(strValue, nativeFont, int.MaxValue, format);
      nativeFont.Dispose();
      bookGraphics.TextRenderingHint = textRenderingHint;
    }
    if (sizeF == SizeF.Empty)
      sizeF = this.MeasureString(strValue);
    float height = sizeF.Height;
    float num = 0.0f;
    Dictionary<double, float> dictionary = (Dictionary<double, float>) null;
    if (ApplicationImpl.FontsHeight.TryGetValue(this.FontName, out dictionary) && dictionary.TryGetValue(this.Size, out num))
      height = (float) this.Application.ConvertUnits((double) num, MeasureUnits.Point, MeasureUnits.Pixel) * (float) (strValue.Length - strValue.Replace("\n", "").Length + 1);
    return new SizeF(sizeF.Width, height);
  }

  public SizeF[] MeasureCharacterRanges(string strValue, CharacterRange[] ranges)
  {
    System.Drawing.Font nativeFont = this.GenerateNativeFont();
    StringFormat stringFormat = new StringFormat();
    stringFormat.SetMeasurableCharacterRanges(ranges);
    stringFormat.FormatFlags = StringFormatFlags.NoClip;
    Region[] regionArray = this.BookGraphics.MeasureCharacterRanges(strValue, nativeFont, new RectangleF(0.0f, 0.0f, 1000f, 1000f), stringFormat);
    SizeF[] sizeFArray = new SizeF[regionArray.Length];
    int index = 0;
    for (int length = regionArray.Length; index < length; ++index)
    {
      RectangleF bounds = regionArray[index].GetBounds(this.BookGraphics);
      sizeFArray[index] = new SizeF(bounds.Width, bounds.Height);
    }
    return sizeFArray;
  }
  ...
  {
    return this.MeasureCharacterRanges(new string(value, 3), FontImpl.characterRanges)[1];
  }
  ...
  {
    FontImpl fontImpl = (FontImpl) this.MemberwiseClone();
    fontImpl.SetParent(parent);
    fontImpl.InitializeParent();
    fontImpl.m_bIsDisposed = this.m_bIsDisposed;
    fontImpl.m_index = -1;
    fontImpl.m_font = (FontRecord) this.m_font.Clone();
    if (this.m_fontNative != null)
      fontImpl.m_fontNative = (System.Drawing.Font) this.m_fontNative.Clone();
    fontImpl.m_color = !(this.m_color != (ColorObject) null) || this.m_color.ColorType == ColorType.Indexed ? new ColorObject((ExcelKnownColors) fontImpl.m_font.PaletteColorIndex) : this.m_color.Clone();
    fontImpl.m_color.AfterChange -= new ColorObject.AfterChangeHandler(this.UpdateRecord);
    fontImpl.m_color.AfterChange += new ColorObject.AfterChangeHandler(fontImpl.UpdateRecord);
    return fontImpl;
  }
  ...
  public FontStyle GetSupportedFontStyle(string fontName)
  {
    ...
    for (int index = 0; index < fontStyleArray.Length; ++index)
    {
      try
      {
        new System.Drawing.Font(fontName, 12f, fontStyleArray[index]).Dispose();
