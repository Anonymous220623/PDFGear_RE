...
// Type: Syncfusion.XlsIO.Implementation.Collections.Grouping.RangeGroup
...
public class RangeGroup : CommonObject, IRange, IParentApplication, IEnumerable<IRange>, IEnumerable
{
  ...
  public IHyperLinks Hyperlinks => throw new NotImplementedException("Hyperlinks property");
  ...
  public IRange Resize(int row, int colummn) => throw new NotImplementedException();
  ...
  public bool HasDataValidation
  {
    get
    {
      if (this.m_sheetGroup.IsEmpty)
        return false;
      bool hasDataValidation1 = this.GetRange(0).HasDataValidation;
      int iSheetIndex = 1;
      for (int count = this.m_sheetGroup.Count; iSheetIndex < count; ++iSheetIndex)
      {
        bool hasDataValidation2 = this.GetRange(iSheetIndex).HasDataValidation;
        if (hasDataValidation1 != hasDataValidation2)
        {
          hasDataValidation1 = false;
          break;
        }
      }
      return hasDataValidation1;
    }
  }
  ...
  public ExcelHAlign HorizontalAlignment
  {
    get
    {
      ...
      ExcelHAlign horizontalAlignment1 = this.GetRange(0).HorizontalAlignment;
      int iSheetIndex = 1;
      for (int count = this.m_sheetGroup.Count; iSheetIndex < count; ++iSheetIndex)
      {
        ExcelHAlign horizontalAlignment2 = this.GetRange(iSheetIndex).HorizontalAlignment;
  ...
  public ExcelVAlign VerticalAlignment
  {
    get
    {
      ...
      ExcelVAlign verticalAlignment1 = this.GetRange(0).VerticalAlignment;
      int iSheetIndex = 1;
      for (int count = this.m_sheetGroup.Count; iSheetIndex < count; ++iSheetIndex)
      {
        ExcelVAlign verticalAlignment2 = this.GetRange(iSheetIndex).VerticalAlignment;
  ...
  public ICommentShape Comment => throw new NotSupportedException();
  ...
  public ExcelIgnoreError IgnoreErrorOptions
  {
    get
    {
      ...
      int iSheetIndex = 0;
      for (int count = this.m_sheetGroup.Count; iSheetIndex < count && ignoreErrorOptions != ExcelIgnoreError.None; ++iSheetIndex)
        ignoreErrorOptions &= this.GetRange(iSheetIndex).IgnoreErrorOptions;
    ...
    set
    {
      int iSheetIndex = 0;
      for (int count = this.m_sheetGroup.Count; iSheetIndex < count; ++iSheetIndex)
        this.GetRange(iSheetIndex).IgnoreErrorOptions = value;
    }
  }

  public bool? IsStringsPreserved
  {
    get => this.m_sheetGroup.GetStringPreservedValue(this);
    set => this.m_sheetGroup.SetStringPreservedValue(this, value);
  }
  ...
  public IRange Group(ExcelGroupBy groupBy) => throw new NotSupportedException();

  public IRange Group(ExcelGroupBy groupBy, bool bCollapsed) => throw new NotSupportedException();
  ...
  public IRange Ungroup(ExcelGroupBy groupBy) => throw new NotSupportedException();
  ...
  public ICommentShape AddComment() => throw new NotSupportedException();
  ...
  IRange IRange.FindFirst(double findValue, ExcelFindType flags)
  {
    throw new NotImplementedException();
  }
  ...
  {
    ExcelKnownColors nearestColor = this.Workbook.GetNearestColor(borderColor);
