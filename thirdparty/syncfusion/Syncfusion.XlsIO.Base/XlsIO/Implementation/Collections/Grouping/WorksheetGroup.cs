...
// Type: Syncfusion.XlsIO.Implementation.Collections.Grouping.WorksheetGroup
  ...
  ICloneParent
{
  ...
  public int Add(ITabSheet sheet)
  {
    ...
    if (sheet.IsSelected && !this.m_book.Loading)
      return -1;
    ...
    {
      if (sheet == this.m_book.ActiveTabSheet)
        (sheet as WorksheetBaseImpl).SelectTab();
      num = -1;
    }
    return num;
  }
  ...
  public IAutoFilters AutoFilters => throw new NotSupportedException();
  ...
  public IDataSort DataSorter => throw new NotSupportedException();
  ...
  public ExcelSheetType Type => throw new NotImplementedException();
  ...
  public WorksheetVisibility Visibility
  {
    ...
    {
      IList<IWorksheet> innerList = (IList<IWorksheet>) this.InnerList;
      WorksheetVisibility visibility = innerList[0].Visibility;
      int index = 1;
      for (int count = innerList.Count; index < count; ++index)
      {
  ...
  public IVPageBreaks VPageBreaks => (IVPageBreaks) null;

  public IHPageBreaks HPageBreaks => (IHPageBreaks) null;
  ...
  public IComments Comments => throw new NotSupportedException();
  ...
  public IHyperLinks HyperLinks => throw new NotSupportedException();
  ...
  public IPivotTables PivotTables => throw new NotSupportedException();

  public IListObjects ListObjects => throw new NotSupportedException();

  public IOleObjects OleObjects => throw new NotSupportedException();
    ...
    bool preserveTypes)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      num = (innerList[index] as WorksheetImpl).ImportData(arrObject, firstRow, firstColumn, includeHeader, includeHeaderParent, nestedLayoutOptions, nestedGroupOptions, collapseLevel, preserveTypes);
    return num;
  }
    ...
    bool preserveTypes)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      num = innerList[index].ImportDataColumn(dataColumn, isFieldNameShown, firstRow, firstColumn, preserveTypes);
    return num;
  }
    ...
    bool isImportStyle)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      innerList[index].ImportDataGrid(dataGrid, firstRow, firstColumn, isImportHeader, isImportStyle);
  }
    ...
    bool isImportStyle)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      innerList[index].ImportGridView(gridView, firstRow, firstColumn, isImportHeader, isImportStyle);
  }
    ...
    bool isImportStyle)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      innerList[index].ImportDataGridView(dataGridView, firstRow, firstColumn, isImportHeader, isImportStyle);
  }
  ...
  {
    return this.ImportDataTable(dataTable, firtRow, firstColumn, importOnSave, false);
  }
    ...
    bool includeHeader)
  {
    ...
    for (int count = innerList.Count; index < count; ++index)
    {
      if (innerList[index] is WorksheetImpl worksheetImpl)
        num = worksheetImpl.ImportDataTable(dataTable, includeHeader, firtRow, firstColumn, -1, -1, (DataColumn[]) null, false, importOnSave);
    }
    return num;
  }
  ...
  public System.Collections.Generic.List<T> ExportData<T>(
    int firstRow,
    int firstColumn,
    int lastRow,
    int lastColumn)
  ...
  public void AutofitRow(int rowIndex)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      (innerList[index] as WorksheetImpl).AutofitRow(rowIndex);
  }
  ...
  public void AutofitColumn(int colIndex)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      (innerList[index] as WorksheetImpl).AutofitColumn(colIndex);
  }
  ...
  public void Replace(string oldValue, string newValue, ExcelFindOptions findOptions)
  {
    ...
    int index = 0;
    for (int count = innerList.Count; index < count; ++index)
      innerList[index].Replace(oldValue, newValue, findOptions);
  }
  ...
  public ExcelKnownColors TabColor
  {
    ...
    {
      IList<IWorksheet> innerList = (IList<IWorksheet>) this.InnerList;
      ExcelKnownColors tabColor = innerList[0].TabColor;
      int index = 1;
      for (int count = innerList.Count; index < count; ++index)
      {
  ...
  public ICheckBoxes CheckBoxes => throw new NotSupportedException();

  public IOptionButtons OptionButtons => throw new NotSupportedException();

  public IComboBoxes ComboBoxes => throw new NotSupportedException();
