...
// Type: Syncfusion.XlsIO.Implementation.Collections.FormatsCollection
...
public class FormatsCollection(IApplication application, object parent) : CommonObject(application, parent)
{
  ...
  internal const int MAX_FORMATS_COUNT = 242;
  ...
  private string[] DEF_CURRENCY_FORMAT_STRING = new string[44]
  {
    "\"$\"#,##0.00",
    "#,##0.00\\ [$֏-42B]",
    "[$₸-43F]#,##0.00",
  ...
  private string[] DEF_CURRENCY_SYMBOL = new string[44]
  {
    "$",
    "֏",
    "₸",
  ...
  public void Serialize(OffsetArrayList records)
  {
    ...
    records.AddRange((ICollection) this.GetUsedFormats(ExcelVersion.Excel97to2003));
  }
  ...
  public int CreateFormat(string formatString)
  {
    switch (formatString)
    {
      ...
      default:
        ...
        {
          string str = formatString.Replace("\"$\"", "$");
          foreach (string key in this.m_hashFormatStrings.Keys)
          {
            if (key.Replace("\\", string.Empty).Replace("\"$\"", "$") == str)
            {
              FormatImpl hashFormatString = this.m_hashFormatStrings[key];
              hashFormatString.isUsed = true;
              return hashFormatString.Index;
            }
          }
        }
        int count = this.m_rawFormats.Count;
        int num = this.m_rawFormats.GetKey(count - 1);
        if (num < 163)
          num = 163;
        int format = num + 1;
        if (count >= 242)
  ...
  internal string GetCustomizedString(string formatString)
  {
    ...
    if (formatString.StartsWith("\"") && formatString.EndsWith("\""))
      return formatString;
    string str2 = this.CheckForCulturePattern(str1);
    List<string> stringList = this.CheckforColorPattern(str1);
    for (int index = 0; index < stringList.Count; ++index)
      str1 = str1.Replace(stringList[index], $"[C_{index.ToString()}]");
    if (str2 != string.Empty)
      str1 = str1.Replace(str2, "[C]");
    string str3 = this.GetFormattedStringFormats(str1);
    if (formatString.Equals("Standard") || str3.Contains(CultureInfo.CurrentCulture.NumberFormat.CurrencySymbol) || !str3.Contains("d") && !str3.Contains("m") && !str3.Contains("y") && !str3.Contains("h") && !str3.Contains("s"))
      return formatString;
    List<char> formatStringCharArray = new List<char>((IEnumerable<char>) str3.ToCharArray());
    char[] chArray = new char[4]{ ',', ' ', '.', '-' };
    foreach (char ch in chArray)
    {
      if (str3.Trim().Contains(ch.ToString()))
      {
        int num1 = formatStringCharArray.IndexOf(ch);
        int num2 = formatStringCharArray.LastIndexOf(ch);
        for (int index = num1; index <= num2; ++index)
        {
          if ((int) formatStringCharArray[index] == (int) ch)
          {
            if (index == 0 || index == formatStringCharArray.Count - 1)
            {
              if (index == 0 || formatStringCharArray[index - 1] != '\\')
              {
                formatStringCharArray.Insert(index, '\\');
                ++index;
                ++num2;
              }
            }
            else if (this.CheckIsBetweenDate(index, formatStringCharArray))
            {
              formatStringCharArray.Insert(index, '\\');
              ++index;
              ++num2;
      ...
      str3 = new string(formatStringCharArray.ToArray());
    }
    string customizedString = str3.Replace("[C]", str2);
    if (stringList.Count > 0)
    {
      for (int index = 0; index < stringList.Count; ++index)
        customizedString = customizedString.Replace($"[C_{index.ToString()}]", stringList[index]);
    }
  ...
  {
    Match match = CultureToken.CultureRegex.Match(formatString);
    return match.Success ? match.Value : string.Empty;
  }

  private List<string> CheckforColorPattern(string formatString)
  {
    List<string> stringList = new List<string>();
    Regex regex = new Regex("(\\[(?<Color>.*)\\])");
    foreach (string input in Regex.Split(formatString, ";"))
    {
    ...
    if (str.Contains("\""))
    {
      ...
      for (int index2 = 0; index2 < charArray.Length; ++index2)
      {
        ...
        if (charArray[index2] == '"')
        {
          ...
          if (index2 != 0)
          {
            strArray[index1] = str.Substring(startIndex, num2 - startIndex);
            startIndex = num2;
            ++index1;
          }
        }
        else if (index2 == charArray.Length - 1)
        {
          strArray[index1] = str.Substring(startIndex, charArray.Length - startIndex);
          ++index1;
        }
      }
      stringFormats = (string) null;
      for (int index3 = 0; index3 < index1; ++index3)
      {
        if (!strArray[index3].Contains("\""))
          strArray[index3] = this.ReplaceStringFormats(strArray[index3]);
        stringFormats += strArray[index3];
      }
    }
    else
      stringFormats = this.ReplaceStringFormats(stringFormats);
    if (strArray != null)
      ;
  ...
  {
    stringFormats = stringFormats.Replace("H", "h");
    stringFormats = stringFormats.Replace("M", "m");
    stringFormats = stringFormats.Replace("S", "s");
    stringFormats = stringFormats.Replace("D", "d");
    stringFormats = stringFormats.Replace("Y", "y");
    stringFormats = stringFormats.Replace("Am", "AM");
    stringFormats = stringFormats.Replace("Pm", "PM");
    return stringFormats;
  }
  ...
  internal string GetDateFormat(string strValue)
  {
    ...
    switch (strArray2.Length)
    {
      case 2:
        ...
        break;
      case 3:
        dateFormat = !flag ? (strArray2[1].Length <= 1 || !char.IsLetter(strArray2[1], 1) ? (strArray2[2].CompareTo("1900") < 0 ? "General" : this.m_rawFormats[formatIndex[0]].FormatString) : this.m_rawFormats[formatIndex[1]].FormatString) : (!this.IsStandardTimeFormat(strValue) ? this.m_rawFormats[formatIndex[4]].FormatString : this.m_rawFormats[formatIndex[5]].FormatString);
        break;
      default:
  ...
  private string ParseNumberFormat(string strValue)
  {
    ...
    {
      numArray = this.m_formatIndexes["."];
      if (flag1)
        numberFormat = "$" + this.m_rawFormats[this.m_formatIndexes[","][1]].FormatString;
      else
        numberFormat = $"0{(object) "."[0]}{new string('0', strValue.Split("."[0])[1].Length)}";
    }
    else
  ...
  private bool CheckIsBetweenDate(int index, List<char> formatStringCharArray)
  {
    char formatStringChar1 = formatStringCharArray[index - 1];
    char formatStringChar2 = formatStringCharArray[index + 1];
    bool flag = false;
    if (formatStringChar1 == 'm' || formatStringChar1 == 'h' || formatStringChar1 == 's' || formatStringChar1 == 'd' || formatStringChar1 == 'y')
      flag = true;
    if (formatStringChar2 == '0')
      flag = false;
    return flag;
  }
