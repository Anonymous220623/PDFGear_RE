...
// Type: Syncfusion.XlsIO.Implementation.Collections.RangesCollection
  ...
  INativePTG
{
  ...
  public string HtmlString
  {
    get
    {
      string htmlString = (this[this.Row, this.Column].RichText as RichTextString).PrepareHtml(this.CellStyle.Font, this[this.Row, this.Column]);
      for (int iFirstRow = this.m_iFirstRow; iFirstRow <= this.LastRow; ++iFirstRow)
      {
        for (int iFirstColumn = this.m_iFirstColumn; iFirstColumn <= this.LastColumn; ++iFirstColumn)
        {
          string str = (this[iFirstRow, iFirstColumn].RichText as RichTextString).PrepareHtml(this[iFirstRow, iFirstColumn].CellStyle.Font, this[iFirstRow, iFirstColumn]);
          if (htmlString != str)
            return (string) null;
        }
      }
    ...
    set
    {
      if (this.htmlStringParser == null)
        this.htmlStringParser = new HtmlStringParser();
      this.htmlStringParser.ParseHtml(value, this.RichText);
    }
  }
  ...
  public bool HasDataValidation
  {
    get
    {
      ...
      if (this.Count == 0)
        return false;
      bool hasDataValidation = this.InnerList[0].HasDataValidation;
      int index = 0;
      for (int count = this.Count; index < count; ++index)
      {
        IRange inner = this.InnerList[index];
        if (hasDataValidation != inner.HasDataValidation)
        {
  ...
  public ExcelHAlign HorizontalAlignment
  {
    get
    {
      ...
      ExcelHAlign horizontalAlignment = this.InnerList[0].HorizontalAlignment;
      int index = 0;
      for (int count = this.Count; index < count; ++index)
      {
        IRange inner = this.InnerList[index];
  ...
  public IHyperLinks Hyperlinks
  {
    get
    {
      this.CheckDisposed();
      if (this.Count == 0)
        return (IHyperLinks) null;
      HyperLinksCollection hyperlinks1 = new HyperLinksCollection(this.Application, (object) this, true);
      int index = 0;
      for (int count = this.Count; index < count; ++index)
      {
        HyperLinksCollection hyperlinks2 = (HyperLinksCollection) this.InnerList[index].Hyperlinks;
        if (hyperlinks2 != null)
          hyperlinks1.AddRange((IList<HyperLinkImpl>) hyperlinks2);
      }
      return (IHyperLinks) hyperlinks1;
    }
  }
  ...
  public ExcelVAlign VerticalAlignment
  {
    get
    {
      ...
      ExcelVAlign verticalAlignment = this.InnerList[0].VerticalAlignment;
      int index = 0;
      for (int count = this.Count; index < count; ++index)
      {
        IRange inner = this.InnerList[index];
  ...
  public IRange this[int row, int column]
  {
    ...
    {
      this.CheckDisposed();
      return this.Worksheet.Range[row, column];
    }
    set
    {
      this.CheckDisposed();
      this.Worksheet.Range[row, column] = value;
    }
  }
  ...
  public IRange this[int row, int column, int lastRow, int lastColumn]
  {
    ...
    {
      this.CheckDisposed();
      return this.Worksheet.Range[row, column, lastRow, lastColumn];
    }
  }
  ...
  public ExcelIgnoreError IgnoreErrorOptions
  {
    get
    {
      ...
      int index = 0;
      for (int count = this.Count; index < count && ignoreErrorOptions != ExcelIgnoreError.None; ++index)
      {
        IRange range = (IRange) innerList[index];
        ignoreErrorOptions &= range.IgnoreErrorOptions;
      }
    ...
    set
    {
      ...
      int index = 0;
      for (int count = this.Count; index < count; ++index)
        ((IRange) innerList[index]).IgnoreErrorOptions = value;
    }
  }
  ...
  public double Sum(bool considerDateAsNumber)
  {
    ...
    {
      IRange inner = this.InnerList[index];
      num += inner.Sum(considerDateAsNumber);
    }
    return num;
  }
  ...
  public double Average(bool considerDateAsNumber)
  {
    double num1 = 0.0;
    double num2 = 0.0;
    System.Collections.Generic.List<System.Collections.Generic.List<double>> doubleListList = new System.Collections.Generic.List<System.Collections.Generic.List<double>>();
    int index = 0;
    for (int count = this.Count; index < count; ++index)
    {
      IRange inner = this.InnerList[index];
      doubleListList.Add((inner as RangeImpl).GetNumberList(considerDateAsNumber));
    }
    foreach (System.Collections.Generic.List<double> doubleList in doubleListList)
    {
  ...
  public double Min(bool considerDateAsNumber)
  {
    ...
    for (int count = this.Count; index < count; ++index)
    {
      double num2 = this.InnerList[index].Min(considerDateAsNumber);
      if (num2 < num1)
        num1 = num2;
    }
    return num1;
  ...
  public double Max(bool considerDateAsNumber)
  {
    ...
    for (int count = this.Count; index < count; ++index)
    {
      double num2 = this.InnerList[index].Max(considerDateAsNumber);
      if (num2 > num1)
        num1 = num2;
    }
    return num1;
  ...
  public IRange Trim()
  {
    IRanges rangesCollection = this.Worksheet.CreateRangesCollection();
    int index = 0;
    for (int count = this.Count; index < count; ++index)
    {
      IRange range = this.InnerList[index].Trim();
      if (range != null)
  ...
  public IRange CopyTo(IRange destination) => this.CopyTo(destination, ExcelCopyRangeOptions.All);
  ...
  public IRange CopyTo(IRange destination, bool pasteLink)
  {
    ...
    if (destination == null)
      throw new ArgumentNullException(nameof (destination));
    return !pasteLink ? this.CopyTo(destination) : this.CheckAndCopyRanges(destination, pasteLink, ExcelCopyRangeOptions.All, false);
  }

  internal IRange CheckAndCopyRanges(
    IRange destination,
    bool pasteLink,
    ...
    bool skipBlank)
  {
    ...
    for (int count = this.Count; index < count; ++index)
    {
      ...
      if (index == 0)
      {
        ...
        num2 = inner.Column;
        num3 = inner.LastRow;
        num4 = inner.LastColumn;
        row = destination.Row;
        column = destination.Column;
      }
      else if (inner.Column == num2 && inner.LastColumn == num4)
      {
        row = num5 + 1;
      }
      else
      {
        if (inner.Row != num1 || inner.LastRow != num3)
          throw new InvalidOperationException("This action won't work on multiple selections.");
        column = num6 + 1;
      }
      RangeImpl rangeImpl2;
      destination = (IRange) (rangeImpl2 = (RangeImpl) rangeImpl1.Worksheet[row, column, row + (inner.LastRow - inner.Row), column + (inner.LastColumn - inner.Column)]);
      if (pasteLink)
        inner.CopyTo(destination, pasteLink);
      if (skipBlank)
        inner.CopyTo(destination, options, skipBlank);
      row = destination.Row;
      column = destination.Column;
      num5 = destination.LastRow;
      num6 = destination.LastColumn;
    }
    return destination;
  }
  ...
  public IRange CopyTo(IRange destination, ExcelCopyRangeOptions options, bool skipBlank)
  {
    ...
    if (destination == null)
      throw new ArgumentNullException(nameof (destination));
    return !skipBlank ? this.CopyTo(destination, options) : this.CheckAndCopyRanges(destination, false, options, skipBlank);
  }
  ...
  public ICommentShape AddComment()
  {
    ...
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      this.InnerList[index].AddComment();
    return this.Comment;
  }
  ...
  public void Replace(string oldValue, string newValue)
  {
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      this.InnerList[index].Replace(oldValue, newValue);
  }
  ...
  public void Replace(string oldValue, string newValue, ExcelFindOptions findOptions)
  {
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      this.InnerList[index].Replace(oldValue, newValue, findOptions);
  }
  ...
  public void Replace(string oldValue, string[] newValues, bool isVertical)
  {
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      this.InnerList[index].Replace(oldValue, newValues, isVertical);
  }
  ...
  public void Replace(string oldValue, DataTable newValues, bool isFieldNamesShown)
  {
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      this.InnerList[index].Replace(oldValue, newValues, isFieldNamesShown);
  }
  ...
  {
    ExcelKnownColors nearestColor = this.m_worksheet.Workbook.GetNearestColor(borderColor);
  ...
  {
    return this.GetPrecedentRanges(isEntireWorkbook, false);
  }
  ...
  {
    return this.GetPrecedentRanges(isEntireWorkbook, true);
  }
  ...
  private IRange[] GetDependentRanges(bool isEntireWorkbook, bool isDirect)
  {
    ...
    {
      IRange inner = this.InnerList[index];
      IRange[] ranges = !isDirect ? inner.GetDependents(isEntireWorkbook) : inner.GetDirectDependents(isEntireWorkbook);
      if (ranges != null)
        this.AddToDictionary(resultCells, ranges);
    }
    return resultCells == null || resultCells.Count == 0 ? (IRange[]) null : new System.Collections.Generic.List<IRange>((IEnumerable<IRange>) resultCells.Values).ToArray();
  }
  ...
  private IRange[] GetPrecedentRanges(bool isEntireWorkbook, bool isDirect)
  {
    ...
    {
      IRange inner = this.InnerList[index];
      IRange[] ranges = !isDirect ? inner.GetPrecedents(isEntireWorkbook) : inner.GetDirectPrecedents(isEntireWorkbook);
      if (ranges != null)
        this.AddToDictionary(resultCells, ranges);
    }
    return resultCells == null || resultCells.Count == 0 ? (IRange[]) null : new System.Collections.Generic.List<IRange>((IEnumerable<IRange>) resultCells.Values).ToArray();
  ...
  private void AddToDictionary(Dictionary<string, IRange> resultCells, IRange[] ranges)
  {
    for (int index = 0; index < ranges.Length; ++index)
    {
      if (!resultCells.ContainsKey(ranges[index].Address))
        resultCells.Add(ranges[index].Address, ranges[index]);
    }
  }

  private void SetCellStyleName(string cellStyleName)
  {
    WorksheetImpl worksheet = this.m_worksheet as WorksheetImpl;
    MigrantRangeImpl migrantRangeImpl = new MigrantRangeImpl(this.Application, this.m_worksheet);
    ...
    for (int count = this.Count; index < count; ++index)
    {
      ...
      for (int iRow = row; iRow <= lastRow; ++iRow)
      {
        for (int iColumn = column; iColumn <= lastColumn; ++iColumn)
        {
          ...
          ushort xfIndex = (ushort) worksheet.GetXFIndex(iRow, iColumn);
          int iXFIndex;
          if (dictionary.TryGetValue((int) xfIndex, out iXFIndex))
          {
            BiffRecordRaw record1 = (BiffRecordRaw) worksheet.GetRecord(iRow, iColumn);
            RangeImpl.TCellType oldType = record1 != null ? (RangeImpl.TCellType) record1.TypeCode : RangeImpl.TCellType.Blank;
            worksheet.CellRecords.SetCellStyle(iRow, iColumn, iXFIndex);
            BiffRecordRaw record2 = (BiffRecordRaw) worksheet.GetRecord(iRow, iColumn);
            if (record2 != null && record2.TypeCode == TBIFFRecord.Formula || record2 == null)
            {
              string old = inner.Value;
              migrantRangeImpl.OnValueChanged(old, old);
            }
            migrantRangeImpl.OnStyleChanged(oldType);
          }
          else
          {
            inner.CellStyleName = cellStyleName;
  ...
  public void ClearConditionalFormats() => this.ClearConditionalFormats(false);

  internal void ClearConditionalFormats(bool isCF)
  {
    int index = 0;
    for (int count = this.Count; index < count; ++index)
      ((ICombinedRange) this[index] as RangeImpl).ClearConditionalFormats(isCF);
  }
