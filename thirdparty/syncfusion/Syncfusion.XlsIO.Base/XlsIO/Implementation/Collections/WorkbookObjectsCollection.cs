...
// Type: Syncfusion.XlsIO.Implementation.Collections.WorkbookObjectsCollection
...
public class WorkbookObjectsCollection : CollectionBaseEx<object>, ITabSheets
{
  ...
  ITabSheet ITabSheets.this[int index]
  {
    get
    {
      return index >= 0 && index <= this.Count - 1 ? this.InnerList[index] as ITabSheet : throw new ArgumentOutOfRangeException(nameof (index), "Value cannot be less than 0 and greater than Count - 1.");
    }
  }
  ...
  private void FindVisibleWorksheet(int proposedIndex)
  {
    ...
    else
    {
      ...
      {
        if ((this[index] as ITabSheet).Visibility == WorksheetVisibility.Visible)
        {
          num = index;
          break;
        }
  ...
  private void object_NameChanged(object sender, ValueChangedEventArgs e)
  {
    ...
    if (oldValue.ToUpper() != newValue.ToUpper() && this.m_hashNameToValue.ContainsKey(newValue))
