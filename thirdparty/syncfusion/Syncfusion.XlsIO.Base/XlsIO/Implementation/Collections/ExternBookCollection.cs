...
// Type: Syncfusion.XlsIO.Implementation.Collections.ExternBookCollection
...
public class ExternBookCollection : CollectionBaseEx<ExternWorkbookImpl>
{
  ...
  internal Dictionary<string, ExternWorkbookImpl> m_hashUrlToBook = new Dictionary<string, ExternWorkbookImpl>();
  ...
  public int Parse(BiffRecordRaw[] arrData, int iOffset)
  {
    ...
    while (arrData[iOffset].TypeCode == TBIFFRecord.SupBook)
    {
      ...
      book.Index = this.InnerList.Count;
  ...
  public void Parse(BiffReader reader, IDecryptor decryptor)
  {
    ...
    for (TBIFFRecord tbiffRecord = reader.PeekRecordType(); tbiffRecord == TBIFFRecord.SupBook; tbiffRecord = reader.PeekRecordType())
    {
      ExternWorkbookImpl book = new ExternWorkbookImpl(this.Application, (object) this);
  ...
  public int AddDDEFile(string fileName)
  {
    ...
    ExternNameImpl externNameImpl = externNames[index];
    externNameImpl.Record.Options = (ushort) 32746;
    externNameImpl.isOle = true;
  ...
  public int InsertSelfSupbook()
  {
    ...
    for (int count = this.Count; index < count; ++index)
    {
      ...
      {
        externWorkbookImpl.SheetNumber = (int) (ushort) (this.m_book.Worksheets.Count + this.m_book.Charts.Count);
        return index;
      }
    }
    base.Add(new ExternWorkbookImpl(this.Application, (object) this)
    {
      ...
      SheetNumber = (int) (ushort) (this.m_book.Worksheets.Count + this.m_book.Charts.Count)
    });
    return this.Count - 1;
  }
  ...
  public bool ContainsExternName(string strName, ref int iBookIndex, ref int iNameIndex)
  {
    ...
    for (int count = this.Count; index < count; ++index)
    {
      ...
      {
        iBookIndex = index;
        this.m_book.AddSheetReference(externWorkbookImpl.Index, 65534, 65534);
        return true;
      }
    }
    return false;
  ...
  protected override void OnInsertComplete(int index, ExternWorkbookImpl value)
  {
    ...
    if (url == null || !(url != " "))
      return;
    if (!this.m_hashUrlToBook.ContainsKey(url) || !this.m_book.Loading)
      this.m_hashUrlToBook[url] = externWorkbookImpl;
  ...
  internal new bool Contains(ExternWorkbookImpl externBook)
  {
    for (int index = 0; index < this.Count; ++index)
    {
      if (externBook.IsInternalReference == this[index].IsInternalReference && externBook.URL == this[index].URL)
        return true;
    }
    return false;
  }

  internal new int IndexOf(ExternWorkbookImpl externBook)
  {
    for (int index = 0; index < this.Count; ++index)
    {
      if (externBook.IsInternalReference == this[index].IsInternalReference && externBook.URL == this[index].URL)
