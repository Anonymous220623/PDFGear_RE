...
internal static class Utility
{
  ...
  public static int GET_X_LPARAM(IntPtr lParam) => Utility.LOWORD((int) lParam.ToInt64());

  public static int GET_Y_LPARAM(IntPtr lParam) => Utility.HIWORD((int) lParam.ToInt64());
  ...
  public static bool IsFlagSet(uint value, uint mask) => 0 != ((int) value & (int) mask);

  public static bool IsFlagSet(long value, long mask) => 0L != (value & mask);

  public static bool IsFlagSet(ulong value, ulong mask) => 0L != ((long) value & (long) mask);
    ...
    int height)
  {
    ...
    {
      int bpp = flag ? frames[index2].Thumbnail.Format.BitsPerPixel : frames[index2].Format.BitsPerPixel;
      if (bpp == 0)
        bpp = 8;
