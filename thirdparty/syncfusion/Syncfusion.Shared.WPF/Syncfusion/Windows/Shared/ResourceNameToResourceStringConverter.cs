// Decompiled with JetBrains decompiler
// Type: Syncfusion.Windows.Shared.ResourceNameToResourceStringConverter
// Assembly: Syncfusion.Shared.Wpf, Version=19.3460.0.57, Culture=neutral, PublicKeyToken=3d67ed1f87d44c89
// MVID: 18386E21-CB25-4FB0-B138-3460FC9545EB
// Assembly location: C:\Program Files\PDFgear\Syncfusion.Shared.WPF.dll

using System;
using System.Globalization;
using System.Windows.Data;

#nullable disable
namespace Syncfusion.Windows.Shared;

public class ResourceNameToResourceStringConverter : IValueConverter
{
  protected virtual LocalizationResourceAccessor GetResourceAccessor()
  {
    throw new NotImplementedException("Should be implemented by derived class");
  }

  public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
  {
    string empty = string.Empty;
    if (value != null)
      empty = value.ToString();
    else if (parameter != null)
      empty = parameter.ToString();
    if (string.IsNullOrEmpty(empty))
      return (object) string.Empty;
    return this.GetResourceAccessor() == null ? (object) string.Empty : (object) this.GetResourceAccessor().GetString(empty);
  }

  public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
  {
    throw new NotImplementedException();
  }
}
