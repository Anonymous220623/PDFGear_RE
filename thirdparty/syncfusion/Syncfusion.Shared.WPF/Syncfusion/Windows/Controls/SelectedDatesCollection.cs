...
// Type: Syncfusion.Windows.Controls.SelectedDatesCollection
...
public sealed class SelectedDatesCollection : ObservableCollection<DateTime>
{
  ...
  public void AddRange(DateTime start, DateTime end)
  {
    ...
    if (this._owner.SelectionMode == CalendarSelectionMode.SingleRange && this.Count > 0)
      this.ClearInternal();
    foreach (DateTime dateTime in SelectedDatesCollection.GetDaysInRange(start, end, this._owner.FormatCalendar))
      this.Add(dateTime);
    this.EndAddRange();
  }
  ...
  protected override void InsertItem(int index, DateTime item)
  {
    ...
    if (!this._isAddingRange)
    {
      ...
      int num = DateTimeHelper.CompareYearMonth(item, this._owner.DisplayDateInternal, this._owner.FormatCalendar);
      if (num >= 2 || num <= -2)
        return;
  ...
  protected override void RemoveItem(int index)
  {
    ...
    else
    {
      ...
      int num = DateTimeHelper.CompareYearMonth(this[index], this._owner.DisplayDateInternal, this._owner.FormatCalendar);
  ...
  internal void AddRangeInternal(DateTime start, DateTime end)
  {
    ...
    foreach (DateTime dateTime2 in SelectedDatesCollection.GetDaysInRange(start, end, this._owner.FormatCalendar))
    {
  ...
  internal void Toggle(DateTime date)
  {
    ...
    switch (this._owner.SelectionMode)
    {
      case CalendarSelectionMode.SingleDate:
        if (!this._owner.SelectedDate.HasValue || DateTimeHelper.CompareDays(this._owner.SelectedDate.Value, date, this._owner.FormatCalendar) != 0)
        {
  ...
  private static IEnumerable<DateTime> GetDaysInRange(
    DateTime start,
    DateTime end,
    System.Globalization.Calendar calendar)
  {
    ...
    do
    {
      ...
      rangeStart = DateTimeHelper.AddDays(rangeStart.Value, increment, calendar);
    }
