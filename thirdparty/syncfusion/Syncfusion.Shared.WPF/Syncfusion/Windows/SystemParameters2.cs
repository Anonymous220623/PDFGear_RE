...
// Type: Syncfusion.Windows.SystemParameters2
...
public class SystemParameters2 : INotifyPropertyChanged
{
  ...
  private Dictionary<WM, List<SystemParameters2._SystemMetricUpdate>> _UpdateTable;
  ...
  private void _InitializeCaptionButtonLocation()
  {
    ...
    else
    {
      ...
      try
      {
        Marshal.StructureToPtr<TITLEBARINFOEX>(structure, hglobal, false);
  ...
  {
    this.HighContrast = (Standard.NativeMethods.SystemParameterInfo_GetHIGHCONTRAST().dwFlags & HCF.HIGHCONTRASTON) != (HCF) 0;
  }
  ...
  ~SystemParameters2() => this._Dispose(false);
  ...
  {
    if (this._messageHwnd != null)
    {
      this._messageHwnd.Dispose();
      this._messageHwnd = (MessageWindow) null;
    }
    this._uxThemeColor = (string) null;
    this._uxThemeName = (string) null;
    this._glassColorBrush = (SolidColorBrush) null;
    this.PropertyChanged = (PropertyChangedEventHandler) null;
    if (this._UpdateTable == null)
      return;
    this._UpdateTable.Clear();
    this._UpdateTable = (Dictionary<WM, List<SystemParameters2._SystemMetricUpdate>>) null;
  }
