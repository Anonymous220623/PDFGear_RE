...
// Type: Syncfusion.Windows.TaskbarItemInfo
...
public sealed class TaskbarItemInfo : Freezable
{
  ...
  {
    return margin.Left < 0.0 || margin.Right < 0.0 || margin.Top < 0.0 || margin.Bottom < 0.0 ? TaskbarItemInfo._EmptyThickness : margin;
  }
    ...
    else
    {
      switch (wm)
      {
        case WM.SIZE:
          ...
          break;
        case WM.COMMAND:
          if (Utility.HIWORD((int) wParam.ToInt64()) == 6144)
          {
            this.ThumbButtonInfos[Utility.LOWORD((int) wParam.ToInt64())].InvokeClick();
            handled = true;
            break;
          }
          break;
  ...
  {
    return !attached || this.ProgressState == TaskbarItemProgressState.None || this.ProgressState == TaskbarItemProgressState.Indeterminate ? HRESULT.S_OK : this._taskbarList.SetProgressValue(this._hwndSource.Handle, (ulong) (this.ProgressValue * 1000.0), 1000UL);
  }
  ...
  private HRESULT _UpdateThumbButtons(bool attached)
  {
    ...
    try
    {
        ...
        foreach (ThumbButtonInfo thumbButtonInfo in (FreezableCollection<ThumbButtonInfo>) thumbButtonInfos)
        {
          ...
          switch (thumbButtonInfo.Visibility)
          {
            ...
            default:
              ...
              {
                ref THUMBBUTTON local = ref thumbbutton;
                local.dwFlags = local.dwFlags;
              }
          ...
          pButtons[(IntPtr) index] = thumbbutton;
          ++index;
          if (index == 7U)
            break;
        }
      ...
      for (; index < 7U; ++index)
        pButtons[(IntPtr) index] = new THUMBBUTTON()
        {
          iId = index,
          dwFlags = THBF.DISABLED | THBF.NOBACKGROUND | THBF.HIDDEN,
          dwMask = THB.ICON | THB.TOOLTIP | THB.FLAGS
