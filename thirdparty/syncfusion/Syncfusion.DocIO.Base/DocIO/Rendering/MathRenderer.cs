...
// Type: Syncfusion.DocIO.Rendering.MathRenderer
...
internal class MathRenderer
{
  ...
  internal MathRenderer(DrawingContext drawingContext) => this.m_drawingContext = drawingContext;

  internal DrawingContext DrawingContext => this.m_drawingContext;
  ...
  internal void Draw(LayoutedLineWidget lineWidget)
  {
    if (lineWidget.Skip)
      return;
    this.DrawingContext.Graphics.DrawLine(new Pen(lineWidget.Color, lineWidget.Width), lineWidget.Point1, lineWidget.Point2);
  }
    ...
    float scalingFactor)
  {
    if ((double) scalingFactor != 1.0)
    {
      ...
      this.DrawingContext.Graphics.Transform = matrix;
    }
    SolidBrush solidBrush = new SolidBrush(this.DrawingContext.GetTextColor(characterFormat));
    StringFormat stringFormt = this.DrawingContext.StringFormt;
    this.DrawingContext.Graphics.DrawString(stringWidget.Text, stringWidget.Font, (Brush) solidBrush, stringWidget.Bounds, stringFormt);
    if ((double) scalingFactor == 1.0)
      return;
    this.DrawingContext.ResetTransform();
  }

  internal void Dispose() => this.m_drawingContext = (DrawingContext) null;
}
