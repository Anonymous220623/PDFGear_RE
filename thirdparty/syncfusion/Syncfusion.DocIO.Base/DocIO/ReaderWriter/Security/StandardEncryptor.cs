...
// Type: Syncfusion.DocIO.ReaderWriter.Security.StandardEncryptor
...
{
  private const int KeyLength = 16 /*0x10*/;
    ...
    using (ICompoundStorage storage1 = dataSpaces.CreateStorage("TransformInfo"))
    {
      using (ICompoundStorage storage2 = storage1.CreateStorage("StrongEncryptionTransform"))
      {
        using (CompoundStream stream = storage2.CreateStream("\u0006Primary"))
        {
          ...
          new EncryptionTransformInfo() { Name = string.Empty }.Serialize((Stream) stream);
        }
      }
    }
  }
  ...
  private byte[] PrepareEncryptionInfo(ICompoundStorage root, string password)
  {
    ...
    byte[] key = this.m_securityHelper.CreateKey(password, salt1, 16 /*0x10*/);
    ...
    {
      StandardEncryptionInfo standardEncryptionInfo = new StandardEncryptionInfo();
      standardEncryptionInfo.VersionInfo = 131075 /*0x020003*/;
      standardEncryptionInfo.Flags = 36;
      EncryptionHeader header = standardEncryptionInfo.Header;
      header.Flags = 36;
      ...
      EncryptionVerifier verifier = standardEncryptionInfo.Verifier;
      verifier.Salt = salt1;
      ...
      if (num != 0)
        numArray = this.m_securityHelper.CombineArray(numArray, new byte[16 /*0x10*/ - num]);
      verifier.EncryptedVerifierHash = this.Encrypt(numArray, key);
      standardEncryptionInfo.Serialize((Stream) stream);
    }
    return key;
  }
  ...
  private byte[] Encrypt(byte[] data, byte[] key)
  {
    ...
    return this.m_securityHelper.EncryptDecrypt(data, new SecurityHelper.EncryptionMethod(aes.Cipher), key.Length);
  }
