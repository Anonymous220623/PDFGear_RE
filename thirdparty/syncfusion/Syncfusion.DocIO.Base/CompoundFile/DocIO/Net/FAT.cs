...
// Type: Syncfusion.CompoundFile.DocIO.Net.FAT
...
internal class FAT
{
  ...
  public FAT(Syncfusion.CompoundFile.DocIO.Net.CompoundFile file, Stream stream, DIF dif, FileHeader header)
  {
    if (file == null)
  ...
  public byte[] GetStream(Stream stream, int firstSector, Syncfusion.CompoundFile.DocIO.Net.CompoundFile file)
  {
    if (stream == null)
      throw new ArgumentNullException(nameof (stream));
    if (file == null)
  ...
  public void Write(Stream stream, DIF dif, FileHeader header)
  {
    ...
    for (; index < num2; ++index)
    {
      ...
      long sectorOffset = Syncfusion.CompoundFile.DocIO.Net.CompoundFile.GetSectorOffset(sectorIds[index], sectorShift);
  ...
  {
    return Syncfusion.CompoundFile.DocIO.Net.CompoundFile.GetSectorOffset(sectorIndex, this.m_usSectorShift, this.m_iHeaderSize);
  }
