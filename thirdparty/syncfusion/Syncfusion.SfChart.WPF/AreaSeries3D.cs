...
// Type: Syncfusion.UI.Xaml.Charts.AreaSeries3D
...
public class AreaSeries3D : XyDataSeries3D
{
  private Point hitPoint = new Point();

  public AreaSeries3D() => this.IsAnimated = false;
  ...
  public override void CreateSegments()
  {
    ...
    if (flag)
    {
      ...
      {
        this.Segment = new AreaSegment3D(xValues, this.GroupedSeriesYValues[0], segmentDepth.Start, segmentDepth.End, this);
    ...
    else
    {
      ...
      else if (xValues != null)
      {
        ...
        {
          this.Segment = new AreaSegment3D(xValues, this.YValues, segmentDepth.Start, segmentDepth.End, this);
    ...
    out List<List<double>> xValList)
  {
    ...
    DoubleRange segmentDepth = this.GetSegmentDepth(this.Area.ActualDepth);
    ...
    if (this.Segment == null || this.Segments.Count == 0)
    {
      for (int index2 = 0; index2 < yValList.Count && index2 < xValList.Count; ++index2)
      {
        ...
        {
          this.Segment = new AreaSegment3D(xValList[index2], (IList<double>) yValList[index2], segmentDepth.Start, segmentDepth.End, this);
  ...
  internal override void Animate()
  {
    ...
    foreach (AreaSegment3D segment in (Collection<ChartSegment>) this.Segments)
    {
      ...
      splineDoubleKeyFrame1.Value = this.ActualYAxis.VisibleRange.Start;
      ...
      splineDoubleKeyFrame2.Value = this.Area.InternalSecondaryAxis.VisibleRange.End;
      ...
      keyFrame3.Value = this.YValues.Max();
      keyFrame3.KeyTime = keyFrame3.KeyTime.GetKeyTime(this.AnimationDuration);
      keyFrame3.EasingFunction = (IEasingFunction) new CircleEase();
      ...
      Storyboard.SetTargetProperty((DependencyObject) element, new PropertyPath((object) AreaSegment3D.YProperty));
      Storyboard.SetTarget((DependencyObject) element, (DependencyObject) segment);
  ...
  protected override DependencyObject CloneSeries(DependencyObject obj)
  {
    ...
    areaSeries3D.SegmentSelectionBrush = this.SegmentSelectionBrush;
    areaSeries3D.SelectedIndex = this.SelectedIndex;
    return base.CloneSeries((DependencyObject) areaSeries3D);
  }
  ...
  private void UpdateToolTip(object source, Point pos)
  {
    ...
    object obj2 = !(source is Shape shape) || !(shape.Tag is ChartSegment3D) ? (object) this.Segments[0] : shape.Tag;
    if (this.Area.SeriesClipRect.Contains(this.mousePos))
    {
      ...
      if (double.IsNaN(x))
        return;
      obj1 = this.ActualData[!(this.ActualXAxis is CategoryAxis3D) || (this.ActualXAxis as CategoryAxis3D).IsIndexed ? this.GetXValues().IndexOf(x) : (int) this.GroupedXValuesIndexes[(int) x]];
    }
    if (this.Area.Tooltip == null)
      this.Area.Tooltip = new ChartTooltip();
    ChartTooltip element = this.Area.Tooltip;
    AreaSegment3D areaSegment3D = obj2 as AreaSegment3D;
    areaSegment3D.Item = obj1;
    areaSegment3D.XData = x;
    areaSegment3D.YData = y;
    if (element == null)
      return;
    element.ClearValue(ContentControl.ContentProperty);
    element.ClearValue(ContentControl.ContentTemplateProperty);
    element.PolygonPath = " ";
    this.ToolTipTag = (object) areaSegment3D;
    if (adorningCanvas.Children.Count == 0 || adorningCanvas.Children.Count > 0 && !this.IsTooltipAvailable(adorningCanvas))
    {
      element.DataContext = (object) areaSegment3D;
