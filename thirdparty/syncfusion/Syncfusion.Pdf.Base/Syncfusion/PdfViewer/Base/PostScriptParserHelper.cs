...
// Type: Syncfusion.PdfViewer.Base.PostScriptParserHelper
...
internal static class PostScriptParserHelper
{
  ...
  static PostScriptParserHelper()
  {
    for (int index = 0; index < 10; ++index)
      PostScriptParserHelper.charValues[48 /*0x30*/ + index] = index;
    for (int index = 0; index < 6; ++index)
    {
      PostScriptParserHelper.charValues[97 + index] = 10 + index;
      PostScriptParserHelper.charValues[65 + index] = 10 + index;
    }
  }
  ...
  {
    PostScriptParserHelper.SkipWhiteSpaces(reader);
    while (!reader.EndOfFile && reader.Peek(0) == (byte) 37)
    {
      PostScriptParserHelper.GoToNextLine(reader);
      PostScriptParserHelper.SkipWhiteSpaces(reader);
    }
  }
  ...
  {
    while (!reader.EndOfFile && Chars.IsWhiteSpace((int) reader.Peek(0)))
    {
  ...
  public static string ReadName(IPostScriptParser reader)
  {
    StringBuilder stringBuilder = new StringBuilder();
    int num = (int) reader.Read();
    while (!reader.EndOfFile && !Chars.IsDelimiter((int) reader.Peek(0)))
    {
      char ch = (char) reader.Read();
      if (ch == '#')
      {
        string result;
        if (PostScriptParserHelper.TryReadHexChar(reader, out result))
        {
  ...
  {
    StringBuilder stringBuilder = new StringBuilder();
    while (!reader.EndOfFile && !Chars.IsDelimiter((int) reader.Peek(0)) && !char.IsNumber((char) reader.Peek(0)))
      stringBuilder.Append((char) reader.Read());
    return stringBuilder.ToString();
  }
  ...
  public static byte[] ReadHexadecimalString(IPostScriptParser reader)
  {
    ...
    {
      if (Chars.IsHexChar((int) reader.Peek(0)))
      {
        stringBuilder.Append((char) reader.Read());
      }
      else
    ...
    return PostScriptParserHelper.GetBytesFromHexString(stringBuilder.ToString());
  }
  ...
  public static byte[] ReadLiteralString(IPostScriptParser reader)
  {
    ...
    while (num2 > 0)
    {
      ...
      {
        if (PostScriptParserHelper.IsValidEscape((int) reader.Peek(1)))
        {
          int num3 = (int) reader.Read();
          if (reader.Peek(0) != (byte) 13 && reader.Peek(0) != (byte) 10)
            byteList1.Add((byte) PostScriptParserHelper.GetSymbolFromEscapeSymbol((int) reader.Read()));
          else
            PostScriptParserHelper.SkipWhiteSpaces(reader);
        }
        else if (Chars.IsOctalChar((int) reader.Peek(1)))
        {
          int num4 = 3;
          int num5 = (int) reader.Read();
          List<byte> byteList2 = new List<byte>();
          for (; Chars.IsOctalChar((int) reader.Peek(0)) && num4 > 0; --num4)
  ...
  public static byte[] GetBytesFromHexString(string hexString)
  {
    ...
    for (int length = hexString.Length; index < length; index += 2)
      bytesFromHexString[index >> 1] = (byte) (PostScriptParserHelper.charValues[(int) hexString[index]] << 4 | PostScriptParserHelper.charValues[(int) hexString[index + 1]]);
  ...
  private static bool TryReadHexChar(IPostScriptParser reader, out string result)
  {
    ...
    for (int index = 0; index < 2; ++index)
    {
      ...
      if (reader.EndOfFile || Chars.IsDelimiter((int) b) || !Chars.IsHexChar((int) b))
      {
    ...
    char ch = (char) PostScriptParserHelper.GetBytesFromHexString(stringBuilder.ToString())[0];
    result = ch.ToString();
    return true;
  }
}
