...
// Type: Syncfusion.Pdf.Xfa.PdfXfaCircleField
...
public class PdfXfaCircleField : PdfXfaField
{
  ...
  internal void Save(XfaWriter xfaWriter)
  {
    if (this.Name == "" || this.Name == string.Empty)
      this.Name = "Circle" + xfaWriter.m_fieldCount++.ToString();
    ...
    switch (this.Appearance)
    {
      ...
      case PdfXfaCircleAppearance.Arc:
        xfaWriter.Write.WriteAttributeString("sweepAngle", this.SweepAngle.ToString());
        xfaWriter.Write.WriteAttributeString("startAngle", this.StartAngle.ToString());
        break;
    }
  ...
  internal void SaveAcroForm(PdfPage page, RectangleF bounds)
  {
    ...
    if (this.Appearance == PdfXfaCircleAppearance.Arc)
      graphics.DrawArc(pen, rectangleF2, (float) this.m_startAngle, (float) this.m_sweepAngle);
    else
