...
public class PdfStandardFont : PdfFont
{
    ...
    : base(size, style)
  {
    this.m_fontFamily = fontFamily;
    this.CheckStyle();
    this.InitializeInternals(isLicense);
  }
  ...
  ~PdfStandardFont()
  {
    if (this.charWidthDictionary == null)
      return;
    this.charWidthDictionary = (Dictionary<char, float>) null;
  }
  ...
  {
    this.fontEncoding = textEncoding;
    if (PdfDocument.Cache.Search((IPdfCache) this) != null)
      ((IPdfCache) this).SetInternals((IPdfPrimitive) this.CreateInternals());
    this.dictionary.BeginSave += new SavePdfPrimitiveEventHandler(this.FontBeginSave);
  }
  ...
  protected internal override float GetLineWidth(string line, PdfStringFormat format)
  {
    ...
    {
      line = this.fontEncoding == null ? PdfStandardFont.Convert(line) : PdfStandardFont.Convert(line, this.fontEncoding);
      int index = 0;
      for (int length = line.Length; index < length; ++index)
      {
        char ch = line[index];
        if (this.charWidthDictionary.ContainsKey(ch))
        {
          num += this.charWidthDictionary[ch];
        }
        else
        {
          float charWidthInternal = this.GetCharWidthInternal(ch, format);
          this.charWidthDictionary.Add(ch, charWidthInternal);
  ...
  protected override bool EqualsToFont(PdfFont font)
  {
    ...
    {
      bool flag1 = this.FontFamily == pdfStandardFont.FontFamily;
      bool flag2 = (this.Style & ~(PdfFontStyle.Underline | PdfFontStyle.Strikeout)) == (pdfStandardFont.Style & ~(PdfFontStyle.Underline | PdfFontStyle.Strikeout));
      font1 = flag1 && flag2;
    }
    return font1;
  }

  private void InitializeInternals() => this.InitializeInternals(false);
  ...
  {
    this.dictionary["Type"] = (IPdfPrimitive) new PdfName("Font");
    this.dictionary["Subtype"] = (IPdfPrimitive) new PdfName("Type1");
    this.dictionary["BaseFont"] = (IPdfPrimitive) new PdfName(this.Metrics.PostScriptName);
    if (this.FontFamily != PdfFontFamily.Symbol && this.FontFamily != PdfFontFamily.ZapfDingbats)
      this.dictionary["Encoding"] = (IPdfPrimitive) new PdfName(FontEncoding.WinAnsiEncoding.ToString());
  ...
  private Dictionary<int, string> CharacterMapTable
  {
    get
    {
      if (this.unicodeToNames == null)
      {
        this.unicodeToNames = new Dictionary<int, string>();
        string[] strArray = this.unicodeNames.Split(',');
        for (int index = 0; index < this.charcode.Length; ++index)
          this.unicodeToNames[this.charcode[index]] = strArray[index];
      }
      return this.unicodeToNames;
    }
  }
  ...
  private Dictionary<string, int> FindWidth
  {
    get
    {
      if (this.widthTableUpdate == null)
      {
        this.widthTableUpdate = new Dictionary<string, int>();
        string[] strArray = this.characterNames.Split(',');
        int[] standardWidth = this.FindStandardWidth();
        for (int index = 0; index < standardWidth.Length; ++index)
          this.widthTableUpdate[strArray[index]] = standardWidth[index];
      }
      return this.widthTableUpdate;
    }
  }

  private int[] FindStandardWidth()
  {
    int[] standardWidth = new int[0];
    if (this.FontFamily == PdfFontFamily.Helvetica)
      standardWidth = (this.Style & PdfFontStyle.Bold) > PdfFontStyle.Regular && (this.Style & PdfFontStyle.Italic) > PdfFontStyle.Regular || (this.Style & PdfFontStyle.Bold) > PdfFontStyle.Regular ? this.heveticaBold : this.hevetica;
    else if (this.FontFamily == PdfFontFamily.Courier)
      standardWidth = this.courier;
    else if (this.FontFamily == PdfFontFamily.TimesRoman)
      standardWidth = (this.Style & PdfFontStyle.Bold) <= PdfFontStyle.Regular || (this.Style & PdfFontStyle.Italic) <= PdfFontStyle.Regular ? ((this.Style & PdfFontStyle.Bold) <= PdfFontStyle.Regular ? ((this.Style & PdfFontStyle.Italic) <= PdfFontStyle.Regular ? this.timeRoman : this.timesItaic) : this.timesBold) : this.timesBoldItalic;
    else if (this.FontFamily == PdfFontFamily.Symbol)
      standardWidth = this.symbol;
    else if (this.FontFamily == PdfFontFamily.ZapfDingbats)
      standardWidth = this.zapfDingbat;
    return standardWidth;
  }

  private void FontBeginSave(object sender, SavePdfPrimitiveEventArgs ars)
  {
    if (this.fontEncoding == null || this.FontFamily == PdfFontFamily.Symbol || this.FontFamily == PdfFontFamily.ZapfDingbats)
      return;
    if (this.widthTableUpdate == null)
      this.UpdateWidthTable();
    List<int> usedChars = PdfStandardFont.usedChars;
    usedChars.Sort();
    short[] numArray = new short[256 /*0x0100*/];
    for (int index = 0; index < usedChars.Count; ++index)
      numArray[usedChars[index] & (int) byte.MaxValue] = (short) 1;
    int index1 = 0;
    while (index1 < 256 /*0x0100*/ && numArray[index1] == (short) 0)
      ++index1;
    int maxValue = (int) byte.MaxValue;
    while (maxValue >= index1 && numArray[maxValue] == (short) 0)
      --maxValue;
    if (index1 > (int) byte.MaxValue)
    {
      index1 = (int) byte.MaxValue;
      maxValue = (int) byte.MaxValue;
    }
    PdfDictionary pdfDictionary = new PdfDictionary();
    pdfDictionary["Type"] = (IPdfPrimitive) new PdfName("Encoding");
    PdfArray pdfArray1 = new PdfArray();
    for (int index2 = index1; index2 <= maxValue; ++index2)
    {
      string empty = string.Empty;
      string str = this.fontEncoding.GetString(new byte[1]
      {
        (byte) index2
      }, 0, 1);
      char key = '?';
      if (str.Length > 0)
        key = str.ToCharArray()[0];
      this.CharacterMapTable.TryGetValue((int) key, out empty);
      if (empty != null)
      {
        index1 = index2;
        break;
    ...
    bool flag = true;
    for (int index3 = index1; index3 <= maxValue; ++index3)
    {
      if (numArray[index3] != (short) 0)
      {
        if (flag)
        {
          pdfArray1.Add((IPdfPrimitive) new PdfNumber(index3));
          flag = false;
        }
        string empty = string.Empty;
        string str = this.fontEncoding.GetString(new byte[1]
        ...
        this.CharacterMapTable.TryGetValue((int) key, out empty);
        if (empty != null)
          pdfArray1.Add((IPdfPrimitive) new PdfName(empty));
        else
          pdfArray1.Add((IPdfPrimitive) new PdfName(".notdef"));
      }
      else
        flag = true;
    }
    pdfDictionary["Differences"] = (IPdfPrimitive) pdfArray1;
    this.dictionary["Encoding"] = (IPdfPrimitive) pdfDictionary;
    this.dictionary["FirstChar"] = (IPdfPrimitive) new PdfNumber(index1);
    this.dictionary["LastChar"] = (IPdfPrimitive) new PdfNumber(maxValue);
    PdfArray pdfArray2 = new PdfArray();
    for (int index4 = index1; index4 <= maxValue; ++index4)
    {
      int num1 = 0;
      int index5 = index4;
      if (this.Metrics != null && this.Metrics.WidthTable != null && index5 > -1 && usedChars.Contains(index4) && this.Metrics.WidthTable.ToArray().Count > index5)
      {
        int num2 = this.Metrics.WidthTable[index5];
        pdfArray2.Add((IPdfPrimitive) new PdfNumber(num2));
      }
      else
        pdfArray2.Add((IPdfPrimitive) new PdfNumber(num1));
    }
    this.dictionary["Widths"] = (IPdfPrimitive) pdfArray2;
  }
  ...
  {
    int num = !Enum.IsDefined(typeof (PdfFontFamily), (object) this.Name) ? (int) charCode : (int) charCode - 32 /*0x20*/;
    int index = num < 0 || num == 128 /*0x80*/ ? 0 : num;
    if (this.fontEncoding != null && this.widthTableUpdate == null)
      this.UpdateWidthTable();
    WidthTable widthTable = this.Metrics.WidthTable;
    return this.fontEncoding == null ? (float) widthTable[index] : (float) widthTable[(int) charCode];
  }
  ...
  {
    byte[] numArray = text != null ? encoding.GetBytes(text) : throw new ArgumentNullException(nameof (text));
    int length = numArray.Length;
    char[] chArray = new char[length];
    for (int index = 0; index < length; ++index)
    {
      chArray[index] = (char) numArray[index];
      if (!PdfStandardFont.usedChars.Contains((int) numArray[index]))
        PdfStandardFont.usedChars.Add((int) numArray[index]);
    }
    return new string(chArray);
  }
  ...
  {
    int[] widths = new int[256 /*0x0100*/];
    string empty = string.Empty;
    byte[] bytes = new byte[1];
    for (int index = 0; index < 256 /*0x0100*/; ++index)
    {
      bytes[0] = (byte) index;
      string str = this.fontEncoding.GetString(bytes, 0, 1);
      string key;
      this.CharacterMapTable.TryGetValue(str.Length <= 0 ? (int) '?' : (int) str[0], out key);
      if (key != null && this.FindWidth.ContainsKey(key))
        widths[index] = this.FindWidth[key];
    }
    this.Metrics.WidthTable = (WidthTable) new StandardWidthTable(widths);
  }
}
