...
// Type: Syncfusion.Pdf.Parsing.SystemFontOperatorDescriptor
...
internal class SystemFontOperatorDescriptor
{
  ...
  {
    if (!(obj is SystemFontOperatorDescriptor operatorDescriptor) || this.value.Length != operatorDescriptor.value.Length)
      return false;
    for (int index = 0; index < this.value.Length; ++index)
    {
