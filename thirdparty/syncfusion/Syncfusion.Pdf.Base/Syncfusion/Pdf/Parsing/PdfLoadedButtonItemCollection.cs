...
public class PdfLoadedButtonItemCollection : PdfCollection
{
  ...
  {
    PdfLoadedButtonItemCollection buttonItemCollection = new PdfLoadedButtonItemCollection();
    for (int index = 0; index < this.List.Count; ++index)
    {
      if (this.List[index] is PdfLoadedButtonItem)
        buttonItemCollection.Add((this.List[index] as PdfLoadedButtonItem).Clone());
    }
