...
// Type: Syncfusion.Pdf.Interactive.PdfLoadedSquareAnnotation
...
public class PdfLoadedSquareAnnotation : PdfLoadedStyledAnnotation
{
  ...
  internal PdfLoadedSquareAnnotation(
  ...
  private RectangleF ObtainStyle(PdfPen mBorderPen, RectangleF rectangle, float borderWidth)
  {
    ...
    {
      this.m_dashedArray = pdfDictionary["D"] as PdfArray;
      if (this.m_dashedArray != null)
      {
        float[] numArray = new float[this.m_dashedArray.Count];
        bool flag = false;
        for (int index = 0; index < this.m_dashedArray.Count; ++index)
        {
          numArray[index] = (this.m_dashedArray.Elements[index] as PdfNumber).FloatValue;
