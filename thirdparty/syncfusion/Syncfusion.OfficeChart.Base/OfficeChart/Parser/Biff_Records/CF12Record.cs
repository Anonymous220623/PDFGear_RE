...
// Type: Syncfusion.OfficeChart.Parser.Biff_Records.CF12Record
...
internal class CF12Record : BiffRecordRaw
{
  ...
  public override int GetStoreSize(OfficeVersion version)
  {
    ...
    if (this.FormatType == ExcelCFType.ColorScale)
      storeSize = (this.m_colorScale.ListCFInterpolationCurve.Count != 0 ? storeSize + this.m_colorScale.GetStoreSize(version) : storeSize + (int) this.m_colorScale.DefaultRecordSize) + DVRecord.GetFormulaSize(this.m_arrFormulaParsed, version, true);
    if (this.FormatType == ExcelCFType.DataBar)
      storeSize = storeSize + this.m_dataBar.GetStoreSize(version) + DVRecord.GetFormulaSize(this.m_arrFormulaParsed, version, true);
    if (this.FormatType == ExcelCFType.IconSet)
      storeSize = (this.m_iconSet.ListCFIconSet.Count != 0 ? storeSize + this.m_iconSet.GetStoreSize(version) : storeSize + (int) this.m_iconSet.DefaultRecordSize) + DVRecord.GetFormulaSize(this.m_arrFormulaParsed, version, true);
    if (this.FormatType == ExcelCFType.CellValue)
      storeSize = storeSize + (int) this.m_sizeOfDXF + DVRecord.GetFormulaSize(this.m_arrFirstFormulaParsed, version, true) + DVRecord.GetFormulaSize(this.m_arrSecondFormulaParsed, version, true);
    return storeSize;
  }
