...
// Type: Syncfusion.OfficeChart.Parser.Biff_Records.ObjRecords.ftLbsData
...
internal class ftLbsData : ObjSubRecord
{
  ...
  protected override void Parse(byte[] buffer)
  {
    ...
    if (int16_1 > 0)
    {
      ...
      this.m_arrFormula = FormulaUtil.ParseExpression((DataProvider) new ByteArrayDataProvider(numArray), int16_2, OfficeVersion.Excel97to2003);
  ...
  {
    int formulaSize = DVRecord.GetFormulaSize(this.m_arrFormula, OfficeVersion.Excel97to2003, false);
    if (formulaSize % 2 != 0)
      ++formulaSize;
    if (formulaSize > 0)
      formulaSize += 8;
  ...
  {
    byte[] data = this.m_arrFormula != null ? FormulaUtil.PtgArrayToByteArray(this.m_arrFormula, OfficeVersion.Excel97to2003) : new byte[0];
