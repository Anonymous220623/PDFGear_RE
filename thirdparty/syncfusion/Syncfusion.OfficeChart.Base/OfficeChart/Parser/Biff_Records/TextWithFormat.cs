...
// Type: Syncfusion.OfficeChart.Parser.Biff_Records.TextWithFormat
...
label_10:
  ...
  internal void RemoveAllInsideRange(int iStartPos, int iEndPos)
  {
    ...
    int num = iEndPos == -1 ? array.Length - 1 : Array.BinarySearch<int>(array, iEndPos);
  ...
  private void UpdateFontIndexes(Dictionary<int, int> dicFontIndexes)
  {
    ...
    for (int formattingRunsCount = this.FormattingRunsCount; index < formattingRunsCount; ++index)
      this.m_arrFormattingRuns[keys[index]] = FontImpl.UpdateFontIndexes(values[index], dicFontIndexes, OfficeParseOptions.Default);
  }
