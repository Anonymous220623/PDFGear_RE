...
// Type: Syncfusion.OfficeChart.Parser.Biff_Records.Formula.Ptg
...
internal abstract class Ptg : ICloneable
{
  ...
  public int CompareTo(Ptg token)
  {
    ...
    if (num == 0)
      num = this.GetSize(OfficeVersion.Excel2007) - token.GetSize(OfficeVersion.Excel2007);
    if (num == 0)
  ...
  protected int CompareContent(Ptg token)
  {
    ...
    return !BiffRecordRaw.CompareArrays(this.ToByteArray(OfficeVersion.Excel2007), token.ToByteArray(OfficeVersion.Excel2007)) ? 1 : 0;
  }
