...
// Type: Syncfusion.OfficeChart.Implementation.SSTDictionary
...
internal class SSTDictionary : IParseable, IDisposable
{
  ...
  private void CheckLength(object key)
  {
    ...
    if (this.m_book.Version == OfficeVersion.Excel97to2003 && num > (int) short.MaxValue)
  ...
  {
    if (this.newRefCount.Count > index)
      this.newRefCount[index] = count;
    else
      this.newRefCount.Add(count);
  }
