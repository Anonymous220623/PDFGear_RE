...
// Type: Syncfusion.OfficeChart.Implementation.Clipboard.DelimiterClipboardProvider
...
internal class DelimiterClipboardProvider : ClipboardProvider
{
  ...
  protected virtual void FillData(IRange range)
  {
    ...
    for (int row = range.Row; row <= lastRow; ++row)
    {
      ...
      {
        if (cellRecords.Contains(row, index))
          this.m_strCSVValue.Append(this.Worksheet.Range[row, index].Value);
        if (index != lastColumn)
  ...
  private void FillSheet(IWorksheet sheet, string sheetData)
  {
    ...
    while (num1 < sheetData.Length)
    {
      ...
      {
        sheet.Range[row, column].Value = sheetData.Substring(startIndex, num1 - startIndex);
