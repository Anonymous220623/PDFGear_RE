...
// Type: Syncfusion.OfficeChart.Implementation.Collections.ExtendedFormatsCollection
...
internal class ExtendedFormatsCollection(IApplication application, object parent) : 
  CollectionBaseEx<ExtendedFormatImpl>(application, parent)
{
  ...
  public ExtendedFormatImpl Add(ExtendedFormatImpl format)
  {
    ...
    if (this.m_hashFormats.ContainsKey(format))
    {
      ...
      if (this.ParentWorkbook.Version == OfficeVersion.Excel97to2003 && format.Index != 0 && format.Index < 15)
    ...
    {
      if (this.Count >= (this.Count > 0 ? this.ParentWorkbook : format.Workbook).MaxXFCount)
  ...
  public ExtendedFormatImpl GatherTwoFormats(int iFirstXF, int iEndXF)
  {
    ...
    {
      border1 = extendedFormatImpl2.Borders[OfficeBordersIndex.EdgeRight];
    ...
    else
    {
      ...
      border1 = extendedFormatImpl4.Borders[OfficeBordersIndex.EdgeRight];
      border2 = extendedFormatImpl4.Borders[OfficeBordersIndex.EdgeBottom];
    }
    IBorder border3 = extendedFormatImpl3.Borders[OfficeBordersIndex.EdgeRight];
    border3.ColorObject.CopyFrom(border1.ColorObject, true);
    border3.LineStyle = border1.LineStyle;
    IBorder border4 = extendedFormatImpl3.Borders[OfficeBordersIndex.EdgeBottom];
  ...
  public Dictionary<int, int> RemoveAt(int xfIndex)
  {
    ...
    for (int index = 0; index < count3; ++index)
    {
      ...
      if (extendedFormatImpl != null)
      {
        ...
        extendedFormatImpl.Index = (int) (ushort) num2;
      }
      else
        ++num1;
    }
    ...
    Dictionary<int, int> dicFormatIndexes)
  {
    ...
    ExtendedFormatImpl extendedFormatImpl = this.Add(format1);
