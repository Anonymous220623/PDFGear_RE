...
// Type: Syncfusion.OfficeChart.Implementation.Collections.CollectionBaseEx`1
...
internal class CollectionBaseEx<T> : 
  CollectionBase<T>,
  IList<T>,
  ICollection<T>,
  IEnumerable<T>,
  ...
  ICloneParent
{
  ...
  protected override void OnClear()
  {
    if (this.Clearing != null && !this.m_bSkipEvents)
      this.Clearing();
    CollectionBaseEx<T>.m_dictCollectionsMaxValues.Clear();
    base.OnClear();
  }
...
label_10:
  ...
  public static string GenerateDefaultName(ICollection<T> namesCollection, string strStart)
  {
    ...
    lock (CollectionBaseEx<T>.m_threadLocker)
    {
      if (CollectionBaseEx<T>.m_dictCollectionsMaxValues.ContainsKey(strStart))
      {
        val2 = CollectionBaseEx<T>.m_dictCollectionsMaxValues[strStart];
        ++val2;
        CollectionBaseEx<T>.m_dictCollectionsMaxValues[strStart] = val2;
      }
      else
      {
        ...
        CollectionBaseEx<T>.m_dictCollectionsMaxValues.Add(strStart, val2);
      }
    }
  ...
  public static string GenerateDefaultName(string strStart, params ICollection[] arrCollections)
  {
    ...
    for (int length2 = arrCollections.Length; index < length2; ++index)
    {
      foreach (object obj in (IEnumerable) arrCollections[index])
      {
        ...
        double result;
        if (str.StartsWith(strStart) && double.TryParse(str.Substring(length1, str.Length - length1), NumberStyles.Integer, (IFormatProvider) null, out result))
          val2 = Math.Max((int) result + 1, val2);
      }
    }
    return strStart + val2.ToString();
