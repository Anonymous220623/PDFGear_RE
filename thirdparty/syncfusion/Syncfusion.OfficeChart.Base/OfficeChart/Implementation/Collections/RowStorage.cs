...
// Type: Syncfusion.OfficeChart.Implementation.Collections.RowStorage
...
internal class RowStorage : IDisposable, IBiffStorage, ICloneable, IOutline
{
  ...
  public int CellPositionSize => this.m_version != OfficeVersion.Excel97to2003 ? 8 : 4;

  public OfficeVersion Version => this.m_version;
  ...
  public object Clone(IntPtr heapHandle) => (object) null;
  ...
  public List<long> Find(
    ...
    WorkbookImpl book)
  {
    return this.Find(iFirstColumn, iLastColumn, findValue, flags, OfficeFindOptions.None, iErrorCode, bIsFindFirst, book);
  }
  ...
  public List<long> Find(
    ...
    WorkbookImpl book)
  {
    bool flag1 = (flags & OfficeFindType.Text) == OfficeFindType.Text;
      ...
      if (column <= iLastColumn)
      {
        switch (tbiffRecord1)
        {
          case TBIFFRecord.Formula:
            if (flag3)
            {
              FormulaRecord record = (FormulaRecord) BiffRecordFactory.GetRecord(this.m_dataProvider, index, this.Version);
              string first = '='.ToString() + book.FormulaUtil.ParseFormulaRecord(record);
              flag5 = findOptions != OfficeFindOptions.None ? this.CheckStringValue(first, findValue, findOptions, book) : first.ToLower().Contains(findValue.ToLower());
            }
            ...
            if (iOffset < this.m_iUsedSize)
            {
              ...
              if (tbiffRecord2 == TBIFFRecord.String)
              {
                ...
                if (flag4)
                {
                  StringRecord record = (StringRecord) BiffRecordFactory.GetRecord(this.m_dataProvider, iOffset, this.Version);
                  flag5 = findOptions != OfficeFindOptions.None ? this.CheckStringValue(record.Value, findValue, findOptions, book) : record.Value == findValue;
                  break;
                }
                break;
              }
          ...
          case TBIFFRecord.LabelSST:
            ...
            flag5 = findOptions != OfficeFindOptions.None ? this.CheckStringValue(text, findValue, findOptions, book) : text.ToLower().Contains(findValue.ToLower());
            break;
          case TBIFFRecord.Label:
            if (flag1)
            {
              LabelRecord record = (LabelRecord) BiffRecordFactory.GetRecord(this.m_dataProvider, index, this.Version);
              flag5 = findOptions != OfficeFindOptions.None ? this.CheckStringValue(record.Label, findValue, findOptions, book) : record.Label == findValue;
              break;
            }
            break;
          case TBIFFRecord.BoolErr:
            if (flag2)
            {
              ...
              flag5 = record.IsErrorCode && (int) record.BoolOrError == iErrorCode;
              break;
            }
            break;
        }
  ...
  private bool CheckStringValue(
    string first,
    string second,
    OfficeFindOptions options,
    WorkbookImpl book)
  {
    StringComparison comparisonType = (options & OfficeFindOptions.MatchCase) != OfficeFindOptions.None ? StringComparison.CurrentCulture : StringComparison.CurrentCultureIgnoreCase;
    bool flag = (options & OfficeFindOptions.MatchEntireCellContent) != OfficeFindOptions.None;
    if (!book.IsStartsOrEndsWith.HasValue || flag)
      return !flag ? first.IndexOf(second, 0, comparisonType) == 0 : first == second;
  ...
  public List<long> Find(
    ...
    WorkbookImpl book)
  {
    bool flag1 = (flags & OfficeFindType.FormulaValue) != (OfficeFindType) 0;
    bool flag2 = (flags & OfficeFindType.Number) != (OfficeFindType) 0;
    if (!flag1 && !flag2)
      throw new ArgumentException("Parameter flags is not valid.", nameof (flags));
    List<long> longList = new List<long>();
    if (this.m_dataProvider == null || this.m_iUsedSize <= 0 || iLastColumn < this.m_iFirstColumn || iFirstColumn > this.m_iLastColumn)
    ...
    for (int index = this.LocateRecord(iFirstColumn, out bFound); index < this.m_iUsedSize; index = this.MoveNext(index))
    {
      ...
      if (column <= iLastColumn)
      {
        switch (tbiffRecord1)
        {
          case TBIFFRecord.Formula:
            if (flag1)
              bFound = ((FormulaRecord) BiffRecordFactory.GetRecord(this.m_dataProvider, index, this.Version)).Value == findValue;
            int iOffset = index + num + 4;
            if (iOffset < this.m_iUsedSize)
            {
          ...
          case TBIFFRecord.RK:
            if (flag2)
            {
              bFound = ((IDoubleValue) BiffRecordFactory.GetRecord(this.m_dataProvider, index, this.Version)).DoubleValue == findValue;
              break;
            }
            break;
        }
  ...
  internal void CreateDataProvider()
  {
    if (this.m_dataProvider != null)
      return;
    this.m_dataProvider = ApplicationImpl.CreateDataProvider();
  ...
  private void AddNamedRangeTokens(Dictionary<int, object> result, Ptg[] tokens)
  {
    ...
    for (int index = tokens.Length - 1; index >= 0; --index)
    {
      if (tokens[index] is NamePtg token)
        result[(int) token.ExternNameIndex - 1] = (object) null;
    }
  }
      ...
      for (; iOffset1 < this.m_iUsedSize; iOffset1 = iOffset2 + num2)
      {
        ...
        if (tbiffRecord != TBIFFRecord.Array && tbiffRecord != TBIFFRecord.String)
        {
          ...
          if (num4 <= maxValue2 && num3 <= maxValue1)
          {
            ...
            {
              case TBIFFRecord.Formula:
                FormulaRecord record = (FormulaRecord) BiffRecordFactory.GetRecord(this.m_dataProvider, iOffset2 - 4, OfficeVersion.Excel2007);
                int storeSize = record.GetStoreSize(OfficeVersion.Excel97to2003);
                ...
                record.InfillInternalData(this.m_dataProvider, iOffset5, OfficeVersion.Excel97to2003);
        ...
        {
          BiffRecordRaw record = BiffRecordFactory.GetRecord(this.m_dataProvider, iOffset2 - 4, OfficeVersion.Excel2007);
          int storeSize = record.GetStoreSize(OfficeVersion.Excel97to2003);
          this.m_dataProvider.WriteInt16(num1, (short) tbiffRecord);
          int iOffset8 = num1 + 2;
          this.m_dataProvider.WriteInt16(iOffset8, (short) storeSize);
          int iOffset9 = iOffset8 + 2;
          record.InfillInternalData(this.m_dataProvider, iOffset9, OfficeVersion.Excel97to2003);
  ...
  private void ExtendDataStorage(int iBlockSize)
  {
    ...
    for (; iOffset1 < this.m_iUsedSize; iOffset1 = num + iLength1)
    {
      ...
      {
        BiffRecordRaw record = BiffRecordFactory.GetRecord(this.m_dataProvider, num - 4, OfficeVersion.Excel97to2003);
        ...
        record.InfillInternalData(dataProvider, iOffset4, OfficeVersion.Excel2007);
    ...
    for (; iOffset < this.m_iUsedSize; iOffset += num + 4)
    {
      ...
      switch (tbiffRecord)
      {
        case TBIFFRecord.Formula:
          ...
          enlargedDataSize += record1.GetStoreSize(OfficeVersion.Excel2007) + 4;
          break;
        case TBIFFRecord.MulRK:
        case TBIFFRecord.MulBlank:
        ...
        case TBIFFRecord.Array:
          ...
          enlargedDataSize += record2.GetStoreSize(OfficeVersion.Excel2007) + 4;
          break;
        default:
    ...
    for (int length = parsedExpression.Length; index < length; ++index)
    {
      ...
      switch (ptg)
      {
        case ISheetReference _:
          ...
          {
            string str = sourceBook.GetSheetNameByReference((int) refIndex1);
            if (dicSheetNames != null && dicSheetNames.ContainsKey(str))
              str = dicSheetNames[str];
            int num = destBook.AddSheetReference(str);
            sheetReference.RefIndex = (ushort) num;
            flag = true;
            break;
          ...
          {
            int refIndex2 = (int) nameXptg.RefIndex;
            int num = dictExternSheets.ContainsKey(refIndex2) ? dictExternSheets[refIndex2] : refIndex2;
            nameXptg.RefIndex = (ushort) num;
            flag = true;
            break;
          }
          break;
        case NamePtg namePtg:
          int key = (int) namePtg.ExternNameIndex - 1;
          int num1 = dicNameIndexes.ContainsKey(key) ? dicNameIndexes[key] : key;
          namePtg.ExternNameIndex = (ushort) (num1 + 1);
          flag = true;
          break;
      }
    }
    ...
    int iBlockSize)
  {
    ...
    OfficeVersion version = this.Version;
  ...
  public string GetFormulaStringValueByOffset(int iOffset)
  {
    ...
    if (iOffset >= this.m_iUsedSize)
      return (string) null;
    if (this.m_dataProvider.ReadInt16(iOffset) != (short) 519)
      return (string) null;
    iOffset += 4;
