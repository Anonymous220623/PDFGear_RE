...
// Type: Syncfusion.OfficeChart.Implementation.FormulaTokenizer
...
internal class FormulaTokenizer
{
  ...
  public void NextToken()
  {
    ...
    char ch = this.m_numberFormat.NumberDecimalSeparator[0];
    char c1 = ' ';
    char c2 = ' ';
    if (this.m_strFormula.IndexOf(this.m_chCurrent) > 0)
      c1 = this.m_strFormula[this.m_strFormula.IndexOf(this.m_chCurrent) - 1];
    if (this.m_strFormula.IndexOf(this.m_chCurrent) < this.m_strFormula.Length - 1)
      c2 = this.m_strFormula[this.m_strFormula.IndexOf(this.m_chCurrent) + 1];
    while (true)
    {
      this.m_iStartPos = this.m_iPos;
      if (char.IsDigit(this.m_chCurrent))
        this.ParseNumber();
      else if (this.m_chCurrent == '.' || (int) this.m_chCurrent == (int) ch && char.IsDigit(c1) && char.IsDigit(c2))
      {
        this.ParseNumber();
      }
      else
      {
        switch (this.m_chCurrent)
        {
          ...
          case '\'':
            ...
            {
              int num = this.CharOccurs(this.m_strFormula, '\'');
              if (num % 2 != 0 || num / this.CharOccurs(this.m_strFormula, '!', '\'') <= 2 && (this.CharOccurs(this.m_strFormula, '(') + this.CharOccurs(this.m_strFormula, ')')) % 2 != 0)
          ...
          case '{':
            ...
            break;
          default:
            if ((int) this.m_chCurrent == (int) this.m_chArgumentSeparator)
            {
              this.NextChar();
  ...
  {
    int num1 = 0;
    foreach (int num2 in stringToSearch.ToCharArray())
    {
      if (num2 == (int) charToFind)
        ++num1;
    }
    return num1;
  }
  ...
  private void ParseNumber()
  {
    ...
    this.AppendNumbers();
    if ((int) this.m_chCurrent == (int) this.m_numberFormat.NumberDecimalSeparator[0])
    {
  ...
  private void ParseIdentifier()
  {
    ...
    bool flag3 = false;
    int num2 = 0;
    while (char.IsLetterOrDigit(this.m_chCurrent) || this.m_chCurrent >= '\u0080' || this.m_chCurrent == '_' || this.m_chCurrent == '!' || this.m_chCurrent == ':' || this.m_chCurrent == '.' || this.m_chCurrent == '$' || this.m_chCurrent == '[' || this.m_chCurrent == '\'' || this.m_chCurrent == '#' || this.m_chCurrent == ']' || this.m_chCurrent == '?' || this.m_chCurrent == ' ' && flag3)
    {
      if (this.m_chCurrent == '!')
      {
        flag1 = true;
    ...
    string strA = this.m_value.ToString();
    if (strA.Contains(":") && !strA.EndsWith(":") && !strA.StartsWith(":"))
    {
      string[] strArray = strA.Split(':');
      string str1 = strArray[0];
      string str2 = strArray[1];
      char c1 = str1[str1.Length - 1];
      char c2 = str2[str2.Length - 1];
      if (!char.IsNumber(c2) && char.IsLetter(c2) && char.IsNumber(c1))
      {
        this.m_iPos = num2;
        this.m_chCurrent = ':';
        this.m_value.Remove(0, this.m_value.Length);
        this.m_value.Append(strArray[0]);
      }
    }
    if (this.TokenType == FormulaToken.DDELink)
      return;
    ...
    else if (string.Compare(strA, "true", StringComparison.CurrentCultureIgnoreCase) == 0)
      this.TokenType = FormulaToken.ValueTrue;
    else if (string.Compare(strA, "false", StringComparison.CurrentCultureIgnoreCase) == 0)
