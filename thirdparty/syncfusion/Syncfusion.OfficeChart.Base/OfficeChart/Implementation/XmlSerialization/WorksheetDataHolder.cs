...
// Type: Syncfusion.OfficeChart.Implementation.XmlSerialization.WorksheetDataHolder
...
internal class WorksheetDataHolder : IDisposable
{
  ...
  public void ParseConditionalFormatting(List<DxfImpl> dxfStyles, WorksheetImpl sheet)
  {
    ...
    if (reader1.LocalName == "root")
      reader1.Read();
    int num = reader1.LocalName == "conditionalFormattings" ? 1 : 0;
    sheet.m_parseCondtionalFormats = false;
    ...
    bool parseOnDemand)
  {
    ...
    else
    {
      ...
      sheet.ArchiveItemName = this.m_archiveItem.ItemName;
    ...
    Dictionary<string, object> dictItemsToRemove)
  {
    ...
    this.ParseDrawings(sheet, relation, dictItemsToRemove);
  }
    ...
    Dictionary<string, object> dictItemsToRemove)
  {
    ...
    parser.ParseDrawings(reader, sheet, path, lstRelationIds, dictItemsToRemove);
  ...
  private void SerializeChartsheetPart(ChartImpl chart)
  {
    ...
    this.SerializeRelations(relationCollection, str, (WorksheetDataHolder) null);
  }
    ...
    string relationType)
  {
    ...
    int num = 0;
    if (shapes.Count - sheet.VmlShapesCount - num <= 0 && !Excel2007Serializator.HasAlternateContent((IShapes) shapes))
      return false;
    MemoryStream newDataStream = new MemoryStream();
    StreamWriter data = new StreamWriter((Stream) newDataStream);
    XmlWriter writer = UtilityMethods.CreateWriter((TextWriter) data);
    ...
    if (this.m_drawingsRelation != null && this.m_drawingsRelation.Count > 0)
      this.SerializeRelations(this.m_drawingsRelation, drawingsName, (WorksheetDataHolder) null);
    return true;
  }
  ...
  private void SerializeVmlDrawings(WorksheetBaseImpl sheet)
  {
    ...
    this.SerializeRelations(relationCollection, vmlDrawingsName, (WorksheetDataHolder) null);
  }
  ...
  public void SerializeHeaderFooterImages(WorksheetBaseImpl sheet, RelationCollection relations)
  {
    ...
    {
      if (this.m_strVmlHFDrawingsId == null)
        return;
    ...
    else
    {
      ...
      this.SerializeRelations(this.m_hfDrawingsRelation, vmlDrawingsName, (WorksheetDataHolder) null);
    }
  }
