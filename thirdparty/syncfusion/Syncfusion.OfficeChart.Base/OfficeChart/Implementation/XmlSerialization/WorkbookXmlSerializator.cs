...
// Type: Syncfusion.OfficeChart.Implementation.XmlSerialization.WorkbookXmlSerializator
...
internal class WorkbookXmlSerializator : IXmlSerializator
{
  ...
  public static readonly string[] DEF_COMPARISION_OPERATORS_PREF = new string[9]
  {
    "None",
    "Between",
    "NotBetween",
  ...
  public static readonly string[] DEF_PRINT_LOCATION_STRING = new string[3]
  {
    "InPlace",
    "NoComments",
    "SheetEnd"
  };
  public static readonly string[] DEF_PRINT_ERROR_STRING = new string[4]
  {
    "none",
    "Blank",
    "Dash",
  ...
  public static readonly string[] DEF_VISIBLE_STRING = new string[3]
  {
    "error",
    "SheetHidden",
    "SheetVeryHidden"
  ...
  public static readonly string[] DEF_ERRORSTYLE = new string[3]
  {
    "Stop",
    "Warn",
    "Info"
  };
  public static readonly string[] DEF_ALLOWTYPE_STRING = new string[8]
  {
    "Any",
    "Whole",
    "Decimal",
  ...
  private void SerializeNames(XmlWriter writer, INames names, bool isLocal)
  {
    ...
    int num = names != null ? names.Count : throw new ArgumentNullException(nameof (names));
    if (num == 0)
      return;
    writer.WriteStartElement("ss", "Names", (string) null);
    for (int index = 0; index < num; ++index)
    {
      IName name = names[index];
  ...
  private void SerializeAlignment(XmlWriter writer, ExtendedFormatImpl format)
  {
    ...
    if (format.ReadingOrder != OfficeReadingOrderType.Context)
  ...
  private void SerializeInterior(XmlWriter writer, ExtendedFormatImpl format)
  {
    ...
    if (format.FillPattern != OfficePattern.None)
    {
  ...
  private void SerializeBorders(XmlWriter writer, IBorders borders)
  {
    ...
    OfficeBordersIndex[] officeBordersIndexArray = new OfficeBordersIndex[6]
    {
    ...
    int index = 0;
    for (int length = officeBordersIndexArray.Length; index < length; ++index)
    {
      OfficeBordersIndex Index = officeBordersIndexArray[index];
  ...
  private void SerializeCell(XmlWriter writer, WorksheetImpl sheet, int iRowIndex)
  {
    ...
    for (int lastColumn = sheet.LastColumn; firstColumn <= lastColumn; ++firstColumn)
    {
      ...
      bool flag = cells.Contains(cellIndex) && (mergedRegion == null || bMerge);
      if (flag || bMerge)
      {
        ...
        if (flag && cells.GetCellRecord(cellIndex).TypeCode != TBIFFRecord.Blank)
  ...
  private void SerializeWorksheetOption(XmlWriter writer, WorksheetImpl sheet)
  {
    ...
    if (sheet.Visibility != OfficeWorksheetVisibility.Visible)
    {
  ...
  private void SerializeLayout(XmlWriter writer, PageSetupImpl pageSetup)
  {
    if (writer == null)
      throw new ArgumentNullException(nameof (writer));
    OfficePageOrientation officePageOrientation = pageSetup != null ? pageSetup.Orientation : throw new ArgumentNullException(nameof (pageSetup));
    ...
    if (officePageOrientation != OfficePageOrientation.Portrait)
      writer.WriteAttributeString("x", "Orientation", (string) null, officePageOrientation.ToString());
  ...
  private void SerializePrint(XmlWriter writer, IPageSetup pageSetup)
  {
    ...
    if (pageSetup.PrintErrors != OfficePrintErrors.PrintErrorsDisplayed)
    {
  ...
  {
    return hAlign == OfficeHAlign.HAlignGeneral ? "Automatic" : hAlign.ToString().Remove(0, 6);
  }

  private string GetStyleVAlignString(OfficeVAlign vAlign) => vAlign.ToString().Remove(0, 6);
  ...
  private List<ExtendedFormatImpl> GetMergedList(IWorksheets sheets)
  {
    ...
    for (int count1 = sheets.Count; Index < count1; ++Index)
    {
      ...
      int index = 0;
      for (int count2 = mergedExtendedFormats.Count; index < count2; ++index)
      {
        ExtendedFormatImpl extendedFormatImpl = mergedExtendedFormats[index];
        Rectangle rectangle = mergeCells[index];
        long cellIndex = RangeImpl.GetCellIndex(rectangle.X + 1, rectangle.Y + 1);
