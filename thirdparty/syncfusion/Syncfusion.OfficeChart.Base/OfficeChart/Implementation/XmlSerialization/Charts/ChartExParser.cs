...
// Type: Syncfusion.OfficeChart.Implementation.XmlSerialization.Charts.ChartExParser
...
internal class ChartExParser(WorkbookImpl book) : ChartParser(book)
{
        ...
        if (reader.NodeType == XmlNodeType.Element)
        {
          switch (reader.LocalName)
          {
            case "f":
              ...
              continue;
            case "lvl":
              this.ParseChartExLevelElement(reader, cache, isCategoryValues, category, objectList);
              continue;
            default:
              reader.Skip();
              continue;
        ...
        if (reader.NodeType == XmlNodeType.Element)
        {
          switch (reader.LocalName)
          {
            case "pt":
              ...
              int num = this.AddNumericPoint(reader, list, isCategoryValues, ref formatCode);
              if (num != -1 && num > index)
              {
                do
                {
    ...
    RelationCollection relations)
  {
      ...
      if (reader.NodeType == XmlNodeType.Element)
      {
        switch (reader.LocalName)
        {
          ...
          case "legend":
            ...
            this.ParseLegend(reader, (IOfficeChartLegend) (chart.Legend as ChartLegendImpl), chart, relations);
            continue;
          case "title":
    ...
    if (flag)
    {
      ...
      ChartParserCommon.ParseChartTitleElement((Stream) memoryStream, chart.ChartTitleArea as IInternalOfficeChartTextArea, parentHolder, relations, 18f);
      chart.HasTitle = true;
    }
    reader.Read();
    ...
    out ChartFrameFormatImpl paretoLineFormat)
  {
    ...
    if (reader.MoveToAttribute("layoutId"))
    {
      ...
      else
      {
        ...
        serieType = (OfficeChartType) Enum.Parse(typeof (OfficeChartType), reader.Value.ToLowerInvariant(), true);
      }
      if (serieType != OfficeChartType.Pareto)
      {
        seriesFromAttributes = chart.Series.Add(serieType) as ChartSerieImpl;
        seriesFromAttributes.Number = -1;
        if (serieType == OfficeChartType.Histogram)
    ...
    if (reader.MoveToAttribute("ownerIdx") && seriesFromAttributes == null)
    {
      ...
      chart.Series[int32].SerieType = OfficeChartType.Pareto;
      paretoLineFormat = (chart.Series[int32] as ChartSerieImpl).ParetoLineFormat as ChartFrameFormatImpl;
    }
        ...
        if (reader.NodeType == XmlNodeType.Element)
        {
          switch (reader.LocalName)
          {
            case "parentLabelLayout":
              if (reader.MoveToAttribute("val"))
                serieFormat.TreeMapLabelOption = (TreeMapLabelOption) Enum.Parse(typeof (TreeMapLabelOption), reader.Value, true);
              reader.Skip();
              continue;
            case "visibility":
            ...
            case "statistics":
              ...
              {
                serieFormat.QuartileCalculationType = !(reader.Value == "inclusive") ? QuartileCalculation.ExclusiveMedian : QuartileCalculation.InclusiveMedian;
                continue;
              }
              continue;
            case "subtotals":
  ...
  internal void ParseChartDataLabelVisibility(XmlReader reader, ChartDataLabelsImpl dataLabels)
  {
    if (reader == null)
      throw new ArgumentNullException(nameof (reader));
    if (dataLabels == null)
