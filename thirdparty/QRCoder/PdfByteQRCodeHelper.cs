...
// Type: QRCoder.PdfByteQRCodeHelper
...
public static class PdfByteQRCodeHelper
{
    ...
    int requestedVersion = -1)
  {
    using (QRCodeGenerator qrCodeGenerator = new QRCodeGenerator())
    {
      ...
      {
        using (PdfByteQRCode pdfByteQrCode = new PdfByteQRCode(qrCode))
          return pdfByteQrCode.GetGraphic(pixelsPerModule, darkColorHtmlHex, lightColorHtmlHex);
      }
    }
  }
  ...
  public static byte[] GetQRCode(string txt, QRCodeGenerator.ECCLevel eccLevel, int size)
  {
    using (QRCodeGenerator qrCodeGenerator = new QRCodeGenerator())
    {
      ...
      {
        using (PdfByteQRCode pdfByteQrCode = new PdfByteQRCode(qrCode))
          return pdfByteQrCode.GetGraphic(size);
      }
    }
  }
}
