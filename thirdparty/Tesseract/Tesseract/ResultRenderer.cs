...
public abstract class ResultRenderer : DisposableBase, IResultRenderer, IDisposable
{
    ...
    foreach (int outputFormat in outputFormats)
    {
      switch (outputFormat)
      {
        ...
        case 2:
          if (renderers == null)
          {
            renderers = ResultRenderer.CreatePdfRenderer(outputbase, dataPath, false);
            continue;
          }
          TessApi.Native.ResultRendererInsert(((ResultRenderer) renderers).Handle, new PdfResultRenderer(outputbase, dataPath, false).Handle);
          continue;
        case 3:
          if (renderers == null)
          {
  ...
  public static IResultRenderer CreatePdfRenderer(
    ...
    bool textonly)
  {
    return (IResultRenderer) new PdfResultRenderer(outputFilename, fontDirectory, textonly);
  }
