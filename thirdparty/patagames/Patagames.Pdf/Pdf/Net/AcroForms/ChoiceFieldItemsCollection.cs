...
// Type: Patagames.Pdf.Net.AcroForms.ChoiceFieldItemsCollection
...
/// </summary>
public class ChoiceFieldItemsCollection : 
  IList<ChoiceFieldItem>,
  ICollection<ChoiceFieldItem>,
  IEnumerable<ChoiceFieldItem>,
  IEnumerable
{
  ...
  /// <summary>
  /// Initialize new instance of <see cref="T:Patagames.Pdf.Net.AcroForms.ChoiceFieldItemsCollection" /> class.
  /// </summary>
  /// <param name="field">The <see cref="T:Patagames.Pdf.Net.PdfField" />.</param>
  public ChoiceFieldItemsCollection(PdfField field) => this._field = field;
  ...
  private PdfTypeBase GetItem(ChoiceFieldItem item)
  {
    if (item.Value == null)
      return (PdfTypeBase) PdfTypeString.Create(item.Name, true);
    PdfTypeArray pdfTypeArray = PdfTypeArray.Create();
    pdfTypeArray.Add((PdfTypeBase) PdfTypeString.Create(item.Value, true));
    pdfTypeArray.Add((PdfTypeBase) PdfTypeString.Create(item.Name ?? "", true));
  ...
  public ChoiceFieldItem? Find(Predicate<ChoiceFieldItem> match)
  {
    if (match == null)
      throw new ArgumentNullException(nameof (match));
    if (this._array == null)
      return new ChoiceFieldItem?();
    foreach (ChoiceFieldItem choiceFieldItem in this)
    {
      if (match(choiceFieldItem))
        return new ChoiceFieldItem?(choiceFieldItem);
    }
    return new ChoiceFieldItem?();
  }
  ...
  public ChoiceFieldItem this[int index]
  {
    get
    {
      if (index < 0 || index >= this.Count)
        throw new ArgumentOutOfRangeException();
      if (this._array[index].Is<PdfTypeString>())
        return new ChoiceFieldItem(this._array[index].As<PdfTypeString>().UnicodeString);
      if (this._array[index].Is<PdfTypeName>())
        return new ChoiceFieldItem(this._array[index].As<PdfTypeName>().Value);
      if (this._array[index].Is<PdfTypeNull>())
        return new ChoiceFieldItem((string) null);
      if (!this._array[index].Is<PdfTypeArray>())
        return new ChoiceFieldItem((string) null);
      PdfTypeArray pdfTypeArray = this._array[index].As<PdfTypeArray>();
      string str1 = "";
      string str2 = "";
      if (pdfTypeArray.Count >= 1)
      {
        if (pdfTypeArray[0].Is<PdfTypeString>())
          str2 = pdfTypeArray[0].As<PdfTypeString>().UnicodeString;
        else if (pdfTypeArray[0].Is<PdfTypeName>())
          str2 = pdfTypeArray[0].As<PdfTypeName>().Value;
      }
      if (pdfTypeArray.Count >= 2)
      {
        if (pdfTypeArray[1].Is<PdfTypeString>())
          str1 = pdfTypeArray[1].As<PdfTypeString>().UnicodeString;
        else if (pdfTypeArray[1].Is<PdfTypeName>())
          str1 = pdfTypeArray[1].As<PdfTypeName>().Value;
      }
      return new ChoiceFieldItem(str1 ?? "", str2);
    }
    set
    {
      if (index < 0 || index >= this.Count)
        throw new ArgumentOutOfRangeException();
      this._array[index] = this.GetItem(value);
    }
  }
  ...
  public int IndexOf(ChoiceFieldItem item)
  {
    for (int index = 0; index < this.Count; ++index)
    {
      if (this[index].Name == item.Name && this[index].Value == item.Value)
        return index;
    }
    return -1;
  }
  ...
  public bool Contains(ChoiceFieldItem item) => this.IndexOf(item) >= 0;
  ...
  public void CopyTo(ChoiceFieldItem[] array, int arrayIndex)
  {
    if (array == null)
    ...
    foreach (ChoiceFieldItem choiceFieldItem in this)
    {
      if (arrayIndex > array.Length - 1)
        break;
      array[arrayIndex++] = choiceFieldItem;
    }
  }
  ...
  public IEnumerator<ChoiceFieldItem> GetEnumerator()
  {
    return (IEnumerator<ChoiceFieldItem>) new CollectionEnumerator<ChoiceFieldItem>((IList<ChoiceFieldItem>) this);
  }
  ...
  public void Insert(int index, ChoiceFieldItem item)
  {
    if (index < 0 || index > this.Count)
      throw new ArgumentOutOfRangeException();
    this.AddToField();
    this._array.Insert(index, this.GetItem(item));
  }
  ...
  public void Add(ChoiceFieldItem item)
  {
    this.AddToField();
    this._array.Add(this.GetItem(item));
  }
  ...
  public bool Remove(ChoiceFieldItem item)
  {
