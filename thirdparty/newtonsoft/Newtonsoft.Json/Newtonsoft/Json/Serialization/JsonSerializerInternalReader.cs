...
internal class JsonSerializerInternalReader(JsonSerializer serializer) : JsonSerializerInternalBase(serializer)
{
  ...
  {
    return type == (Type) null ? (JsonContract) null : this.GetContract(type);
  }
  ...
  private JToken? CreateJToken(JsonReader reader, JsonContract? contract)
  {
    ...
    if (contract != null && token != null && !contract.UnderlyingType.IsAssignableFrom(token.GetType()))
      throw JsonSerializationException.Create(reader, "Deserialized JSON type '{0}' is not compatible with expected type '{1}'.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) token.GetType().FullName, (object) contract.UnderlyingType.FullName));
    return token;
  }
  ...
  {
    return StringUtils.IsNullOrEmpty(s) && objectType != (Type) null && objectType != typeof (string) && objectType != typeof (object) && contract != null && contract.IsNullable;
  }
    ...
    out bool ignoredValue)
  {
    ...
    if ((property.ObjectCreationHandling ?? this.Serializer._objectCreationHandling) != ObjectCreationHandling.Replace && (tokenType == JsonToken.StartArray || tokenType == JsonToken.StartObject || propertyConverter != null) && property.Readable)
    {
      currentValue = property.ValueProvider.GetValue(target);
              ...
              try
              {
                switch (primitiveTypeCode)
                {
                  ...
                  case PrimitiveTypeCode.DateTimeOffsetNullable:
                    ...
                    break;
                  default:
                    obj1 = contract.KeyContract == null || !contract.KeyContract.IsEnum ? this.EnsureType(reader, obj1, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType) : EnumUtils.ParseEnum(contract.KeyContract.NonNullableUnderlyingType, this.Serializer._contractResolver is DefaultContractResolver contractResolver ? contractResolver.NamingStrategy : (NamingStrategy) null, obj1.ToString(), false);
                    break;
                }
              }
              catch (Exception ex)
    ...
    foreach (JsonSerializerInternalReader.CreatorPropertyContext creatorPropertyContext in source)
    {
      ...
      if (constructorProperty != null && !constructorProperty.Ignored)
      {
        if (flag)
        {
          ...
          if (this.HasFlag(constructorProperty.DefaultValueHandling ?? this.Serializer._defaultValueHandling, DefaultValueHandling.Populate))
    ...
    try
    {
      ...
      switch (presence)
      {
        case JsonSerializerInternalReader.PropertyPresence.None:
          ...
          if (property.PropertyContract == null)
            property.PropertyContract = this.GetContractSafe(property.PropertyType);
          if (!this.HasFlag(property.DefaultValueHandling ?? this.Serializer._defaultValueHandling, DefaultValueHandling.Populate) || !property.Writable)
            break;
