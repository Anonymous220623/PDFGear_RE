...
public class DiscriminatedUnionConverter : JsonConverter
{
  ...
  {
    return (Type) FSharpUtils.Instance.GetUnionCaseInfoDeclaringType(((IEnumerable<object>) (object[]) FSharpUtils.Instance.GetUnionCases((object) null, (object) t, null)).First<object>());
  }

  private static DiscriminatedUnionConverter.Union CreateUnion(Type t)
  {
    DiscriminatedUnionConverter.Union union = new DiscriminatedUnionConverter.Union((FSharpFunction) FSharpUtils.Instance.PreComputeUnionTagReader((object) null, (object) t, null), new List<DiscriminatedUnionConverter.UnionCase>());
    ...
    {
      DiscriminatedUnionConverter.UnionCase unionCase = new DiscriminatedUnionConverter.UnionCase((int) FSharpUtils.Instance.GetUnionCaseInfoTag(target), (string) FSharpUtils.Instance.GetUnionCaseInfoName(target), (PropertyInfo[]) FSharpUtils.Instance.GetUnionCaseInfoFields(target), (FSharpFunction) FSharpUtils.Instance.PreComputeUnionReader((object) null, target, null), (FSharpFunction) FSharpUtils.Instance.PreComputeUnionConstructor((object) null, target, null));
  ...
  public override bool CanConvert(Type objectType)
  {
    ...
    if (!flag)
      return false;
    return (bool) FSharpUtils.Instance.IsUnion((object) null, (object) objectType, null);
  }
