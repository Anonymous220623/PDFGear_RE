...
// Type: PDFKit.Utils.DrawingHelpers.Indeterminate_GeometryGenerator
...
internal static class Indeterminate_GeometryGenerator
{
  ...
  {
    "M1024 544L0 544 0 480 1024 480Z"
  };
  ...
  public static Geometry Geometry
  {
    get
    {
      if (Indeterminate_GeometryGenerator.geometry == null)
      {
        if (Indeterminate_GeometryGenerator.GeometryData.Count == 0)
          return (Geometry) null;
        GeometryGroup geometryGroup = new GeometryGroup();
        foreach (string source in (IEnumerable<string>) Indeterminate_GeometryGenerator.GeometryData)
          geometryGroup.Children.Add(Geometry.Parse(source));
        geometryGroup.Transform = (Transform) new TranslateTransform(-Indeterminate_GeometryGenerator.Viewbox.Left, -Indeterminate_GeometryGenerator.Viewbox.Top);
        geometryGroup.Freeze();
        Indeterminate_GeometryGenerator.geometry = (Geometry) geometryGroup;
      }
      return Indeterminate_GeometryGenerator.geometry;
    }
  }
    ...
    float strokeWidth)
  {
    ...
    identity.Scale((double) rect.Width / Indeterminate_GeometryGenerator.Viewbox.Width, (double) rect.Height / Indeterminate_GeometryGenerator.Viewbox.Height);
